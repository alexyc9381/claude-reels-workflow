...
export const S3Probe: React.FC<{ lf: number }> = ({ lf }) => {
  const clamp = { extrapolateLeft: "clamp", extrapolateRight: "clamp" } as const;
  const cl = (v: number) => Math.max(0, Math.min(1, v));
  const pulse = (at: number, len: number) => (lf >= at ? Math.max(0, 1 - (lf - at) / len) : 0);
  const ip = (p: number[], v: number[]) => interpolate(lf, p, v, clamp);
  const ipe = (p: number[], v: number[], e: (n: number) => number) => interpolate(lf, p, v, { ...clamp, easing: e });
  const VPX = 972, VPY = 640;                       // vanishing point, on the ground line
  const OLIVE = "#4A5449", OLIVE2 = "#2E3630";      // his tarp: canvas. GREEN (#3F9E74) belongs to the S6 press.

  // ===================== L11 CAMERA: one unbroken push, plus a lazy crane DOWN onto the LEFT PASSAGE =====================
  const camS = ip([0, 254], [1.0, 1.145]);
  const crane = ramp(lf, 150, 254);
  const camOX = interpolate(crane, [0, 1], [520, 232], clamp);
  const camOY = interpolate(crane, [0, 1], [604, 552], clamp);
  const shk = pulse(170, 9) * 5.2 + pulse(188, 13) * 9.6;      // two motivated shakes, 18f apart. The f218 slam gets none.
  const sx = Math.sin(lf * 4.1) * shk + Math.sin(lf * 0.09) * 1.1;
  const sy = Math.cos(lf * 4.7) * shk * 0.65 + Math.cos(lf * 0.11) * 1.1 + crane * 24;

  // ===================== L5 LIGHT ORDER: the sodium dies at us, the floods march away from us =====================
  const sodLive = 1 - ramp(lf, 60, 72);
  const sodFil = (i: number) => {
    if (lf < 56) return 0.86 + Math.abs(Math.sin(lf * 1.9 + i * 2.1)) * 0.12;
    const t = ramp(lf, 56, 70 + i * 3);
    return (1 - t) * (0.3 + (Math.floor(lf * 0.8 + i * 4) % 3 === 0 ? 1 : 0.14) * 0.7);
  };
  const DEEPSOD = [
    { x: 560, base: 690, head: 470, s: 0.42, die: 92 },
    { x: 700, base: 672, head: 512, s: 0.30, die: 112 },
    { x: 810, base: 658, head: 540, s: 0.23, die: 132 },
    { x: 885, base: 650, head: 556, s: 0.18, die: 150 },
  ];
  const BANKS = [
    { x: 110, head: 95, base: 790, s: 1.0, on: 88, near: true },
    { x: 430, head: 238, base: 746, s: 0.74, on: 100, near: true },
    { x: 545, head: 324, base: 706, s: 0.55, on: 112, near: false },
    { x: 730, head: 404, base: 682, s: 0.40, on: 124, near: false },
    { x: 870, head: 462, base: 664, s: 0.29, on: 140, near: false },
    { x: 975, head: 511, base: 650, s: 0.20, on: 248, near: false },
  ];
  const bankLit = (on: number) => {
    if (lf < on) return 0;
    const d = lf - on;
    if (d < 10) return (Math.floor(d / 2) % 2 === 0 ? 0.34 : 0.92) * cl(d / 6);   // hand-thrown: it stutters onto the ballast
    return 0.9 + Math.sin(lf * 0.31 + on) * 0.06;
  };
  const bankShaft = (on: number) => (lf < on ? 0 : cl((lf - on) / (on === 248 ? 62 : 12)));   // bank 6's shaft has not reached the floor at f254
  const coldKey = BANKS.reduce((a, b) => a + bankLit(b.on) * b.s, 0) / 3.2;

  // ===================== L1 THE HERO: pleading at YOU, forged, scrubbing, then measuring himself =====================
  const HS = 262;
  const hu = (u: number) => (u * HS) / 200;                                       // mascot viewBox unit -> px
  const heroX = ip([0, 78, 96, 150, 168, 204, 216, 222, 238, 254], [505, 505, 350, 350, 342, 342, 205, 205, 166, 170]);
  const heroBase = ip([0, 78, 96, 150, 160, 168, 254], [706, 706, 712, 712, 706, 700, 700]);
  const heroKneel = ip([0, 78, 96, 148, 158, 168], [0.8, 0.8, 0.62, 0.62, 0.78, 1.0]);
  const heroTilt = ip([0, 78, 96, 148, 158, 168], [0, 0, 15, 15, 6, 0]);
  const heroTop = heroBase - 0.92 * HS;
  const scrub = lf >= 96 && lf < 148 ? Math.sin(lf * 0.62) * 11 : 0;              // he is WORKING, not watching
  const stampHit = pulse(42, 7);
  const heroWalk = lf >= 204 && lf < 216 ? Math.sin(lf * 1.35) * 3.2 : 0;
  const measure = ramp(lf, 222, 240);                                             // the nub goes flat on his own head
  const nubTopY = heroTop + hu(24) - measure * 11;                                // 479 at full measure
  const BAND_T = 440, BAND_B = 453;                                               // the red PROVEN band: 193px clear of the ground line
  const smear = 34 + ramp(lf, 58, 130) * 30;                                      // scrubbing only ever spreads it
  const heroGaze = lf < 50 ? 0 : lf < 96 ? -2 : lf < 172 ? -3 : lf < 240 ? 2 : ip([240, 246, 254], [2, 5, 3]);
  // the costume must ride the sprite's own bob, or the cap floats off his head
  const HNOD = lf >= 96 && lf < 148 ? 1.1 : 3.2, HSPD = lf >= 204 && lf < 216 ? 6 : 10;
  const hHopP = Math.max(0, Math.sin(lf / (HSPD * 0.6)));
  const hBob = `translateY(${-hHopP * HNOD * 2.2}px) scaleY(${1 - hHopP * 0.045})`;

  // ===================== L2 THE VILLAIN: his podium, his wall, his evidence =====================
  const villX = ip([0, 176, 188, 194, 200, 206, 214, 224, 254], [612, 612, 672, 706, 748, 760, 714, 714, 890]);
  const villFeet = ip([0, 176, 182, 188, 194, 200, 206, 224, 254], [558, 558, 552, 516, 616, 716, 716, 716, 688]);
  const villRot = ip([0, 186, 190, 196, 201, 208, 214, 254], [0, 0, -24, -66, -90, -90, 0, 0]);
  const villSc = ip([0, 224, 254], [1, 1, 0.536]);
  const VS = 280 * villSc;
  const villTop = villFeet - 0.92 * VS;
  const villLean = ip([0, 34, 42, 52, 58], [0, 0, 16, 16, 0]);                    // he leans in over the hero's shoulder
  const crouch = lf >= 176 && lf < 181 ? 0.9 : 0;
  const glint = pulse(150, 10) + pulse(188, 6);
  const rainHitch = lf < 188 ? 0 : (Math.floor(lf / 4) % 3 === 0 ? 2 : 0) + (seed(Math.floor(lf / 2)) > 0.72 ? 1 : 0);
  const rainO = lf < 188 ? 0.95 : (Math.floor(lf / 3) % 4 === 0 ? 0.2 : 0.5);     // his aura is damaged and you can see it
  const dab = lf >= 206 ? Math.sin(lf * 0.55) * 7 : 0;
  const VNOD = lf >= 224 ? 3.4 : 1.1, VSPD = lf >= 224 ? 8 : 14;
  const vHopP = Math.max(0, Math.sin((lf - rainHitch) / (VSPD * 0.6)));
  const vBob = `translateY(${-vHopP * VNOD * 2.2}px) scaleY(${1 - vHopP * 0.045})`;

  // the counterfeit die: shoulder -> lick -> the hero's forehead -> examined -> the plate -> broken forever
  const stFree = lf < 214;
  const SKEYS = [0, 28, 34, 42, 48, 58, 100, 130, 176, 180, 188, 194, 201, 208, 214];
  const stX = stFree ? ip(SKEYS, [700, 700, 652, 505, 505, 690, 560, 560, 700, 662, 700, 742, 790, 776, 824]) : ip([214, 224, 254], [824, 824, 949]);
  const stY = stFree ? ip(SKEYS, [400, 400, 424, 552, 552, 396, 430, 430, 400, 452, 470, 560, 700, 712, 597]) : ip([214, 224, 254], [597, 597, 624]);
  const stRot = lf < 58 ? ip([0, 28, 34, 42, 48, 58], [-14, -14, 6, 12, 12, -10])
    : lf < 130 ? -10 + Math.sin(lf * 0.09) * 32
      : lf < 176 ? -12
        : lf < 188 ? ip([176, 188], [-12, 26])
          : lf < 214 ? ip([188, 196, 206, 214], [26, -140, -300, -352]) : -8;
  const stSc = ip([0, 224, 254], [1, 1, 0.536]);
  const broke = lf >= 188;
  const shatter = pulse(188, 16);

  // ===================== L3 THE GATE: swung in on a jib at rail height, planted, riveted, still ringing =====================
  const gateIn = lf >= 120;
  const gBase = ipe([120, 140], [-1046, 0], Easing.out(Easing.cubic));
  const gHunt = lf < 150 ? Math.sin((lf - 120) * 0.42) * 52 * (1 - ramp(lf, 120, 150)) : 0;   // it hunts on its chains and overshoots twice
  const gateX = gBase + gHunt;
  const gateY = ip([120, 148, 150, 152, 154], [-16, -16, 0, 4, 0]);
  const gateRot = lf < 150 ? Math.sin((lf - 120) * 0.3) * 1.7 * (1 - ramp(lf, 120, 150)) : 0;
  const ring = lf >= 170 ? Math.sin(lf * 0.9) * 0.85 * Math.exp(-(lf - 170) / 150) : 0;       // the plate never fully settles
  const RIV = [152, 158, 164, 170];
  const rivShake = RIV.reduce((a, t) => a + pulse(t, 6), 0);

  // ===================== L4 THE ARM: one shaft, two answers =====================
  const armLift = ramp(lf, 206, 210) - ramp(lf, 216, 218);
  const ARMY = 548, APX = 256;                                                    // rest height: straight across his eyes
  const armDeg = interpolate(armLift, [0, 1], [0, 74], clamp) + (lf >= 218 ? Math.sin((lf - 218) * 1.1) * 1.4 * Math.exp(-(lf - 218) / 26) : 0);
  const shaftRot = armLift * 5 + ramp(lf, 236, 254) * 3;
  const chuteRot = lf < 236 ? 270 : ip([236, 254, 282], [270, 211, 180]);         // starts down, never lands on screen
  const bandOn = lf >= 202;

  // ===================== SET-DRESSING CLOCKS =====================
  const signFall = ipe([126, 135], [0, 1], Easing.in(Easing.quad));
  const signRock = lf >= 135 ? Math.sin((lf - 135) * 0.22) * 5.6 * Math.exp(-(lf - 135) / 95) : 0;
  const fest = lf < 126 ? 400 + Math.sin(lf * 0.09) * 7 : lf < 132 ? ip([126, 132], [400, 484]) : ip([132, 140], [484, 414]) + Math.sin(lf * 0.14) * 3;
  const boomX = ip([0, 95, 126, 130, 160, 190, 220, 250, 254], [700, 700, 700, 516, 474, 432, 390, 348, 342]);
  const LETTERS = "AUTOKARE";
  const LDROP: Record<number, number> = { 7: 130, 6: 160, 5: 190, 4: 220 };
  const attWand = lf >= 112;
  const pickX = ip([0, 138, 152, 158, 164, 170, 186, 254], [866, 866, 606, 658, 710, 766, 830, 830]);
  const pickY = ip([0, 138, 152, 200, 212, 254], [672, 672, 470, 470, 536, 536]);
  const buggyT = ramp(lf, 180, 254);
  const buggyX = 520 + buggyT * 375 - (lf >= 236 && lf < 250 ? 24 : 0);           // steps aside for him, never looks at him
  const buggyY = 786 - buggyT * 122;
  const buggyS = 210 - buggyT * 140;
  const tarpPull = ramp(lf, 228, 240);
  const rookX = ip([0, 196, 204, 210, 216, 240, 254], [-60, -40, 150, 168, 196, 276, 306]);
  const rookY = ip([0, 196, 204, 210, 216, 240, 254], [730, 730, 718, 712, 706, 664, 652]);
  const rookS = ip([0, 196, 212, 216, 240, 254], [258, 258, 258, 220, 84, 62]);

  const flood = (i: number) => {
    const b = BANKS[i], litv = bankLit(b.on), sh = bankShaft(b.on);
    const w = 96 * b.s, z = b.near ? 12 : 4;
    return (
      <React.Fragment key={"fb" + i}>
        <div style={{ position: "absolute", left: b.x - 4 * b.s, top: b.head, width: 8 * b.s, height: b.base - b.head, background: grad("#3B424E", "#171B22"), zIndex: z }} />
        <div style={{ position: "absolute", left: b.x - w / 2, top: b.head - 16 * b.s, width: w, height: 26 * b.s, borderRadius: 3, background: grad("#4C5462", "#20242D"), border: `${1.5 * b.s}px solid rgba(190,210,240,0.22)`, zIndex: z }}>
          {[0, 1, 2, 3].map((k) => (
            <div key={k} style={{ position: "absolute", left: (3 + k * 23) * b.s, top: 4 * b.s, width: 19 * b.s, height: 17 * b.s, borderRadius: 2, background: "#EAF3FF", opacity: 0.1 + litv * 0.9, boxShadow: `0 0 ${20 * b.s * litv}px rgba(214,236,255,${litv})` }} />
          ))}
        </div>
        {litv > 0.02 && (
          <div style={{ position: "absolute", left: b.x - w * 1.5, top: b.head, width: w * 3, height: (760 - b.head) * sh, background: "linear-gradient(180deg, rgba(212,234,255,0.26), rgba(190,220,255,0.06) 62%, transparent)", clipPath: "polygon(44% 0%, 56% 0%, 100% 100%, 0% 100%)", mixBlendMode: "screen", opacity: litv * 0.9, zIndex: 24, pointerEvents: "none" }} />
        )}
      </React.Fragment>
    );
  };

  const sodMast = (x: number, base: number, head: number, s: number, i: number, deep: boolean, dieAt: number) => {
    const g = deep ? (1 - ramp(lf, dieAt - 8, dieAt)) * (0.82 + Math.abs(Math.sin(lf * 1.6 + i)) * 0.14) : sodFil(i);
    const z = deep ? 4 : 11;
    return (
      <React.Fragment key={"sm" + (deep ? "d" : "n") + i}>
        <div style={{ position: "absolute", left: x - 4 * s, top: head, width: 8 * s, height: base - head, background: grad("#39323C", "#1B1720"), zIndex: z }} />
        <div style={{ position: "absolute", left: x - 26 * s, top: head - 12 * s, width: 52 * s, height: 15 * s, borderRadius: "5px 5px 10px 10px", background: grad("#4A4250", "#241F2B"), border: `${1.2 * s}px solid rgba(255,220,160,0.18)`, boxShadow: `0 0 ${18 * s * g}px rgba(255,196,90,0.5)`, zIndex: z }} />
        <div style={{ position: "absolute", left: x - 19 * s, top: head + s, width: 38 * s, height: 7 * s, borderRadius: 4, background: "#FFD383", opacity: g, boxShadow: `0 0 ${24 * s * g}px rgba(255,196,90,0.9)`, zIndex: z }} />
        {g > 0.02 && !deep && (
          <div style={{ position: "absolute", left: x - 230 * s, top: head + 6, width: 460 * s, height: 792 - head, background: "linear-gradient(180deg,rgba(255,196,90,0.20),rgba(255,178,80,0.05) 58%,transparent)", clipPath: "polygon(46% 0%, 54% 0%, 100% 100%, 0% 100%)", mixBlendMode: "screen", opacity: g, zIndex: 24, pointerEvents: "none" }} />
        )}
      </React.Fragment>
    );
  };

  const crew = (k: string, x: number, base: number, s: number, z: number, o = 1, gaze = 0, nod = 2.2) => (
    <div key={k} style={{ position: "absolute", left: x - s / 2, top: base - 0.92 * s, width: s, height: s, zIndex: z, opacity: o }}>
      <div style={{ position: "absolute", left: s * 0.1, top: s * 0.9, width: s * 0.8, height: s * 0.07, borderRadius: "50%", background: "rgba(0,0,0,0.5)", filter: "blur(4px)" }} />
      <Mascot lf={lf + s} size={s} hiVis={1} gaze={gaze} nodAmp={nod} nodSpeed={11} />
    </div>
  );

  const SLOTS = [64, 243, 422, 780];                                              // four empty. The rack could hold five rules.
  const SKIRT = [[58, 92], [260, 498], [900, 54]];                                // the two gaps ARE the two passages

  return (
    <Panel label="one-rule-gate">
      <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, overflow: "hidden", background: "#0B0D12" }}>
        <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, transform: `translate(${sx}px, ${sy}px) scale(${camS})`, transformOrigin: `${camOX}px ${camOY}px` }}>

          {/* ============ SKY ============ */}
          <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 646, background: "linear-gradient(180deg,#0D1019 0%,#151A27 46%,#231E28 100%)", zIndex: 1 }} />
          {Array.from({ length: 20 }).map((_, i) => {
            const r = seed(i * 3.1 + 7);
            return <div key={"stz" + i} style={{ position: "absolute", left: 20 + r * 970, top: 40 + seed(i * 2.2) * 190, width: 2 + r * 2, height: 2 + r * 2, borderRadius: "50%", background: "rgba(220,232,255,0.55)", opacity: (0.16 + Math.abs(Math.sin(lf * 0.04 + i)) * 0.28) * (0.35 + coldKey * 0.65), zIndex: 1 }} />;
          })}

          {/* ============ 20. THE SHUTTERED UNIT (explicitly no roller shutter: S0 owns that verb) ============ */}
          <div style={{ position: "absolute", left: 180, top: 250, width: 380, height: 392, background: grad("#2A2432", "#171420"), border: "2px solid rgba(200,180,150,0.13)", zIndex: 2 }} />
          <div style={{ position: "absolute", left: 172, top: 244, width: 396, height: 13, background: grad("#3A3242", "#221C2C"), zIndex: 2 }} />
          {LETTERS.split("").map((ch, i) => {
            const dt = LDROP[i];
            const fall = dt ? ramp(lf, dt, dt + 22) : 0;
            if (dt && lf > dt + 62) return <div key={"lf" + i} style={{ position: "absolute", left: 206 + i * 42, top: 626, width: 34, height: 12, background: "#2A2E38", borderRadius: 2, transform: `rotate(${8 + i * 5}deg)`, zIndex: 2 }} />;
            return (
              <div key={"lt" + i} style={{ position: "absolute", left: 206 + i * 42 + fall * (6 - i), top: 278 + fall * fall * 348, width: 34, height: 48, borderRadius: 3, background: grad("#413A4C", "#251F30"), border: "1.5px solid rgba(190,175,200,0.16)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: fraunces.fontFamily, fontWeight: 900, fontSize: 30, color: "rgba(215,200,230,0.3)", transform: `rotate(${fall * (26 + i * 7)}deg) scaleY(${1 - fall * 0.12})`, zIndex: 2 }}>{ch}</div>
            );
          })}
          {[0, 1].map((i) => (
            <div key={"wc" + i} style={{ position: "absolute", left: 214 + i * 190, top: 380, width: 122, height: 96, background: "linear-gradient(180deg,#151A24,#0C0F16)", border: "2px solid #33303E", zIndex: 2, backgroundImage: "repeating-linear-gradient(45deg, rgba(150,160,180,0.2) 0 1.2px, transparent 1.2px 12px), repeating-linear-gradient(-45deg, rgba(150,160,180,0.2) 0 1.2px, transparent 1.2px 12px)" }} />
          ))}
          {/* the dead tube sign, strobing on a broken ballast for the entire scene */}
          <div style={{ position: "absolute", left: 232, top: 508, width: 138, height: 15, borderRadius: 7, background: "#B9CBE0", opacity: Math.floor(lf / 3) % 7 === 0 ? 0.75 : Math.floor(lf / 5) % 4 === 0 ? 0.3 : 0.06, boxShadow: Math.floor(lf / 3) % 7 === 0 ? "0 0 24px rgba(190,215,245,0.7)" : "none", zIndex: 2 }} />
          {/* THE KEY CABINET: wired glass, red LOCKED bolt, the ring inside. Never lit, never approached, never referenced. */}
          <div style={{ position: "absolute", left: 392, top: 474, width: 74, height: 70, background: "linear-gradient(180deg,#242B33,#141920)", border: "2px solid #39424C", zIndex: 2, backgroundImage: "repeating-linear-gradient(45deg, rgba(120,135,150,0.18) 0 1px, transparent 1px 9px), repeating-linear-gradient(-45deg, rgba(120,135,150,0.18) 0 1px, transparent 1px 9px)" }}>
            <div style={{ position: "absolute", left: 6, top: 54, width: 30, height: 7, borderRadius: 2, background: "#6E2A22" }} />
            <div style={{ position: "absolute", left: 30, top: 16, width: 15, height: 15, borderRadius: "50%", border: "2.5px solid rgba(140,120,70,0.45)" }} />
            <div style={{ position: "absolute", left: 35, top: 30, width: 5, height: 17, background: "rgba(140,120,70,0.4)" }} />
            <div style={{ position: "absolute", left: 41, top: 26, width: 4, height: 13, background: "rgba(140,120,70,0.32)" }} />
          </div>
          {/* the skip and its burst bin bag, shredding for 254 frames */}
          <div style={{ position: "absolute", left: 462, top: 590, width: 92, height: 52, background: grad("#4A3A2C", "#2A2018"), border: "1.5px solid rgba(200,170,130,0.14)", transform: "skewX(-6deg)", zIndex: 2 }} />
          {Array.from({ length: 7 }).map((_, i) => {
            const r = seed(i * 5.7 + 2);
            return <div key={"bg" + i} style={{ position: "absolute", left: 548 + r * 18, top: 604 + r * 26, width: 4 + r * 9, height: 3 + r * 4, background: "rgba(30,34,44,0.9)", transform: `rotate(${Math.sin(lf * (0.11 + r * 0.1) + i) * 42}deg)`, zIndex: 2 }} />;
          })}

          {/* ============ 18. THE JIFFY LOOB SIGN: the old regime's promise, unbolted and dropped face-down ============ */}
          <div style={{ position: "absolute", left: 596, top: 244, width: 320, height: 398, background: grad("#272130", "#151220"), border: "2px solid rgba(200,180,150,0.1)", zIndex: 2 }} />
          <div style={{ position: "absolute", left: 620 + signFall * 4, top: 250 + signFall * 362, width: 260, height: 95, transform: `rotate(${signRock}deg) scaleY(${1 - signFall * 0.68})`, transformOrigin: "50% 100%", zIndex: 2 }}>
            <div style={{ width: 260, height: 95, borderRadius: 999, background: signFall > 0.9 ? grad("#5A4C34", "#302819") : grad("#F0C63E", "#C9902A"), border: `4px solid ${signFall > 0.9 ? "#4A2018" : "#B2372A"}`, boxShadow: signFall > 0.9 ? "0 8px 16px rgba(0,0,0,0.7)" : `0 18px 34px rgba(0,0,0,0.6), 0 0 ${22 + Math.abs(Math.sin(lf * 0.11)) * 16}px rgba(240,198,62,0.32)`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: signFall > 0.9 ? 1 : lf % 97 < 3 ? 0.62 : 1 }}>
              {signFall < 0.5 && <>
                <div style={{ fontFamily: fraunces.fontFamily, fontWeight: 900, fontSize: 34, color: "#8E2A1E", lineHeight: 1 }}>JIFFY LOOB</div>
                <div style={{ fontFamily: mono, fontSize: 10.5, fontWeight: 700, color: "rgba(60,26,14,0.85)", letterSpacing: 1, marginTop: 4 }}>CERTIFIED IN 5 MINUTES OR IT'S FREE</div>
              </>}
            </div>
            {signFall < 0.05 && [0, 1, 2, 3].map((i) => {
              const out = ramp(lf, 95 + i * 8, 102 + i * 8);
              return <div key={"bo" + i} style={{ position: "absolute", left: 26 + i * 70, top: 6 - out * 5, width: 11, height: 11, borderRadius: "50%", background: grad("#C9CFDC", "#5A6274"), transform: `rotate(${out * 700}deg)`, opacity: 1 - out }} />;
            })}
          </div>

          {/* ============ 19. THE FESTOON: flapping, dragged slack, then draped on the top chord ============ */}
          <svg width={1012} height={792} style={{ position: "absolute", left: 0, top: 0, zIndex: 3, pointerEvents: "none" }}>
            <path d={`M 620 268 Q 370 ${fest} 120 300`} fill="none" stroke="rgba(210,196,180,0.34)" strokeWidth={2} />
            {Array.from({ length: 15 }).map((_, i) => {
              const t = (i + 0.5) / 15, mt = 1 - t;
              const px = mt * mt * 620 + 2 * mt * t * 370 + t * t * 120;
              const py = mt * mt * 268 + 2 * mt * t * fest + t * t * 300;
              return <polygon key={"fp" + i} points={`${px - 7},${py} ${px + 7},${py} ${px},${py + 17}`} fill={["#C8543E", "#E7B24C", "#4C7FA3", "#D9D3C4"][i % 4]} opacity={0.5} transform={`rotate(${Math.sin(lf * 0.16 + i * 1.1) * (lf < 126 ? 12 : 4)} ${px} ${py})`} />;
            })}
          </svg>

          {/* ============ 21. THE BACK FENCE + ITS RATTLING BANNER ============ */}
          <div style={{ position: "absolute", left: 0, top: 558, width: 950, height: 84, zIndex: 3, backgroundImage: "repeating-linear-gradient(45deg, rgba(170,185,205,0.24) 0 1.4px, transparent 1.4px 11px), repeating-linear-gradient(-45deg, rgba(170,185,205,0.24) 0 1.4px, transparent 1.4px 11px)", borderTop: "2px solid rgba(170,185,205,0.3)" }} />
          <div style={{ position: "absolute", left: 300, top: 570, width: 120, height: 30, background: "rgba(120,60,50,0.5)", border: "1px solid rgba(220,190,170,0.2)", transform: `rotate(${Math.sin(lf * 0.19) * 2.6}deg) skewY(${Math.sin(lf * 0.24) * 2}deg)`, zIndex: 3 }} />

          {/* ============ 16/15. FLOOD BANKS 3..6 + THE DEEP SODIUM DYING IN A WAVE (upstage) ============ */}
          {[2, 3, 4, 5].map((i) => flood(i))}
          {DEEPSOD.map((m, i) => sodMast(m.x, m.base, m.head, m.s, i + 4, true, m.die))}

          {/* ============ 11. S1's EMPTY TWO-POST LIFT: the old regime's machine, on the wrong side of a fence ============ */}
          {[690, 830].map((px, i) => <div key={"tp" + i} style={{ position: "absolute", left: px, top: 520, width: 13, height: 152, borderRadius: 3, background: grad("#6B5A28", "#3A3016"), opacity: 0.72, zIndex: 5 }} />)}
          <div style={{ position: "absolute", left: 703, top: 646, width: 60, height: 7, borderRadius: 3, background: "rgba(120,130,150,0.45)", zIndex: 5 }} />

          {/* ============ 31. THE S4 BAY GLOW, sitting ON the vanishing point. It is already running. ============ */}
          <div style={{ position: "absolute", left: VPX - 20, top: VPY - 15, width: 40, height: 30, background: "linear-gradient(180deg,rgba(150,190,230,0.3),rgba(110,150,200,0.14))", filter: "blur(1.6px)", zIndex: 5 }}>
            <div style={{ position: "absolute", left: 6 + Math.abs(((lf * 0.9) % 52) - 26), top: 17, width: 5, height: 8, background: "rgba(20,26,36,0.8)" }} />
          </div>

          {/* ============ 27. THE TEARDOWN BOOM LIFT: it never finishes ============ */}
          <div style={{ position: "absolute", left: 694, top: 620, width: 42, height: 44, borderRadius: 4, background: grad("#5E5326", "#2C2612"), zIndex: 6, opacity: 0.85 }} />
          <svg width={1012} height={792} style={{ position: "absolute", left: 0, top: 0, zIndex: 6, pointerEvents: "none" }}>
            <line x1={715} y1={626} x2={boomX + 22} y2={334} stroke="#5E5326" strokeWidth={11} opacity={0.8} />
            <line x1={715} y1={626} x2={boomX + 22} y2={334} stroke="#8A7A38" strokeWidth={3} opacity={0.5} />
          </svg>
          <div style={{ position: "absolute", left: boomX, top: 300, width: 46, height: 36, border: "2.5px solid #7B6C30", borderTop: "none", background: "rgba(40,34,18,0.5)", zIndex: 6 }} />
          <div style={{ position: "absolute", left: boomX + 4, top: 268, width: 40, height: 40, zIndex: 6 }}><Mascot lf={lf} size={40} hiVis={1} nodAmp={1.4} nodSpeed={13} /></div>
          {lf >= 250 && <div style={{ position: "absolute", left: boomX + 40, top: 288, width: 20, height: 28, borderRadius: 2, background: grad("#413A4C", "#251F30"), transform: `rotate(${Math.sin(lf * 0.3) * 12}deg)`, zIndex: 6 }} />}

          {/* ============ 13/24. THE APRON + THE LANE PAINT, converging on the point ============ */}
          <div style={{ position: "absolute", left: 0, top: 640, width: 1012, height: 152, background: "linear-gradient(180deg,#2A2531 0%,#332B33 34%,#3E3338 100%)", zIndex: 7 }} />
          <svg width={1012} height={792} style={{ position: "absolute", left: 0, top: 0, zIndex: 7, pointerEvents: "none" }}>
            {[-460, -90, 300, 700, 1120, 1560].map((bx, i) => <line key={"cv" + i} x1={bx} y1={792} x2={VPX} y2={VPY} stroke="rgba(255,196,120,0.07)" strokeWidth={2} />)}
            <line x1={500} y1={792} x2={952} y2={650} stroke="rgba(240,238,230,0.55)" strokeWidth={5} />
            <line x1={735} y1={792} x2={960} y2={646} stroke="rgba(240,238,230,0.4)" strokeWidth={4} />
          </svg>
          {[[190, 690, 130, 22], [640, 668, 108, 17], [860, 654, 76, 12]].map((o, i) => (
            <div key={"oil" + i} style={{ position: "absolute", left: o[0], top: o[1], width: o[2], height: o[3], borderRadius: "50%", background: "rgba(10,8,14,0.5)", filter: "blur(3px)", zIndex: 7 }} />
          ))}
          <div style={{ position: "absolute", left: 412, top: 730, width: 58, height: 22, borderRadius: 2, background: grad("#3B3F48", "#1D2027"), border: "2px solid #4E545E", zIndex: 7 }}>
            {[0, 1, 2, 3].map((k) => <div key={k} style={{ position: "absolute", left: 5, top: 3 + k * 4.4, width: 44, height: 2, background: "#12151A" }} />)}
          </div>
          {/* 22. the tyre stack, and the one the fourth rivet's shake knocks off */}
          {[0, 1, 2].map((i) => <div key={"ty" + i} style={{ position: "absolute", left: 8 + i * 3, top: 716 - i * 26, width: 84, height: 27, borderRadius: 13, background: grad("#26232C", "#131118"), border: "3px solid #3A3641", zIndex: 7 }} />)}
          {(() => {
            const t = lf >= 170 ? ramp(lf, 170, 254) : 0;
            return <div style={{ position: "absolute", left: 12 + t * 318, top: 638 + t * t * 124, width: 84 * (1 + t * 0.18), height: 27 + t * 52, borderRadius: t > 0.1 ? "50%" : 13, background: grad("#26232C", "#131118"), border: "3px solid #3A3641", transform: `rotate(${t * 620}deg)`, zIndex: t > 0 ? 28 : 7 }} />;
          })()}
          <div style={{ position: "absolute", left: 560 - ramp(lf, 0, 254) * 55 - rivShake * 3, top: 770 + ramp(lf, 0, 254) * 20, width: 13, height: 13, background: grad("#9AA2B2", "#4A5060"), clipPath: "polygon(25% 0%,75% 0%,100% 50%,75% 100%,25% 100%,0% 50%)", transform: `rotate(${lf * 3.4}deg)`, zIndex: 28 }} />

          {/* ============ 1..7. THE ONE-RULE GATE: architecture, planted from below, never a portal ============ */}
          {gateIn && (
            <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, zIndex: 8, transform: `translate(${gateX}px, ${gateY}px) rotate(${gateRot}deg)`, transformOrigin: "506px 560px" }}>
              {[8, 954].map((ux, i) => (
                <React.Fragment key={"up" + i}>
                  <div style={{ position: "absolute", left: ux, top: 380, width: 50, height: 320, background: grad("#5D646E", "#262B33"), border: "2px solid rgba(190,208,232,0.18)", boxShadow: "0 16px 30px rgba(0,0,0,0.6), inset 2px 0 0 rgba(255,255,255,0.14)" }} />
                  <div style={{ position: "absolute", left: ux - 12, top: 690, width: 74, height: 16, borderRadius: 2, background: grad("#6B7280", "#2B303A") }} />
                  {[0, 1, 2, 3].map((k) => <div key={k} style={{ position: "absolute", left: ux - 6 + (k % 2) * 58, top: 693 + Math.floor(k / 2) * 8, width: 8, height: 8, borderRadius: "50%", background: "#8C95A4", opacity: lf >= 150 ? 1 : 0.2 }} />)}
                </React.Fragment>
              ))}
              {/* the chain-link skirt, in three runs. The two gaps ARE the two passages. */}
              {SKIRT.map((s, i) => (
                <div key={"sk" + i} style={{ position: "absolute", left: s[0], top: 590, width: s[1], height: 110, backgroundImage: "repeating-linear-gradient(45deg, rgba(178,194,214,0.34) 0 1.5px, transparent 1.5px 12px), repeating-linear-gradient(-45deg, rgba(178,194,214,0.34) 0 1.5px, transparent 1.5px 12px)", borderTop: "3px solid rgba(178,194,214,0.4)" }} />
              ))}
              {/* the cross-rail slot rack */}
              <div style={{ position: "absolute", left: 58, top: 400, width: 896, height: 15, background: grad("#68707C", "#2C323B"), boxShadow: "inset 0 2px 0 rgba(255,255,255,0.16)" }} />
              <div style={{ position: "absolute", left: 58, top: 575, width: 896, height: 15, background: grad("#5A616C", "#242931") }} />
              <div style={{ position: "absolute", left: 60, top: 590, width: 5, height: 34, background: "linear-gradient(180deg,rgba(160,86,40,0.55),transparent)" }} />
              <div style={{ position: "absolute", left: 402, top: 590, width: 4, height: 26, background: "linear-gradient(180deg,rgba(160,86,40,0.5),transparent)" }} />
              {/* 4. FOUR EMPTY SLOTS, bolt holes bare. That is "one rule" with no reading. */}
              {SLOTS.map((sxx, i) => (
                <React.Fragment key={"sl" + i}>
                  <div style={{ position: "absolute", left: sxx, top: 400, width: 172, height: 190, border: "4px solid rgba(96,104,116,0.9)", boxSizing: "border-box" }} />
                  {[[6, 6], [158, 6], [6, 174], [158, 174]].map((p, k) => <div key={k} style={{ position: "absolute", left: sxx + p[0], top: 400 + p[1], width: 9, height: 9, borderRadius: "50%", background: "#12151B", boxShadow: "inset 0 1px 2px rgba(0,0,0,0.9)" }} />)}
                  {i === 1 && <div style={{ position: "absolute", left: sxx + 12, top: 414, width: 148, height: 162, border: "2px solid rgba(150,160,175,0.13)" }} />}
                </React.Fragment>
              ))}
              {/* 2. THE PLATE (the scene's ONE header) + 3. THE CAST 1 (the scene's only number, cast into the same steel) */}
              <div style={{ position: "absolute", left: 601, top: 400, width: 172, height: 190, background: "linear-gradient(160deg,#7A828D 0%,#5A626D 44%,#3E454E 100%)", border: "4px solid #2C323A", boxSizing: "border-box", boxShadow: "0 14px 26px rgba(0,0,0,0.6), inset 0 2px 0 rgba(255,255,255,0.2)", transform: `rotate(${ring}deg)`, transformOrigin: "50% 4%" }}>
                <svg viewBox="0 0 172 190" width={172} height={190} style={{ position: "absolute", left: 0, top: 0 }}>
                  <polygon points="23,30 47,14 49,156 23,156" fill="#8E96A2" />
                  <polygon points="23,30 47,14 47,28 27,42" fill="#6E7682" />
                  <rect x={9} y={156} width={54} height={14} fill="#8E96A2" />
                  <rect x={9} y={166} width={54} height={4} fill="#4A515B" />
                  <polygon points="23,30 27,42 27,156 23,156" fill="#5E656F" />
                  <rect x={33} y={44} width={4} height={110} fill="rgba(160,86,40,0.45)" />
                  <rect x={30} y={62} width={9} height={5} fill="rgba(30,34,40,0.5)" />
                  <rect x={38} y={112} width={7} height={5} fill="rgba(30,34,40,0.45)" />
                </svg>
                <div style={{ position: "absolute", left: 66, top: 20, width: 102, height: 152, display: "flex", flexDirection: "column", justifyContent: "center", gap: 3 }}>
                  {["NO SOLO", "WORK", "UNTIL", "PROVEN"].map((t, i) => (
                    <div key={i} style={{ fontFamily: mono, fontWeight: 700, fontSize: 24, letterSpacing: -0.7, lineHeight: "30px", color: "rgba(238,242,248,0.9)", textShadow: "0 2px 0 rgba(0,0,0,0.55), 0 -1px 0 rgba(255,255,255,0.16)" }}>{t}</div>
                  ))}
                </div>
                <div style={{ position: "absolute", left: 0, top: 0, width: 172, height: 190, boxShadow: "inset 0 0 34px rgba(10,14,22,0.6)" }} />
              </div>
              {RIV.map((t, i) => lf < t ? null : (
                <div key={"rv" + i} style={{ position: "absolute", left: 606 + i * 52, top: 401, width: 13, height: 13, borderRadius: "50%", background: grad("#D6DCE6", "#69707C"), transform: `scale(${interpolate(over(lf, t, 4, Easing.out(Easing.back(3))), [0, 1], [2.1, 1], clamp)})`, boxShadow: "0 3px 5px rgba(0,0,0,0.7)" }} />
              ))}
              {/* 26. the pointless bracket, riveted on after the rule is already up */}
              {lf >= 210 && <div style={{ position: "absolute", left: 754, top: 590, width: 30, height: 16, background: grad("#8A929E", "#454C56"), transform: "skewX(-12deg)", opacity: over(lf, 210, 4) }} />}
              {/* 7. THE SHAFT: one law, two arms, two answers */}
              <div style={{ position: "absolute", left: 58, top: 388, width: 896, height: 9, borderRadius: 5, background: grad("#98A2B2", "#3E4550") }} />
              {[260, 758].map((cx, i) => (
                <div key={"cp" + i} style={{ position: "absolute", left: cx - 13, top: 380, width: 26, height: 26, borderRadius: "50%", background: grad("#B0B8C6", "#4A515C"), border: "2px solid rgba(20,24,30,0.6)", transform: `rotate(${shaftRot * 4}deg)` }}>
                  <div style={{ position: "absolute", left: 10, top: 1, width: 3, height: 10, background: "#20252D" }} />
                </div>
              ))}
            </div>
          )}

          {/* ============ 25. THE CRANE JIB: lateral, at rail height. Never a descent from the top of frame. ============ */}
          {lf >= 118 && lf < 164 && (
            <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, zIndex: 9, opacity: 1 - ramp(lf, 152, 162) }}>
              {(() => { const jx = gateX + 300 - ramp(lf, 150, 162) * 260; return (<>
                <div style={{ position: "absolute", left: jx, top: 332, width: 200, height: 15, borderRadius: 3, background: grad("#D8A93A", "#6E5418"), boxShadow: "0 8px 16px rgba(0,0,0,0.6)" }} />
                <div style={{ position: "absolute", left: jx + 168, top: 336, width: 26, height: 26, borderRadius: 4, background: grad("#9AA2B2", "#3E4550") }} />
                {[0, 1].map((i) => <div key={"ch" + i} style={{ position: "absolute", left: jx + 152 + i * 42, top: 356, width: 3, height: 46 - gateY, background: "rgba(190,200,215,0.6)", transform: `rotate(${gHunt * 0.16}deg)`, transformOrigin: "50% 0%" }} />)}
              </>); })()}
            </div>
          )}

          {/* ============ 5/6. THE HEIGHT POST, THE PRE-OWNED RED BAND, AND THE TWO ARMS ============ */}
          {gateIn && (
            <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, zIndex: 10, transform: `translate(${gateX}px, ${gateY}px)` }}>
              <div style={{ position: "absolute", left: 250, top: 418, width: 22, height: 282, background: grad("#6B7280", "#2B303A"), border: "1.5px solid rgba(200,215,240,0.2)", boxShadow: "0 12px 22px rgba(0,0,0,0.55)" }} />
              {[422, 428, 434, 462, 470, 478].map((ny, i) => <div key={"nt" + i} style={{ position: "absolute", left: 251, top: ny, width: 9, height: 3, background: "rgba(230,238,250,0.45)" }} />)}
              {/* scuffed, chipped, one bolt short: the institution keeps a stock of these */}
              {bandOn && (
                <div style={{ position: "absolute", left: 246, top: BAND_T, width: 32, height: BAND_B - BAND_T, background: grad("#D8483A", "#8E2418"), border: "1px solid rgba(255,200,190,0.3)", opacity: over(lf, 202, 4), boxShadow: "0 4px 8px rgba(0,0,0,0.5)" }}>
                  <div style={{ position: "absolute", left: 3, top: 3, width: 6, height: 6, borderRadius: "50%", background: "#5E1810" }} />
                  <div style={{ position: "absolute", left: 18, top: 4, width: 7, height: 4, background: "rgba(90,24,16,0.7)" }} />
                  <div style={{ position: "absolute", left: 30, top: 2, width: 10, height: 9, background: grad("#D8483A", "#8E2418"), transform: `rotate(${Math.sin(lf * 0.35) * 17}deg)`, transformOrigin: "0% 50%" }} />
                </div>
              )}
              {lf >= 202 && <div style={{ position: "absolute", left: 259, top: 456, width: 8, height: 8, borderRadius: "50%", background: grad("#D6DCE6", "#69707C"), transform: `scale(${interpolate(over(lf, 202, 4, Easing.out(Easing.back(3))), [0, 1], [2, 1], clamp)})` }} />}
              {/* THE CHUTE ARM: same shaft, starts down at f236, still descending at f254 */}
              <div style={{ position: "absolute", left: 900, top: 645, width: 11, height: 58, background: grad("#5A616C", "#242931"), zIndex: 2 }} />
              <div style={{ position: "absolute", left: 905, top: 641, width: 92, height: 8, transformOrigin: "0% 50%", transform: `rotate(${chuteRot}deg)`, borderRadius: 3, overflow: "hidden" }}>
                {Array.from({ length: 4 }).map((_, i) => <div key={i} style={{ position: "absolute", left: i * 23, top: 0, width: 23, height: 8, background: i % 2 ? "#D8483A" : "#F0EDE4" }} />)}
              </div>
            </div>
          )}

          {/* ============ 14/16. THE NEAR SODIUM MASTS + FLOOD BANKS 1..2 (downstage) ============ */}
          {[0, 1].map((i) => flood(i))}
          {sodMast(306, 772, 182, 1, 0, false, 72)}
          {sodMast(490, 758, 230, 0.94, 1, false, 72)}
          {lf >= 60 && lf < 96 && [306, 490].map((mx, i) => (
            <React.Fragment key={"gl" + i}>
              {Array.from({ length: 7 }).map((_, k) => {
                const r = seed(i * 9 + k * 3.3), age = lf - 62 - i * 6 - k * 2;
                if (age < 0 || age > 30) return null;
                const t = age / 30;
                return <div key={k} style={{ position: "absolute", left: mx - 18 + r * 36 + t * (r - 0.5) * 30, top: 196 + i * 44 + t * t * 540, width: 3, height: 5, background: "rgba(240,230,200,0.8)", transform: `rotate(${t * 340}deg)`, opacity: 1 - t * 0.5, zIndex: 26 }} />;
              })}
            </React.Fragment>
          ))}

          {/* ============ 12. THE PUDDLE: the rule arrives in the water a beat before it arrives in the frame ============ */}
          <div style={{ position: "absolute", left: 240, top: 706, width: 180, height: 38, borderRadius: "50%", overflow: "hidden", background: "rgba(6,8,12,0.9)", zIndex: 13, boxShadow: "0 0 12px rgba(0,0,0,0.6)" }}>
            <div style={{ position: "absolute", inset: 0, background: `linear-gradient(180deg, rgba(255,190,90,${0.5 * sodLive}), rgba(190,120,40,${0.2 * sodLive}))` }} />
            <div style={{ position: "absolute", inset: 0, background: `linear-gradient(180deg, rgba(200,228,255,${0.42 * coldKey}), rgba(120,170,220,${0.14 * coldKey}))` }} />
            {lf >= 120 && (
              <div style={{ position: "absolute", left: 6 + Math.max(-170, gateX * 0.16), top: 5, width: 168, height: 28, opacity: 0.5, transform: "scaleY(-1)" }}>
                <div style={{ position: "absolute", left: 0, top: 4, width: 168, height: 3, background: "rgba(190,210,240,0.7)" }} />
                <div style={{ position: "absolute", left: 0, top: 20, width: 168, height: 2.5, background: "rgba(190,210,240,0.5)" }} />
                <div style={{ position: "absolute", left: 96, top: 4, width: 30, height: 18, background: "rgba(210,225,246,0.55)" }} />
                {[10, 40, 70, 130, 158].map((v, i) => <div key={i} style={{ position: "absolute", left: v, top: 2, width: 2, height: 26, background: "rgba(190,210,240,0.4)" }} />)}
              </div>
            )}
            {/* one travelling band of his rain: the thing that carries the near-black */}
            {lf >= 66 && lf < 96 && <CodeRain lf={lf} x={((lf - 66) * 4) % 150} y={-4} h={46} cols={3} o={1} gap={13} />}
            {/* rings the fourth rivet put in it, which have not settled at f254 */}
            {lf >= 170 && [0, 1, 2].map((i) => {
              const t = (((lf - 170) * 0.9 + i * 22) % 66) / 66;
              return <div key={"rg" + i} style={{ position: "absolute", left: 90 - t * 84, top: 19 - t * 15, width: t * 168, height: t * 30, borderRadius: "50%", border: "1.5px solid rgba(220,238,255,0.4)", opacity: (1 - t) * 0.7 }} />;
            })}
            {lf < 60 && [0, 1].map((i) => <div key={"sq" + i} style={{ position: "absolute", left: 40 + Math.sin(lf * 0.16 + i * 2) * 44, top: 6 + i * 12, width: 52, height: 3, background: "rgba(255,225,170,0.4)", filter: "blur(1px)" }} />)}
          </div>

          {/* ============ 8/9/10. THE WRECK: his podium at f0, his wall at f200, his evidence at f228 ============ */}
          {/* S1's fire is out, but the cowl is still hot: the only warm pool on this side of the lot, and it is what the villain stands in */}
          <div style={{ position: "absolute", left: 400, top: 470, width: 440, height: 320, borderRadius: "50%", background: `radial-gradient(ellipse, rgba(255,150,60,${0.2 + Math.abs(Math.sin(lf * 0.07)) * 0.06}), transparent 66%)`, filter: "blur(12px)", mixBlendMode: "screen", zIndex: 13, pointerEvents: "none" }} />
          <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, zIndex: 14, isolation: "isolate" }}>
            <div style={{ position: "absolute", left: 470, top: 742, width: 290, height: 22, borderRadius: "50%", background: "rgba(0,0,0,0.62)", filter: "blur(9px)" }} />
            <div style={{ position: "absolute", left: 490, top: 735, width: 74, height: 26, borderRadius: "8px 4px 4px 8px", background: grad("#B6B0A2", "#6E695E"), border: "1.5px solid rgba(255,255,255,0.2)", transform: "rotate(-5deg) skewX(-16deg)", boxShadow: "0 8px 14px rgba(0,0,0,0.6)" }} />
            <div style={{ position: "absolute", left: 478, top: 700, width: 264, height: 54, borderRadius: "10px 14px 6px 6px", background: grad("#9A9488", "#4E4A42"), border: "2px solid rgba(255,255,255,0.16)", transform: "rotate(1.6deg)", boxShadow: "0 14px 26px rgba(0,0,0,0.5)" }} />
            <div style={{ position: "absolute", left: 490, top: 654, width: 240, height: 54, borderRadius: "8px 8px 2px 2px", background: grad("#B4AEA0", "#5E594F"), border: "2px solid rgba(255,255,255,0.14)", transform: "rotate(1.2deg)" }} />
            <div style={{ position: "absolute", left: 530, top: 558, width: 148, height: 100, borderRadius: "16px 12px 2px 2px", background: grad("#A79F92", "#4A463E"), border: "2px solid rgba(255,255,255,0.18)", boxShadow: "0 12px 22px rgba(0,0,0,0.45)" }}>
              <div style={{ position: "absolute", left: 12, top: 16, width: 122, height: 44, borderRadius: 4, background: grad("#1E2838", "#0E141F"), border: "1.5px solid rgba(170,195,235,0.2)", overflow: "hidden" }}>
                {[0, 1, 2].map((k) => <div key={k} style={{ position: "absolute", left: 8 + k * 30, top: 2, width: 2, height: 42, background: "rgba(220,235,255,0.3)", transform: `rotate(${11 + k * 8}deg)` }} />)}
              </div>
            </div>
            {/* both front wheels gone: two bare hubs */}
            {[[498, 716], [560, 722]].map((h, i) => (
              <div key={"hub" + i} style={{ position: "absolute", left: h[0], top: h[1], width: 30, height: 30, borderRadius: "50%", background: grad("#6E5236", "#2A1E12"), border: "3px solid rgba(255,190,120,0.16)", boxShadow: "inset 0 2px 7px rgba(0,0,0,0.85)" }}>
                {[0, 1, 2, 3].map((k) => <div key={k} style={{ position: "absolute", left: 11 + Math.cos(k * 1.57) * 8, top: 11 + Math.sin(k * 1.57) * 8, width: 4, height: 4, borderRadius: "50%", background: "#150F0A" }} />)}
              </div>
            ))}
            {[[672, 714], [716, 710]].map((w, i) => <div key={"whl" + i} style={{ position: "absolute", left: w[0], top: w[1], width: 40, height: 40, borderRadius: "50%", background: "#171520", border: "5px solid #35313C" }} />)}
            {/* 9. THE LIGHT BAR: his footrest, it rattles on every rivet, its lens goes when he tumbles past it */}
            <div style={{ position: "absolute", left: 590, top: 544 - rivShake * 1.6, width: 88, height: 14, borderRadius: 3, background: grad("#3E444E", "#1A1E25"), transform: `rotate(${-1.4 + (lf >= 170 ? Math.sin(lf * 0.5) * 1.6 * Math.exp(-(lf - 170) / 40) : 0)}deg)`, boxShadow: "0 5px 9px rgba(0,0,0,0.6)" }}>
              <div style={{ position: "absolute", left: 4, top: 3, width: 24, height: 8, borderRadius: 2, background: "rgba(200,60,50,0.55)" }} />
              <div style={{ position: "absolute", left: 34, top: 3, width: 24, height: 8, borderRadius: 2, background: "rgba(60,110,200,0.45)" }} />
            </div>
            {lf < 196 ? (
              <div style={{ position: "absolute", left: 664, top: 552 + Math.sin(lf * 0.24) * 3, width: 16, height: 12, borderRadius: 2, background: "rgba(200,70,55,0.5)", border: "1px solid rgba(255,190,170,0.3)", transform: `rotate(${34 + Math.sin(lf * 0.24) * 10}deg)`, transformOrigin: "0% 0%" }}>
                <div style={{ position: "absolute", left: 3, top: 0, width: 1.5, height: 12, background: "rgba(255,230,220,0.5)", transform: "rotate(18deg)" }} />
              </div>
            ) : Array.from({ length: 6 }).map((_, k) => {
              const r = seed(k * 4.4 + 1), t = cl((lf - 196) / 26);
              return <div key={"lp" + k} style={{ position: "absolute", left: 668 + (r - 0.5) * 60 * t, top: 556 + t * t * 180, width: 4, height: 3, background: "rgba(200,70,55,0.7)", transform: `rotate(${t * 400}deg)`, opacity: 1 - t * 0.4 }} />;
            })}
            {/* nine counterfeit passes, matching S1's shipped signature */}
            {[[500, 682, -12], [548, 700, 9], [592, 664, -6], [628, 690, 13], [664, 668, -10], [700, 700, 5], [736, 686, -14], [566, 590, 7], [634, 600, -8]].map((s, i) => (
              <DoneSticker key={"ds" + i} x={s[0]} y={s[1]} s={0.31} rot={s[2]} o={0.95} />
            ))}
            {/* 10. THE TARP: one corner lifting in the draught from f0, yanked over the wreck at f228 */}
            <div style={{ position: "absolute", left: 706 - tarpPull * 214, top: 690 - tarpPull * 124, width: 66 + tarpPull * 214, height: 34 + tarpPull * 100, borderRadius: tarpPull > 0.4 ? "20px 14px 4px 4px" : 8, background: `linear-gradient(160deg, ${OLIVE}, ${OLIVE2})`, border: "1.5px solid rgba(150,170,150,0.16)", boxShadow: "0 8px 16px rgba(0,0,0,0.6)", transform: `skewX(${-4 + tarpPull * 3}deg)` }}>
              {Array.from({ length: 5 }).map((_, k) => <div key={k} style={{ position: "absolute", left: 5 + k * (11 + tarpPull * 42), top: 4, width: 2, height: 28 + tarpPull * 92, background: "rgba(0,0,0,0.28)", transform: `skewX(${Math.sin(lf * 0.09 + k) * 4}deg)` }} />)}
              <div style={{ position: "absolute", left: -6, top: 2, width: 20, height: 15, background: OLIVE, transform: `rotate(${-22 - Math.abs(Math.sin(lf * 0.11)) * 20}deg)`, transformOrigin: "100% 100%", borderRadius: 3 }} />
            </div>
          </div>

          {/* ============ 26. THE CHERRY-PICKER + THE BOLT-GUN CLAUDE: working machinery needs a working guy ============ */}
          <svg width={1012} height={792} style={{ position: "absolute", left: 0, top: 0, zIndex: 15, pointerEvents: "none" }}>
            <line x1={880} y1={704} x2={pickX + 26} y2={pickY + 34} stroke="#5E5326" strokeWidth={13} />
            <line x1={880} y1={704} x2={pickX + 26} y2={pickY + 34} stroke="#9A8838" strokeWidth={4} opacity={0.5} />
            <path d={`M ${pickX + 40} ${pickY + 34} Q ${pickX + 60} 742 880 708`} fill="none" stroke="rgba(30,34,44,0.85)" strokeWidth={3} />
          </svg>
          <div style={{ position: "absolute", left: 852, top: 694, width: 58, height: 26, borderRadius: 3, background: grad("#5E5326", "#2C2612"), zIndex: 15 }} />
          <div style={{ position: "absolute", left: pickX, top: pickY, width: 54, height: 38, border: "3px solid #7B6C30", borderTop: "none", background: "rgba(40,34,18,0.45)", zIndex: 15, boxShadow: "0 8px 16px rgba(0,0,0,0.5)" }} />
          <div style={{ position: "absolute", left: pickX + 5, top: pickY - 34, width: 46, height: 46, zIndex: 15 }}>
            <Mascot lf={lf} size={46} hiVis={1} nodAmp={rivShake > 0.1 ? 5 : 1.6} nodSpeed={9} />
          </div>

          {/* ============ 17. THE SPIDER BOX + THE SPARKY: every light in this scene is a nub on a lever ============ */}
          <div style={{ position: "absolute", left: 188, top: 694, width: 58, height: 52, borderRadius: 4, background: grad("#3E444E", "#191D24"), border: "2px solid rgba(190,210,240,0.16)", zIndex: 16, boxShadow: "0 10px 18px rgba(0,0,0,0.6)" }}>
            {BANKS.map((b, i) => <div key={"br" + i} style={{ position: "absolute", left: 5 + (i % 3) * 16, top: 7 + Math.floor(i / 3) * 21, width: 9, height: 14, borderRadius: 2, background: lf >= b.on ? "#C9522F" : "#5A616C", transform: `rotate(${lf >= b.on ? 0 : 26}deg)`, transformOrigin: "50% 100%" }} />)}
          </div>
          <div style={{ position: "absolute", left: 236, top: 740, width: 210, height: 4, borderRadius: 2, background: "rgba(24,26,34,0.9)", transform: "rotate(3deg)", zIndex: 16 }} />
          {crew("sparky", 146, 786, 106, 16, 1, -2)}
          <div style={{ position: "absolute", left: 184, top: 706 - (lf >= 248 ? Math.sin(lf * 0.9) * 2 : 0), width: 22, height: 8, borderRadius: 3, background: "#D97757", zIndex: 16, transform: "rotate(-14deg)" }} />

          {/* ============ 28. THE ATTENDANT: conscripted into the OS and has not noticed ============ */}
          {crew("att", 392, 782, 106, 16, 1, attWand ? -3 : 3)}
          {attWand ? (
            <div style={{ position: "absolute", left: 424, top: 684 - ramp(lf, 120, 150) * 14, width: 8, height: 40, borderRadius: 4, background: grad("#D8483A", "#7A1E14"), boxShadow: `0 0 ${12 + Math.abs(Math.sin(lf * 0.5)) * 12}px rgba(216,72,58,0.8)`, transformOrigin: "50% 100%", transform: `rotate(${lf < 150 ? -32 + Math.sin(lf * 0.34) * 22 : -14 + Math.sin(lf * 0.5) * 34}deg)`, zIndex: 16 }} />
          ) : (
            <div style={{ position: "absolute", left: 360, top: 726, width: 6, height: 54, background: "#8A6844", transformOrigin: "50% 100%", transform: `rotate(${-26 + Math.sin(lf * 0.16) * 22}deg)`, zIndex: 16 }}>
              <div style={{ position: "absolute", left: -12, top: 50, width: 30, height: 6, borderRadius: 2, background: "#2E3440" }} />
            </div>
          )}
          <div style={{ position: "absolute", left: 432, top: 756, width: 22, height: 20, borderRadius: "2px 2px 5px 5px", background: grad("#5A616C", "#242931"), zIndex: 16 }} />

          {/* ============ 24. THE PAINT BUGGY: still 60px short of the point at f254 ============ */}
          {lf >= 180 && (<>
            {crew("bug", buggyX, buggyY, buggyS, 16, 0.95, 3)}
            <div style={{ position: "absolute", left: buggyX + buggyS * 0.22, top: buggyY - buggyS * 0.1, width: buggyS * 0.4, height: buggyS * 0.16, borderRadius: 4, background: grad("#8A929E", "#3A4048"), zIndex: 16 }} />
            {Array.from({ length: 5 }).map((_, k) => {
              const r = seed(k * 2.7 + Math.floor(lf / 3));
              return <div key={"mist" + k} style={{ position: "absolute", left: buggyX + buggyS * 0.3 + r * 22, top: buggyY - r * 20, width: 2 + r * 2, height: 2 + r * 2, borderRadius: "50%", background: "rgba(245,242,235,0.55)", opacity: 0.5, zIndex: 24 }} />;
            })}
          </>)}

          {/* ============ 30. THE LEFT-OUT ROOKIE: upside-down L, rotated when he thinks nobody is looking ============ */}
          <div style={{ position: "absolute", left: -7, top: 581, width: 190, height: 190, zIndex: 17, opacity: 0.62 }}>
            <Mascot lf={lf + 13} size={190} nodAmp={1.8} nodSpeed={13} gaze={lf < 244 ? 3 : 0} />
            <div style={{ position: "absolute", left: 108, top: 100, width: 30, height: 30, borderRadius: 3, background: PAPER, border: `2px solid ${RED}`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: fraunces.fontFamily, fontWeight: 900, fontSize: 20, color: RED, transform: `rotate(${lf < 244 ? 180 : interpolate(over(lf, 244, 5), [0, 1], [180, 0], clamp)}deg)` }}>L</div>
            <div style={{ position: "absolute", left: 34, top: 22, width: 122, height: 20, borderRadius: "8px 8px 3px 3px", background: grad("#3A4763", "#1E2739") }} />
            {lf >= 248 && <div style={{ position: "absolute", left: 66, top: 22 - over(lf, 248, 5) * 12, width: 58, height: 9, borderRadius: 3, background: "#D97757", opacity: over(lf, 248, 4) }} />}
          </div>

          {/* ============ 29. THE PLATELESS ROOKIE: the arm lifts, he walks off alone, never looks back ============ */}
          {lf >= 196 && (
            <div style={{ position: "absolute", left: rookX - rookS / 2, top: rookY - 0.92 * rookS, width: rookS, height: rookS, zIndex: 17, opacity: interpolate(rookS, [62, 210], [0.42, 1], clamp) }}>
              <div style={{ position: "absolute", left: rookS * 0.1, top: rookS * 0.9, width: rookS * 0.8, height: rookS * 0.06, borderRadius: "50%", background: "rgba(0,0,0,0.5)", filter: "blur(4px)" }} />
              <Mascot lf={lf + 7} size={rookS} nodAmp={3.6} nodSpeed={7} gaze={-4} />
            </div>
          )}

          {/* ============ L2 THE VILLAIN ============ */}
          <div style={{ position: "absolute", left: villX - VS / 2, top: villTop, width: VS, height: VS, zIndex: 18, transform: `rotate(${villRot}deg)`, transformOrigin: "50% 92%" }}>
            <div style={{ position: "absolute", left: VS * 0.12, top: VS * 0.9, width: VS * 0.76, height: VS * 0.06, borderRadius: "50%", background: "rgba(0,0,0,0.55)", filter: "blur(5px)", opacity: villRot === 0 ? 1 : 0.3 }} />
            <div style={{ width: VS, height: VS, transform: `rotate(${villLean + dab * 0.3}deg) scaleY(${1 - crouch * 0.16})`, transformOrigin: "50% 100%" }}>
              {/* he is a black suit at night: give him a cold separation halo so the silhouette reads */}
              <div style={{ position: "absolute", left: -VS * 0.16, top: VS * 0.05, width: VS * 1.32, height: VS * 1.0, borderRadius: "50%", background: `radial-gradient(ellipse, rgba(150,180,150,${0.13 + coldKey * 0.1}), transparent 62%)`, filter: "blur(9px)", mixBlendMode: "screen", zIndex: 0 }} />
              <Villain lf={lf - rainHitch} size={VS} rain={rainO} gaze={lf >= 150 && lf < 176 ? -3 : lf >= 200 && lf < 210 ? -4 : 0} nodAmp={VNOD} nodSpeed={VSPD} />
              <div style={{ position: "absolute", left: 0, top: 0, width: VS, height: VS, transform: vBob, transformOrigin: "50% 100%", zIndex: 4 }}>
                {/* a hard rim down his leading edge, so a black suit still has a shape */}
                <div style={{ position: "absolute", left: VS * 0.17, top: VS * 0.22, width: VS * 0.022, height: VS * 0.52, background: "rgba(255,214,150,0.5)", borderRadius: 2 }} />
                <div style={{ position: "absolute", left: VS * 0.17, top: VS * 0.22, width: VS * 0.66, height: VS * 0.022, background: "rgba(255,214,150,0.42)", borderRadius: 2 }} />
                {/* MICRO-GAG: a bandolier of spare dies. He has broken stamps before. He does NOT reload. */}
                <div style={{ position: "absolute", left: VS * 0.24, top: VS * 0.6, width: VS * 0.52, height: VS * 0.045, background: "#171A20", transform: "rotate(-16deg)" }}>
                  {[0, 1, 2, 3].map((k) => <div key={k} style={{ position: "absolute", left: `${9 + k * 23}%`, top: "-25%", width: "9%", height: "150%", borderRadius: 1, background: grad("#98A03E", "#5C6420") }} />)}
                </div>
                {/* his mouth: a flat line, and only on the shatter */}
                {shatter > 0.05 && <div style={{ position: "absolute", left: VS * 0.44, top: VS * 0.55, width: VS * 0.12, height: VS * 0.028, background: "#0E1014", borderRadius: 1, opacity: Math.min(1, shatter * 2.4) }} />}
                {glint > 0.02 && <div style={{ position: "absolute", left: VS * 0.24, top: VS * 0.33, width: VS * 0.16, height: VS * 0.035, background: "#FFFFFF", opacity: glint * 0.9, transform: "rotate(-16deg)", filter: "blur(0.6px)" }} />}
                {/* MICRO-GAG, THE COLLAR: his own ink on his own white collar, and it will not dab out */}
                {lf >= 190 && [[0.4, 0.53, 0.09], [0.47, 0.57, 0.055], [0.53, 0.52, 0.04]].map((s, i) => (
                  <div key={"ink" + i} style={{ position: "absolute", left: VS * s[0], top: VS * s[1], width: VS * s[2], height: VS * s[2] * 0.8, borderRadius: "50%", background: FAKE, opacity: 0.85, transform: `rotate(${i * 30}deg)` }} />
                ))}
                {lf >= 206 && <div style={{ position: "absolute", left: VS * 0.3 + dab * 0.5, top: VS * 0.52, width: VS * 0.085, height: VS * 0.085, background: "#E6E3DA", transform: `rotate(${dab * 3}deg)` }} />}
              </div>
            </div>
          </div>

          {/* ============ THE COUNTERFEIT DIE, and the one damage state that carries to S7 ============ */}
          <div style={{ position: "absolute", left: stX - 30 * stSc, top: stY - 26 * stSc, width: 60 * stSc, height: 52 * stSc, zIndex: 19, transform: `rotate(${stRot}deg)`, transformOrigin: "50% 50%" }}>
            <div style={{ position: "absolute", left: 20 * stSc, top: 14 * stSc, width: 18 * stSc, height: 40 * stSc, borderRadius: 3, background: grad("#6E7788", "#2B3240"), border: `${1.4 * stSc}px solid rgba(210,225,250,0.28)` }} />
            <div style={{ position: "absolute", left: 0, top: 0, width: 60 * stSc, height: 22 * stSc, borderRadius: 4, background: broke ? grad("#5A6274", "#232833") : grad("#7C8698", "#333B4A"), border: `${1.6 * stSc}px solid rgba(220,235,255,0.3)`, boxShadow: `0 ${5 * stSc}px ${10 * stSc}px rgba(0,0,0,0.6)`, overflow: "hidden" }}>
              <div style={{ position: "absolute", left: 4 * stSc, top: 14 * stSc, width: 52 * stSc, height: 7 * stSc, background: broke ? "#4A4E38" : FAKE, boxShadow: broke ? "none" : `0 0 ${8 * stSc}px rgba(168,184,74,0.8)` }} />
              {broke && <>
                <div style={{ position: "absolute", left: 26 * stSc, top: 0, width: 3 * stSc, height: 22 * stSc, background: "#0C0E12", transform: "rotate(9deg)" }} />
                <div style={{ position: "absolute", left: 8 * stSc, top: 3 * stSc, width: 18 * stSc, height: 2 * stSc, background: "#0C0E12", transform: "rotate(-14deg)" }} />
              </>}
            </div>
            {/* the spring, out of the crack */}
            {broke && Array.from({ length: 4 }).map((_, k) => <div key={"sp" + k} style={{ position: "absolute", left: (28 + k * 3) * stSc, top: (-8 + k * 2.2) * stSc, width: 11 * stSc, height: 2 * stSc, borderRadius: 2, background: "#A8B0C0", transform: `rotate(${k % 2 ? 22 : -22}deg)` }} />)}
            {/* the brass flip-counter, jammed from f188 */}
            <div style={{ position: "absolute", left: 44 * stSc, top: 26 * stSc, width: 12 * stSc, height: 9 * stSc, borderRadius: 2, background: broke ? "#6A5A2A" : grad("#E0B84C", "#8A6A1E"), transform: `rotate(${broke ? 24 : 0}deg)` }} />
          </div>

          {/* THE SHATTER: the plate does not move. He does. */}
          {lf >= 188 && lf < 232 && Array.from({ length: 26 }).map((_, k) => {
            const r = seed(k * 3.7 + 5), a = (k / 26) * Math.PI * 2 + r, t = cl((lf - 188) / 44);
            const d = Math.pow(t, 0.55) * (60 + r * 190);
            return <div key={"shd" + k} style={{ position: "absolute", left: 700 + Math.cos(a) * d, top: 470 + Math.sin(a) * d + t * t * 210, width: 4 + r * 7, height: 3 + r * 5, background: r > 0.62 ? "rgba(230,240,255,0.85)" : FAKE, borderRadius: r > 0.8 ? "50%" : 1, transform: `rotate(${t * 520 * (r - 0.5)}deg)`, opacity: 1 - t * 0.8, boxShadow: r > 0.62 ? "none" : "0 0 6px rgba(168,184,74,0.7)", zIndex: 30 }} />;
          })}
          {shatter > 0.02 && <div style={{ position: "absolute", left: 620, top: 390, width: 160, height: 160, borderRadius: "50%", background: `radial-gradient(circle, rgba(168,184,74,${shatter * 0.45}), transparent 66%)`, mixBlendMode: "screen", zIndex: 29 }} />}
          {/* 13. THE SHARD: spinning at the grate's lip like a dropped coin, never settling */}
          {lf >= 190 && <div style={{ position: "absolute", left: 449, top: 743, width: 13, height: 10, background: FAKE, borderRadius: 1, transform: `scaleX(${Math.abs(Math.cos(lf * (0.24 + ramp(lf, 190, 254) * 0.5)))}) rotate(${Math.sin(lf * 0.1) * 12}deg)`, boxShadow: "0 0 7px rgba(168,184,74,0.7)", zIndex: 30 }} />}

          {/* ============ L1 THE HERO ============ */}
          <div style={{ position: "absolute", left: heroX - HS / 2 + scrub, top: heroTop, width: HS, height: HS, zIndex: 20, transform: `scaleY(${heroKneel}) rotate(${heroTilt + stampHit * 8 + heroWalk}deg)`, transformOrigin: "50% 100%" }}>
            <div style={{ position: "absolute", left: hu(20), top: hu(184), width: hu(160), height: hu(14), borderRadius: "50%", background: "rgba(0,0,0,0.55)", filter: "blur(6px)" }} />
            <Mascot lf={lf} size={HS} gaze={heroGaze} nodAmp={HNOD} nodSpeed={HSPD} />
            {/* the costume rides the sprite's own bob */}
            <div style={{ position: "absolute", left: 0, top: 0, width: HS, height: HS, transform: hBob, transformOrigin: "50% 100%" }}>
              <div style={{ position: "absolute", left: hu(34), top: hu(100), width: hu(132), height: hu(50), background: grad("#31456E", "#1A2740"), borderRadius: 3 }} />
              <div style={{ position: "absolute", left: hu(34), top: hu(94), width: hu(132), height: hu(7), background: "rgba(240,236,226,0.8)" }} />
              {/* the L-plate, worn square on the chest like a marathon bib. It is the only thing the gate reads. */}
              <div style={{ position: "absolute", left: hu(112), top: hu(104), width: hu(40), height: hu(40), borderRadius: 4, background: PAPER, border: `${hu(3)}px solid ${RED}`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: fraunces.fontFamily, fontWeight: 900, fontSize: hu(26), color: RED, boxShadow: "0 5px 10px rgba(0,0,0,0.4)", transform: `rotate(${Math.sin(lf * 0.08) * 1.5}deg)` }}>L</div>
              {/* the driving-school cap, one size too big, sitting flat and askew ON his head */}
              <div style={{ position: "absolute", left: hu(30), top: hu(28), width: hu(140), height: hu(22), borderRadius: "9px 9px 3px 3px", background: grad("#3A4763", "#1E2739"), border: `${hu(1.5)}px solid rgba(200,215,245,0.18)`, transform: "rotate(-3deg)", transformOrigin: "50% 100%" }} />
              <div style={{ position: "absolute", left: hu(2), top: hu(42), width: hu(46), height: hu(9), borderRadius: "6px 3px 3px 6px", background: grad("#2E3950", "#1A2333"), transform: "rotate(-3deg)", transformOrigin: "100% 50%" }} />
              {/* f42: his own grey copy answers the question for him, and the answer is a forgery on his face */}
              {lf >= 42 && (
                <div style={{ position: "absolute", left: hu(100) - smear * 0.42, top: hu(46), width: smear, height: smear * 0.4, borderRadius: 3, background: `linear-gradient(150deg, ${FAKE}, #8FA03C)`, opacity: 0.92, transform: `rotate(-14deg) scale(${interpolate(over(lf, 42, 5, Easing.out(Easing.back(3))), [0, 1], [1.7, 1], clamp)})`, transformOrigin: "50% 50%", filter: lf > 58 ? "blur(0.7px)" : "none", boxShadow: "0 2px 5px rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: fraunces.fontFamily, fontWeight: 900, fontSize: 14, color: "rgba(40,46,20,0.65)", overflow: "hidden" }}>DONE</div>
              )}
              {/* by f140 his nubs are yellow-green too: the lie is on his face AND on his hands */}
              {lf >= 118 && [8, 166].map((ax, i) => <div key={"nb" + i} style={{ position: "absolute", left: hu(ax), top: hu(86), width: hu(26), height: hu(26), background: FAKE, opacity: ramp(lf, 118, 140) * 0.8, borderRadius: 2 }} />)}
              {/* the blank provisional card: held out to YOU in his left nub, ALWAYS clear of his eyes (the bible's tell) */}
              {lf < 80 ? (
                <div style={{ position: "absolute", left: hu(-8), top: hu(94) + (1 - ramp(lf, 0, 12)) * 6, width: hu(66), height: hu(44), borderRadius: 4, background: grad("#F7F3EA", "#D8D2C4"), border: "1.5px solid rgba(255,255,255,0.6)", boxShadow: "0 8px 16px rgba(0,0,0,0.5)", transform: `rotate(${-7 + Math.sin(lf * 0.24) * 3}deg) scale(${1 + Math.sin(lf * 0.3) * 0.03})`, zIndex: 4 }}>
                  <div style={{ position: "absolute", left: 4, top: 4, width: hu(20), height: hu(28), border: "1.5px dashed rgba(120,116,104,0.6)", borderRadius: 2 }} />
                  <div style={{ position: "absolute", left: hu(28), top: hu(10), width: hu(32), height: 2.5, background: "rgba(150,146,134,0.5)" }} />
                  <div style={{ position: "absolute", left: hu(28), top: hu(19), width: hu(26), height: 2.5, background: "rgba(150,146,134,0.5)" }} />
                </div>
              ) : (<>
                <div style={{ position: "absolute", left: hu(96), top: hu(96), width: 2, height: hu(28), background: "rgba(30,34,44,0.8)", transformOrigin: "50% 0%", transform: `rotate(${Math.sin(lf * 0.2) * 9}deg)`, zIndex: 4 }} />
                <div style={{ position: "absolute", left: hu(84), top: hu(122), width: hu(30), height: hu(20), borderRadius: 3, background: grad("#F7F3EA", "#D8D2C4"), border: "1px solid rgba(255,255,255,0.5)", transformOrigin: "40% -30%", transform: `rotate(${Math.sin(lf * 0.2) * 9}deg)`, zIndex: 4 }} />
              </>)}
              {/* f222: the nub goes flat on top of his own head. Nobody asked him to. */}
              {measure > 0.02 && <div style={{ position: "absolute", left: hu(64), top: hu(24) - measure * 11, width: hu(72), height: hu(11), borderRadius: 3, background: "#D97757", opacity: measure, boxShadow: "0 3px 6px rgba(0,0,0,0.4)" }} />}
            </div>
          </div>
          {/* f240: he LOOKS, and his nub is sitting a clear gap under the red band */}
          {measure > 0.55 && gateIn && (<>
            <div style={{ position: "absolute", left: heroX - 47, top: nubTopY, width: Math.max(12, 250 - (heroX - 47)), height: 2, backgroundImage: "repeating-linear-gradient(90deg, rgba(240,244,252,0.95) 0 7px, transparent 7px 14px)", opacity: 0.3 + ramp(lf, 238, 248) * 0.6, zIndex: 21 }} />
            <div style={{ position: "absolute", left: 238, top: BAND_B, width: 4, height: nubTopY - BAND_B, background: RED, opacity: 0.35 + ramp(lf, 240, 246) * (0.35 + Math.abs(Math.sin(lf * 0.3)) * 0.3), zIndex: 21 }} />
          </>)}

          {/* ============ 4. THE PEDESTRIAN ARM: staged IN FRONT of him, because the beat is the bar crossing his face ============ */}
          {gateIn && (
            <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, zIndex: 21, transform: `translate(${gateX}px, ${gateY}px)` }}>
              <div style={{ position: "absolute", left: APX - 7, top: ARMY - 7, width: 14, height: 14, borderRadius: "50%", background: grad("#B0B8C6", "#4A515C") }} />
              {/* the short arm + its counterweight can, on the far side of the pivot */}
              <div style={{ position: "absolute", left: APX, top: ARMY - 6, width: 44, height: 12, transformOrigin: "0% 50%", transform: `rotate(${armDeg}deg)` }}>
                <div style={{ position: "absolute", left: 0, top: 3, width: 30, height: 6, background: "#3B414B", borderRadius: 2 }} />
                <div style={{ position: "absolute", left: 24, top: -8, width: 18, height: 28, borderRadius: 4, background: grad("#5A616C", "#242931"), border: "1px solid rgba(200,215,240,0.18)" }} />
              </div>
              <div style={{ position: "absolute", left: APX, top: ARMY - 6, width: 124, height: 12, transformOrigin: "0% 50%", transform: `rotate(${180 + armDeg}deg)`, borderRadius: 3, overflow: "hidden", boxShadow: "0 8px 16px rgba(0,0,0,0.6)" }}>
                {Array.from({ length: 7 }).map((_, i) => <div key={i} style={{ position: "absolute", left: i * 18, top: 0, width: 18, height: 12, background: i % 2 ? "#D8483A" : "#F0EDE4" }} />)}
                <div style={{ position: "absolute", left: 0, top: 0, width: 124, height: 3, background: "rgba(255,255,255,0.3)" }} />
              </div>
              {/* the rubber down-stop it ticks on */}
              <div style={{ position: "absolute", left: 240, top: ARMY + 9, width: 18, height: 7, borderRadius: 3, background: "#1A1D24" }} />
            </div>
          )}
          {/* ============ 23. THE ARMCO: the fence does not decide to rise, a guy kicks it ============ */}
          {Array.from({ length: 6 }).map((_, i) => {
            const t0 = 196 + i * 2;
            if (lf < t0) return null;
            const p = over(lf, t0, 5, Easing.out(Easing.back(2.4)));
            return (
              <div key={"aa" + i} style={{ position: "absolute", left: 800 - i * 63 - 34, top: 700 + i * 15.3 - 20, width: 70, height: 22, zIndex: 22, transformOrigin: "50% 100%", transform: `scaleY(${p}) rotate(${(1 - p) * -22 + 14}deg)` }}>
                <div style={{ width: 70, height: 22, background: `linear-gradient(180deg, ${i === 2 ? "#9AA2A8" : "#B4BCC4"} 0%, #6E7680 42%, ${i === 2 ? "#7A828A" : "#8E969E"} 46%, #4A525C 100%)`, borderRadius: 2, boxShadow: "0 7px 13px rgba(0,0,0,0.6)", border: "1px solid rgba(230,240,250,0.2)" }} />
                <div style={{ position: "absolute", left: 32, top: 20, width: 6, height: 14, background: "#3A414B" }} />
              </div>
            );
          })}
          {Array.from({ length: 5 }).map((_, i) => {
            const t0 = 224 + i * 6;
            if (lf < t0) return null;
            const p = over(lf, t0, 5, Easing.out(Easing.back(2.4))), s = 1 - i * 0.13;
            return (
              <div key={"ab" + i} style={{ position: "absolute", left: 806 + i * 40 - 30 * s, top: 710 - i * 8.6 - 18 * s, width: 62 * s, height: 20 * s, zIndex: 22, transformOrigin: "50% 100%", transform: `scaleY(${p}) rotate(10deg)` }}>
                <div style={{ width: 62 * s, height: 20 * s, background: "linear-gradient(180deg,#B4BCC4 0%,#6E7680 42%,#8E969E 46%,#4A525C 100%)", borderRadius: 2, boxShadow: "0 5px 10px rgba(0,0,0,0.55)" }} />
              </div>
            );
          })}
          {lf >= 194 && lf < 254 && (() => {
            const A = lf < 214, t = A ? cl((lf - 194) / 16) : cl((lf - 222) / 30);
            return crew("kick", A ? 812 - t * 380 : 806 + t * 168, A ? 726 + t * 90 : 736 - t * 40, A ? 168 - t * 4 : 164 - t * 52, 23, 0.9, -3, A ? 6 : 2.4);
          })()}

          {/* ============ 28. TWO HI-VIS CLAUDES, torquing the foot plates ============ */}
          {lf >= 144 && [30, 976].map((fx, i) => (
            <div key={"tq" + i} style={{ position: "absolute", left: fx - 56, top: 700 - 0.92 * 112, width: 112, height: 112, zIndex: 16, opacity: over(lf, 144, 6) }}>
              <Mascot lf={lf + i * 9} size={112} hiVis={1} nodAmp={lf >= 150 && lf < 168 ? 6 : 2} nodSpeed={lf >= 150 && lf < 168 ? 4 : 12} gaze={i ? -3 : 3} />
              <div style={{ position: "absolute", left: i ? 4 : 86, top: 62, width: 24, height: 9, borderRadius: 2, background: grad("#D8483A", "#7A1E14"), transform: `rotate(${lf >= 150 && lf < 168 ? Math.sin(lf * 1.6) * 22 : 0}deg)` }} />
            </div>
          ))}

          {/* ============ 32. THE DUST LADDER, unbroken by handoff. Bank 6 carries no dust: it has no shaft yet. ============ */}
          {Array.from({ length: 34 }).map((_, i) => {
            const r = seed(i * 3.3 + 11), inSod = i < 12, b = BANKS[i % 5];
            const litv = inSod ? (sodLive > 0 && lf < 76 ? 1 : 0) : bankLit(b.on) > 0.1 ? 1 : 0;
            if (litv < 0.05) return null;
            const cx = inSod ? (i < 6 ? 306 : 490) : b.x;
            const head = inSod ? 200 : b.head;
            const top = head + ((((lf * (0.3 + r * 0.75) + r * 620) % 620) + 620) % 620) * 0.9;
            if (top > 780) return null;
            const spread = (top - head) * 0.42;
            return <div key={"du" + i} style={{ position: "absolute", left: cx + (r - 0.5) * spread * 2, top, width: 2 + r * 3, height: 2 + r * 3, borderRadius: "50%", background: inSod ? "rgba(255,214,160,0.8)" : "rgba(226,240,255,0.85)", opacity: 0.14 + r * 0.34, zIndex: 26 }} />;
          })}
          {/* grit jumping half an inch off the concrete on every rivet CHA-CHUNK */}
          {rivShake > 0.02 && Array.from({ length: 16 }).map((_, k) => {
            const r = seed(k * 5.1 + 3);
            return <div key={"gr" + k} style={{ position: "absolute", left: 120 + r * 800, top: 720 - rivShake * (10 + r * 22), width: 2 + r * 2, height: 2 + r * 2, borderRadius: "50%", background: "rgba(220,206,180,0.7)", opacity: rivShake * 0.8, zIndex: 26 }} />;
          })}
          {/* the wreck's smoke, climbing white through shafts 4, 5 and 6 from f140 */}
          {Array.from({ length: 11 }).map((_, i) => {
            const r = seed(i * 2.9 + 4), t = ((lf * (1.1 + r * 0.9) + r * 320) % 320) / 320;
            const sz = 22 + r * 44 + t * 78;
            const white = lf >= 140 && t > 0.32 ? cl((t - 0.32) * 2.2) * 0.55 : 0;
            return <div key={"smk" + i} style={{ position: "absolute", left: 600 + r * 46 + t * (66 + r * 40), top: 600 - t * 400, width: sz, height: sz, borderRadius: "50%", background: `rgba(${Math.floor(60 + white * 175)},${Math.floor(56 + white * 178)},${Math.floor(56 + white * 185)},0.26)`, filter: "blur(8px)", opacity: (1 - t) * 0.72, zIndex: 26 }} />;
          })}

          {/* ============ 33. THE FOREGROUND OCCLUDER ============ */}
          <div style={{ position: "absolute", left: -20, top: 744, width: 250, height: 60, borderRadius: 8, background: grad("#2A2A2E", "#0E0E12"), filter: "blur(3.6px)", transform: "rotate(-2deg)", zIndex: 32 }} />
          <svg width={1012} height={792} style={{ position: "absolute", left: 0, top: 0, zIndex: 32, pointerEvents: "none", filter: "blur(3px)" }}>
            <path d="M -10 762 Q 130 800 268 768" fill="none" stroke="rgba(14,14,18,0.95)" strokeWidth={9} />
            <circle cx={268} cy={768} r={9} fill="rgba(14,14,18,0.95)" />
          </svg>
          {lf >= 188 && Array.from({ length: 7 }).map((_, k) => {
            const r = seed(k * 6.1 + 9), t = (((lf - 188) * (0.9 + r) + r * 90) % 90) / 90;
            return <div key={"nf" + k} style={{ position: "absolute", left: 640 - t * 620, top: 480 + t * t * 330, width: 9 + r * 12, height: 6 + r * 9, background: FAKE, opacity: (1 - t) * 0.5, filter: "blur(2.6px)", transform: `rotate(${t * 300}deg)`, borderRadius: 2, zIndex: 33 }} />;
          })}
        </div>

        {/* ============ GLOBAL WASHES: the palette pivot, and a floor that never goes to a void ============ */}
        <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, pointerEvents: "none", background: `radial-gradient(ellipse at 42% 72%, rgba(255,150,50,${0.2 * sodLive}), transparent 66%)`, mixBlendMode: "screen", zIndex: 40 }} />
        <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, pointerEvents: "none", background: `linear-gradient(190deg, rgba(150,196,246,${0.1 * coldKey}), transparent 60%)`, mixBlendMode: "screen", zIndex: 40 }} />
        <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, pointerEvents: "none", background: `rgba(4,6,10,${Math.max(0, (1 - sodLive) * (1 - Math.min(1, coldKey * 2.6)) * 0.6)})`, zIndex: 41 }} />
        <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, pointerEvents: "none", boxShadow: "inset 0 0 180px rgba(8,6,12,0.6)", zIndex: 42 }} />
      </div>
    </Panel>
  );
};
