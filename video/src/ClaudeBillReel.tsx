...
/* ⛔ ONE BED ACROSS THREE CUTS IS NOT A VARIANT. The VO is the same recording in
   all three, so the bed is one of the few genuinely different audio layers a
   duplicate check can see. These are three different TRACKS — different key,
   tempo, progression and motif — generated by tools/gen_bill_bed.py, not one
   track at three volumes. */
const BED: Record<Variant, string> = {
  bill:  "116_bill_bed.wav",
  amber: "116_bill_bed_amber.wav",
  steel: "116_bill_bed_steel.wav",
};
...
/* ⛔⛔ A GAIN THAT FIXED ONE REEL IS NOT A CONSTANT (SOUND-DESIGN §13). Reel 110
   ...
   INAUDIBLE bed on a different source — was carried forward unmeasured. So this
   one is measured on THESE two files, today:
       VO  file -16.0 LUFS  x LEVELS.DIALOGUE (-6)  ->  -22.0 in the mix
       bed file -17.6 LUFS  x LEVELS.MUSIC  (-20)   ->  -37.6 in the mix
       gap 15.6 dB, against a house figure of ~12 dB under the VO
   +3.5 dB lands the gap at 12.1 dB. Nothing was inherited. */
export const BED_TRIM = { loud: db(3.5), quiet: db(-2.5) } as const;
