...
import {ClaudeSelection,ClaudeRewrite} from './ClaudeTrialHooks';
...
function Shot({i,variant='A'}:{i:number,variant?:string}){const f=useCurrentFrame();const Comp=i===0&&variant!=='A'?(variant==='B'?ClaudeSelection:ClaudeRewrite):scenes[i];return <><Panel><Comp t={f/30*(i===4?41/50:1)}/><ClaudeContext i={i} t={f/30}/></Panel><HookHeader big={headers[i][0]} hot={i===3&&f<14?'A DIFFERENT WAY TO WORK':headers[i][1]} f={30}/></>}
