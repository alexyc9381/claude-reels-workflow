...
import {PromptBreakthrough,IdeaCascade,PromptBuild} from './NarrationAlignedScenes';
...
function Shot({i,variant='A'}:{i:number,variant?:string}){const f=useCurrentFrame();const Comp=i===0&&variant!=='A'?(variant==='B'?PromptBreakthrough:IdeaCascade):i===1&&variant!=='A'?PromptBuild:scenes[i];return <><Panel><Comp t={f/30*(i===4?41/50:1)}/>{!(variant!=='A'&&i<2)&&<ClaudeContext i={i} t={f/30}/>}</Panel><HookHeader big={headers[i][0]} hot={i===3&&f<14?'A DIFFERENT WAY TO WORK':(i===1&&variant!=='A'?'APPS + WEBSITES':headers[i][1])} f={30}/></>}
export function WIN({variant='A'}:{variant?:string}){const activeCuts=variant==='A'?cuts:[0,69,...cuts.slice(2)];return <AbsoluteFill><style>{fontCSS}</style><Bg/>{scenes.map((_,i)=><Sequence key={i} from={activeCuts[i]} durationInFrames={activeCuts[i+1]-activeCuts[i]}><Shot i={i} variant={variant}/></Sequence>)}<ProgressBar/><KaraokeCaption words={words}/><Audio src={staticFile(variant==='A'?'master-overhaul.wav':`master-trial-${variant}.wav`)}/></AbsoluteFill>}
