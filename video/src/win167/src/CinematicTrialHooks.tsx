...
function Actor({x,y,s=240,t,sy=1,rot=0,arms=0,effort=0,shock=0,armor=0,c='#D97757',stride=0}:{x:number,y:number,s?:number,t:number,sy?:number,rot?:number,arms?:number,effort?:number,shock?:number,armor?:number,c?:string,stride?:number}){return <P x={x} y={y} w={s} h={s} style={{transform:`rotate(${rot}deg) scale(${1+(1-sy)*.38},${sy})`,transformOrigin:'50% 95%',filter:'drop-shadow(0 7px 0 #17344744)'}}><svg width={s} height={s} viewBox="0 0 200 200" shapeRendering="crispEdges" style={{overflow:'visible'}}>
 {[52,78,112,138].map((v,i)=><path key={i} d={`M${v} 140h12v${45-(i%2?1:-1)*stride*18}h-12Z`} fill={c}/>)}
 <g transform={`rotate(${-arms*110} 35 96)`}><path d="M8 86h30v26H8Z" fill={c}/>{armor>0&&<path d="M-1 79h37v40H-1Z" fill="#C59A4E" stroke="#785F37" strokeWidth="4" opacity={armor}/>}</g>
 <g transform={`rotate(${arms*110} 165 96)`}><path d="M164 86h29v26h-29Z" fill={c}/>{armor>0&&<path d="M163 79h39v40h-39Z" fill="#DAB468" stroke="#785F37" strokeWidth="4" opacity={armor}/>}</g>
 <path d="M34 44h132v102H34Z" fill={c}/><path d="M34 44h132v10H34Z" fill="#FFFFFF29"/>
 {armor>0&&<g opacity={armor}><path d="M32 116h35l33 22 34-22h34v30H32Z" fill="#B68438"/><path d="M54 45l46-22 47 22-13 15-34-17-33 17Z" fill="#E1BA68" stroke="#6E5739" strokeWidth="4"/><path d="M87 132l13 8 13-8-13 20Z" fill="#F8E7B4"/></g>}
 <path d={`M67 ${71+effort*8}h14v${27+shock*12-effort*10}H67M119 ${71+effort*8}h14v${27+shock*12-effort*10}h-14`} fill="#241E20"/>
 <path d={`M63 ${65+effort*2}l23 ${effort*8}M115 ${65+effort*10}l24 ${-effort*8}`} stroke="#3D2D2B" strokeWidth={4*effort}/>
 {shock>.4?<rect x="93" y="114" width="16" height="21" fill="#382326"/>:<path d={effort>.3?'M85 118h31v10H85Z':'M86 119l13 7 15-8'} fill={effort>.3?'#FFF1D4':'none'} stroke="#382326" strokeWidth="4"/>}
 </svg></P>}
function Paper({x,y,w=180,h=180,rot=0,good=false,fold=0,opacity=1}:{x:number,y:number,w?:number,h?:number,rot?:number,good?:boolean,fold?:number,opacity?:number}){return <P x={x} y={y} w={w} h={h} style={{transform:`rotate(${rot}deg) scaleX(${1-fold*.8})`,opacity,filter:'drop-shadow(6px 8px 0 #20313B35)'}}><svg width={w} height={h} viewBox="0 0 180 180" preserveAspectRatio="none"><path d="M8 5H135l36 37v131H8Z" fill={good?'#DFE7C9':'#F1DDC0'} stroke="#746D5B" strokeWidth="5"/><path d="M135 5v37h36" fill="#C4B599" stroke="#746D5B" strokeWidth="4"/><path d="M42 59h89v54H42M27 82h16v17H27M130 82h18v17h-18M54 113v22m21-22v22m25-22v22m20-22v22" fill={good?'#CE994E':'#BB826B'} stroke={good?'#CE994E':'#BB826B'} strokeWidth="8"/><path d="M62 75v16m47-16v16" stroke="#3C3730" strokeWidth="7"/>{good?<path d="M66 149l13 12 27-25" fill="none" stroke="#337462" strokeWidth="10"/>:<path d="M73 140l23 23m0-23-23 23" stroke="#AC5547" strokeWidth="9"/>}</svg></P>}
...
export function PressureTrial({t}:{t:number}){
 ...
 <Actor x={hx} y={base-hs*.95} s={hs} t={t} sy={sy} arms={Math.min(1,press*.8+brace*.2)*(1-burst*.45)} effort={1-desc} shock={.6*(1-brace)} armor={brace} rot={-5*brace+8*lift-11*run} stride={run}/>
 ...
 <path d={`M${hx+hs*.5} ${base-hs*.3}Q516 389 ${418} 538M${hx+hs*.5} ${base-hs*.3}Q843 409 ${895} 540`} stroke="#CDAE62" strokeWidth="9" fill="none" opacity={desc}/></Svg>
 {[0,1].map(i=><P key={i} x={i?824:333} y={505-70*arc(t,2.35+i*.07,.44)-72*run} w={186} h={186} style={{transform:`scale(${desc}) rotate(${(i?9:-9)*run}deg)`,transformOrigin:i?'0 50%':'100% 50%'}}><Actor x={0} y={0} s={186} t={t} armor={1} arms={.5+run*.4} c={i?'#B39CC7':'#79A8B5'} stride={run}/></P>)}
 </P>
}
...
/** A draft avalanche becomes material: the retained good drafts fold into armor, which breaks the pile. */
export function DraftAvalanche({t}:{t:number}){
 ...
 <Actor x={hx} y={base-hs*.95} s={hs} t={t} sy={sy} arms={.9*pile-.8*grab+.86*wind-.3*tear} effort={1-rise*.7} shock={.85*(1-grab)} armor={fold} rot={-7*pile-13*wind+21*hit-11*drive} stride={drive}/>
 ...
 {/* Two chosen green pages are visibly pulled from the pile and folded onto the hands. */}
 ...
 <P x={580+125*drive} y={478-105*arc(t,2.26,.58)-77*drive} w={240} h={240} style={{transform:`scale(${rise}) rotate(${16*drive}deg)`,transformOrigin:'0 80%'}}><Actor x={0} y={0} s={240} t={t} armor={1} arms={.6+drive*.3} c="#84ADB2" stride={drive}/></P>
 <Svg><path d={`M${hx+hs*.88} ${base-hs*.5}Q558 369 ${627+125*drive} ${580-77*drive}`} fill="none" stroke="#D9B86E" strokeWidth="10" opacity={rise*(1-drive*.4)}/><Puff t={t} at={2.64} x={424} y={724}/></Svg>
 </P>
}
