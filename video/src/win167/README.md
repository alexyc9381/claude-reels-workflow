...
Editable source and editorial evidence for the September 22 continuous-through-cut revision. Composition `WIN`: 1080×1920, 30 fps, 817 frames (27.233333 seconds); `WIN-Cover` uses the revised house cover system.
...
Use `npm run studio` for inspection. Optional audio rebuild requires Python with NumPy/SciPy and FFmpeg on PATH: run `python3 tools/gen-surprise-sfx.py`, then `python3 tools/mix-surprise-v4.py`. For the latest continuation cues, run `python3 tools/mix-continuous-v5.py`, which uses the supplied frozen `public/master-dramatic-base.wav` and replaces `master-overhaul.wav`. These scripts restore their inputs from `public/`; the supplied master is ready to render. `qa/media-manifest.json` describes the delivered media before any regeneration. The mix script keeps a one-time backup under `qa/audio-before-surprise/`; its historical gate names do not establish new user approval.
