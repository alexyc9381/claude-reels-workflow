...
import {C,clamp,lerp,pop,visible,Glass,Logo,Actor,Key,Film,ProgressBorder,BrandedBackground} from './YouTubeV5Primitives';
import {SkillScene,DownloadScene,DirectScene,CompareScene} from './YouTubeV5Scenes';
import {HookScene,RoadmapScene} from './YouTubeV5Intro';
import {ProductionWorkshop as ModelStudio} from './ProductionWorkshop';
...
const Teaser:React.FC<{duration:number}>=({duration})=>{
 const t=useTime();return <AbsoluteFill style={{fontFamily:bodyFont,opacity:visible(t,duration)}}>
  ...
  <Glass x={140} y={125} w={750} h={134} t={t} frost={.12} style={{transform:`translateY(${(1-pop(t,.05))*-40}px)`}}><div style={{position:'absolute',left:30,top:32,fontFamily:displayFont,fontSize:51,fontWeight:600,letterSpacing:-1.5,color:'#FFF7E7',textShadow:'0 3px 18px #573821B0'}}>Later in this video…</div></Glass>
 </AbsoluteFill>;
};
...
export const soundEvents=(m:M)=>{
 ...
 [.18,.6,1.08,2.46,4.65].forEach((x,i)=>add(x,['servo','whip','latch','shutter','latch'][i],.25));
 const at=(id:string)=>find(id).from/fps;
 add(at('s003')-.18,'whip',.23);add(at('s003')+.6,'latch',.23);add(at('s003')+1.15,'servo',.22);add(at('s003')+2.65,'paper',.25);add(at('r-higgsfield')+.35,'assemble',.24);
 add(at('s004')+.12,'typing',.12,.9);
 for(const [off,name] of [[1.25,'zip'],[1.95,'paper'],[2.05,'click'],[2.38,'latch'],[3.2,'servo'],[3.85,'shutter'],[6.7,'paper']] as const)add(at('s004')+off,name,.23);
 for(const [off,name] of [[.1,'paper'],[.6,'zip'],[2.8,'latch']] as const)add(at('s006')+off,name,.22);
 add(at('r-teaser'),'whip',.26);
 add(at('s018')+.4,'paper',.23);add(at('s018')+1.2,'zip',.22);add(at('s018')+2.6,'latch',.22);
 for(const s of fullScenes(m).filter(s=>s.kind==='download'||s.kind==='outro')){const ratio=s.duration/fps/(s.kind==='outro'?7.9:4.2);for(const [u,name] of [[.78,'click'],[1.05,'paper'],[s.kind==='outro'?3.9:1.8,'zip'],[s.kind==='outro'?5.25:3.05,'latch']] as const)add(s.from/fps+u*ratio,name,.23);}
 for(const [u,name] of [[.78,'click'],[1.05,'paper'],[1.8,'zip'],[3.05,'latch']] as const)add(at('s019')+u*find('s019').duration/fps/4.2,name,.22);
