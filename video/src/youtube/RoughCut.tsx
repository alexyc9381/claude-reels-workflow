...
const Screen:React.FC<{m:RoughCutManifest;s:Row;sourceStart?:number;t:number}>=({m,s,sourceStart=s.start,t})=>{
 ...
 return <div style={{position:'absolute',left:screenViewport.x,top:screenViewport.y,width:screenViewport.w,height:screenViewport.h,overflow:'hidden',borderRadius:25,boxShadow:'0 20px 42px #5C3D2438',border:'2px solid #FFFFFFCC'}}>
  <div data-cropped-obs style={{position:'absolute',width:1760,height:screenViewport.h,top:0,overflow:'hidden',transform:`translate(${tr.tx}px,${tr.ty}px) scale(${tr.zoom})`,transformOrigin:'top left',filter:s.teaser?'blur(8px)':undefined}}>
   {s.teaser?<Loop durationInFrames={120}><OffthreadVideo data-teaser-playing-result muted src={staticFile('v4/claude-result.mp4')} style={{width:1760,height:screenViewport.h,objectFit:'cover'}}/></Loop>:<OffthreadVideo src={staticFile(m.obs.source)} startFrom={Math.round(sourceStart*m.fps)} muted style={{position:'absolute',width:1760,height:990,top:-sourceCrop.top*1760/1920}}/>}
