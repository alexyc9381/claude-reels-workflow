...
export const ComparisonV10:React.FC<{revealAt?:number;opening?:boolean}>=({revealAt=999,opening=false})=>{
 ...
 return <AbsoluteFill data-video-first-comparison style={{background:'#F2EBE1',fontFamily:bodyFont}}>
  {[0,1].map(i=><div key={i} style={{position:'absolute',left:opening?32+i*940:12+i*958,top:opening?28:24,width:opening?916:938,height:opening?1016:742,overflow:'hidden',borderRadius:18,boxShadow:'0 10px 30px #40281B25'}}>
   <Loop durationInFrames={120}><OffthreadVideo data-comparison-side={i?'B':'A'} src={staticFile(i?'v4/claude-result.mp4':'v9/higgsfield-comparison.mp4')} muted style={{width:'100%',height:'100%',objectFit:'cover',objectPosition:`${i?50:aPosition}% 50%`,...(opening?{transform:`scale(${1+.065*(e(t,i?2.8:1.6,.32)-e(t,i?3.65:2.45,.38))})`}:{})}}/></Loop>
  ...
  {opening&&t>=1.6&&t<4.6&&<div data-opening-countdown style={{position:'absolute',left:882,top:415,width:156,height:156,borderRadius:'50%',background:'#FFF9EEF2',boxShadow:'0 12px 35px #1A181350',display:'grid',placeItems:'center',fontSize:91,fontWeight:850,color:C.ink,transform:`scale(${1+.08*Math.exp(-9*((t-1.6)%1))})`}}><svg width="156" height="156" style={{position:'absolute',inset:0,transform:'rotate(-90deg)'}}><circle cx="78" cy="78" r="73" stroke="#DCC6A2" strokeWidth="5" fill="none"/><circle cx="78" cy="78" r="73" stroke={C.orange} strokeWidth="6" fill="none" pathLength="1" strokeDasharray="1" strokeDashoffset={1-(t-1.6)%1}/></svg>{3-Math.floor(t-1.6)}</div>}
