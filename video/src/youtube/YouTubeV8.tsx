...
import {Support,LaterCue,TextLens,type SupportKind} from './SupportingScenesV8';
...
import {HookV8,ProductionV8,RoadmapV8,GuideV8,WrapperV8,DirectV8,DownloadV8,SkillV8,CompareV8,OutroV8} from './ScenesV8';
...
export const YouTubePolish:React.FC<{manifest:M}>=({manifest:m})=>{
 ...
 return <>
  {scenes.map(s=><Sequence key={s.id} from={s.from} durationInFrames={s.duration}>{s.kind==='hook'?<HookV8 duration={s.duration/fps} guessAt={s.a!} whyAt={s.b!} higgsAt={s.c!}/>:s.kind==='studio'?<ProductionV8 duration={s.duration/fps}/>:s.kind==='roadmap'?<RoadmapV8 duration={s.duration/fps}/>:s.kind==='direct'?<DirectV8 duration={s.duration/fps} featuresAt={s.a!}/>:s.kind==='wrapper'?<WrapperV8 duration={s.duration/fps}/>:s.kind==='download'?<DownloadV8 duration={s.duration/fps}/>:s.kind==='guide'?<GuideV8 duration={s.duration/fps}/>:s.kind==='outro'?<OutroV8 duration={s.duration/fps}/>:s.kind==='skill'?<SkillV8 duration={s.duration/fps} importAt={s.a!}/>:<CompareV8 duration={s.duration/fps} revealAt={s.a!}/>}</Sequence>)}
...
export const soundEvents=(m:M)=>{
 ...
 for(const s of fullScenes(m)){
  ...
  if(s.kind==='hook'){
   const q=s.b!,span=d-q;beat(1.5/7.6,span,'servo',.13,b+q);beat(3/7.6,span,'paper',.13,b+q);beat(4.1/7.6,span,'land',.12,b+q);
 ...
 for(const c of v8Cues)add(at(c.id)+c.offset+.12,c.kind==='typing'?'typing':c.kind==='lens'?'servo':c.kind==='keys'?'click':'paper',c.kind==='typing'?.09:.13,c.kind==='typing'?1.9:.8);
