...
export const Title:React.FC<{children:React.ReactNode;x?:number;y?:number;size?:number;color?:string;style?:React.CSSProperties}>=({children,x=140,y=112,size=76,color=C.ink,style})=><div style={{position:'absolute',left:x,top:y,fontFamily:displayFont,fontWeight:600,fontSize:size,letterSpacing:-1.8,lineHeight:1.04,color,...style}}>{children}</div>;
export const Actor:React.FC<{t:number;x:number;y:number;size?:number;role?:'courier'|'operator'|'archivist';hop?:number;lift?:number;travel?:number;happy?:boolean}>=({t,x,y,size=230,role='courier',hop=.35,lift=80,travel=0,happy=false})=><SpriteActor t={t} x={x} y={y} size={size} colorful outfit={role} pose={naturalHop(t,hop,.8,lift)} face={happy?facePresets.pleased:hopFace(t,hop,.8)} gait={{phase:t*15,amount:travel}} gesture={.4+.4*Math.sin(t*3.2)}/>;
...
export const SkillFile:React.FC<{t:number;size?:number;open?:number;label?:string}>=({t,size=250,open=0,label='.skill'})=><div style={{position:'relative',width:size,height:size*1.2,perspective:900}}>
 {[2,1,0].map(i=><div key={i} style={{position:'absolute',inset:'0 5%',borderRadius:19,background:i?'#F4E5CA':'linear-gradient(140deg,#FFFFFF,#FFF5E3)',border:'2px solid #FFFFFF',boxShadow:'0 13px 24px #87533324',transform:`translate(${i*12+open*i*38}px,${i*-7-open*i*24}px) rotate(${i*3+open*i*9}deg)`}}>
  {i===0&&<div style={{position:'absolute',left:'13%',top:'16%',color:C.orange,fontFamily:displayFont,fontSize:size*.23}}>{label}</div>}
...
export const Stage:React.FC<{t:number;children:React.ReactNode;title?:string}>=({t,children,title})=><AbsoluteFill style={{fontFamily:bodyFont,color:C.ink,overflow:'hidden'}}><BrandedBackground t={t}/>
