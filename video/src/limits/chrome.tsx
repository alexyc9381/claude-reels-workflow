...
import words from "../data/words_limits.json";
...
const REEL = 32.400;                                  // the finished VO length
const TIPS = [4.360, 13.660, 21.860];                 // the 3 numbered milestones = the 3 tricks
const STAR = 28.460;                                  // splits the 32% run from tip 4 to the end
const PELLETS = [2.0, 7.4, 11.4, 17.4, 24.6, 29.6];  // micro-rewards, spread across every gap
...
export const ProgressBar: React.FC<{ ctaAt: number }> = ({ ctaAt }) => {
  ...
  const TOTAL = VIRT;
  const marks = [4.360, 13.660, 21.860];                         // the 3 tricks
  const STARS = [2.2, 8.2, 12.4, 18.6, 25.2, 29.8];
  const PELLETS = [1.0, 2.8, 5.4, 7.6, 9.8, 11.8, 15.2, 17.6, 19.8, 23.0, 26.2, 28.8, 31.4];
...
/* ⛔ `code` renders the title in the terminal face, lowercase — a command is a
   string you type, and setting it in a serif display face makes it read as a
   THEME rather than as the literal thing being said. */
export const SceneHeader: React.FC<{ lf: number; kicker: string; title: string; dur: number; code?: boolean }> = ({ lf, kicker, title, dur, code }) => {
  ...
  return (
    <div style={{ position: "absolute", left: 0, right: 0, top: 36, zIndex: 64, display: "flex", justifyContent: "center", pointerEvents: "none", opacity: o, transform: `translateY(${(1 - inP) * -14}px) scale(${0.94 + inP * 0.06})` }}>
      <div style={{ background: "#FCFAF5", borderRadius: 16, padding: "12px 26px 14px", textAlign: "center", boxShadow: "0 16px 38px rgba(12,8,12,0.55), 0 3px 0 rgba(255,255,255,0.9) inset", border: "3px solid rgba(26,24,19,0.10)" }}>
        ...
        <div style={code
          ? { fontFamily: mono, fontWeight: 700, fontSize: 42, lineHeight: 1.06, letterSpacing: "-0.01em", color: CLAY, whiteSpace: "nowrap" }
          : { fontFamily: fraunces.fontFamily, fontWeight: 900, fontSize: 46, lineHeight: 1.02, letterSpacing: "-0.02em", color: INK, textTransform: "uppercase", whiteSpace: "nowrap" }}>{title}</div>
      </div>
    </div>
  );
};
...
export const HeroHeader: React.FC<{ outAt: number }> = ({ outAt }) => {
  ...
  const S: React.CSSProperties = { fontFamily: fraunces.fontFamily, fontWeight: 900, fontSize: 52, lineHeight: 1.04, letterSpacing: "-0.02em", color: INK, textTransform: "uppercase", whiteSpace: "nowrap" };
  return (
    <div style={{ position: "absolute", left: 0, right: 0, top: 430, zIndex: 80, opacity: out, pointerEvents: "none", display: "flex", justifyContent: "center", transform: `scale(${pop})` }}>
      <div style={{ background: "#FCFAF5", borderRadius: 16, padding: "13px 22px 15px", textAlign: "center", boxShadow: "0 14px 34px rgba(16,10,16,0.48), 0 2px 0 rgba(255,255,255,0.9) inset", border: "3px solid rgba(26,24,19,0.10)" }}>
        <div style={S}>CLAUDE USAGE LIMIT?</div>
        <div style={S}><span style={{ color: CLAY }}>3 FIXES.</span></div>
      </div>
    </div>
  );
};
