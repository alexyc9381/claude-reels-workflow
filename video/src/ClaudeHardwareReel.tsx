...
export const SFX: Cue[] = [
  ...
  /* ---- S2 · THE DOCK. ⛔⛔ ROUND 50 MOVED EVERY PICTURE EVENT IN THIS SCENE
     and `sfx_audit` reads the SOURCE, not the picture, so it cannot hear a flam.
     Landing f18, the wind f22-50, the last bolt f45, the door off the seal f50. */
  ...layer(S(L.S2 + 18),
    { src: "rebuild_thud.wav", v: LEVELS.SFX_HERO,        dur: 1.2, rate: 0.86 },
    { src: "boom.wav",         v: LEVELS.SFX_MID,         dur: 1.3, rate: 0.76 }),
  { at: S(L.S2 + 22),  src: "gear_shift.wav",    v: LEVELS.SFX_MID,     dur: 1.4, rate: 0.70 },
  { at: S(L.S2 + 45),  src: "lamp_clunk.wav",    v: LEVELS.SFX_MID,     dur: 0.6, rate: 0.88 },
  { at: S(L.S2 + 52),  src: "arrive_chime.wav",  v: LEVELS.SFX_MID,     dur: 1.0, rate: 1.02 },

  /* ---- S3 · THE PARAMETER WALL. He sets ONE, then it divides three times, and
     the divisions climb — an ascending run is what makes a repeat read as
     PROGRESS rather than as repetition. ---------------------------------- */
  { at: S(L.S3 + 9),   src: "ui_tap.wav",        v: LEVELS.SFX_TEXTURE, dur: 0.4, rate: 1.10 },
  { at: S(L.S3 + 11),  src: "mech_clank.wav",    v: LEVELS.SFX_MID,     dur: 0.6, rate: 0.86 },
  { at: S(L.S3 + 25),  src: "mech_clank.wav",    v: LEVELS.SFX_MID * db(1), dur: 0.6, rate: 1.00 },
  ...layer(S(L.S3 + 41),
  ...
  ...[4, 16, 28, 40, 52, 64, 76].map((a, i) => ({
    at: S(L.S4 + a + 12), src: "mech_clank.wav",
    v: LEVELS.SFX_MID * db(i * 0.5), dur: 0.6, rate: 0.86 + i * 0.045,
  })),
  { at: S(L.S4 + 86),  src: "arrive_chime.wav",  v: LEVELS.SFX_MID,     dur: 0.9, rate: 1.10 },

  /* ---- S5 · THE TRADE. The load rumbling out, and ONE card arriving. ---- */
  { at: S(L.S5 + 2),   src: "road_bed_dry.wav",  v: LEVELS.SFX_BED * db(4), dur: 2.0, rate: 1.06 },
  { at: S(L.S5 + 30),  src: "lamp_clunk.wav",    v: LEVELS.SFX_TEXTURE, dur: 0.5, rate: 0.90 },
  ...layer(S(L.S5 + 58),
  ...
  { at: S(L.S6 + 94),  src: "sign_clack.wav",    v: LEVELS.SFX_MID,     dur: 0.6, rate: 0.94 },

  /* ---- S7 · THE SUPPLY STRAINING. A density PEAK. ---------------------- */
  { at: S(L.S7 + 14),  src: "neon_on.wav",      v: LEVELS.SFX_MID,     dur: 1.0, rate: 0.92 },
  ...layer(S(L.S7 + 30),
    { src: "motor_sag.wav",    v: LEVELS.SFX_MID * db(2), dur: 1.2, rate: 0.84 },
    { src: "slate_whump.wav",  v: LEVELS.SFX_TEXTURE,     dur: 0.6, rate: 0.78 }),
  { at: S(L.S7 + 44),  src: "neon_on.wav",      v: LEVELS.SFX_MID,     dur: 1.1, rate: 0.80 },
  ...layer(S(L.S7 + 62),
    { src: "motor_sag.wav",    v: LEVELS.SFX_MID * db(3), dur: 1.4, rate: 0.70 },
    { src: "slate_whump.wav",  v: LEVELS.SFX_TEXTURE,     dur: 0.7, rate: 0.70 }),
  ...layer(S(L.S7 + 78),
    { src: "impact_deep.wav",  v: LEVELS.SFX_HERO,        dur: 1.1, rate: 0.80 },
    { src: "road_bed_dry.wav", v: LEVELS.SFX_BED * db(7), dur: 1.8, rate: 0.70 }),

  /* ---- S8 · THE MONTH RUNNING. The punch at f88 gets its own transient. - */
  { at: S(L.S8 + 4),   src: "road_bed_dry.wav",  v: LEVELS.SFX_BED,     dur: 5.2, rate: 0.96 },
  ...
  /* ⛔ ROUND 58 — the printing press went, so its spring wind-up and its slam
     went with it. `sfx_audit` reads the SOURCE, not the picture: a cue for a
     mechanism that is no longer on screen is inaudible to every gate here. */
  { at: S(L.S10 + 8),   src: "neon_on.wav",      v: LEVELS.SFX_MID,     dur: 1.3, rate: 0.86 },
  ...layer(S(L.S10 + 162),
    { src: "green_tone.wav",   v: LEVELS.SFX_MID,         dur: 0.9, rate: 1.18 },
    { src: "pop.wav",          v: LEVELS.SFX_TEXTURE,     dur: 0.4, rate: 1.06 }),
  ...
  /* the tick is a STAMP and it gets the same transient the X got at 29s */
  ...layer(S(L.S13 + 90),
    { src: "gold_stamp.wav",   v: LEVELS.SFX_HERO,        dur: 0.9, rate: 1.06 },
    { src: "green_tone.wav",   v: LEVELS.SFX_TEXTURE,     dur: 0.9, rate: 1.02 }),

  /* ---- S14 · THREE DOORS. The first bolt throws. ----------------------- */
  /* ⭐ ROUND 58 — asked for directly: *"doe rei mi sound effects."* One cue,
     three RATES: do = 1.000, re = 1.1225 (a whole tone up), mi = 1.2599 (two).
     Equal temperament is 2^(n/12), so a rate IS a transposition — the three
     doors play an ascending major third and the count reads as a phrase. */
  ...[[4, 1.000], [24, 1.1225], [46, 1.2599]].map(([a, r], i) => ({
    at: S(L.S14 + a), src: "arrive_chime.wav",
    v: LEVELS.SFX_MID * db(i * 0.4), dur: 0.8, rate: r,
  })),
  ...
  /* the shackle going home — a lock is a DEAD blow, not a slap */
  ...layer(S(L.S16 + 56),
    { src: "impact_deep.wav",  v: LEVELS.SFX_HERO,        dur: 0.9, rate: 0.90 },
    { src: "mech_clank.wav",   v: LEVELS.SFX_TEXTURE,     dur: 0.6, rate: 0.70 }),

  /* ---- S17 · THE NIGHT FLOOR. ------------------------------------------ */
  { at: S(L.S17 + 2),  src: "office_chatter.wav", v: LEVELS.SFX_BED,    dur: 2.0, rate: 0.94 },
  { at: S(L.S17 + 9),  src: "green_tone.wav",    v: LEVELS.SFX_MID,     dur: 1.0, rate: 0.92 },
  ...
  /* the rain: a BED, not a cue — it arrives with the weather and does not hit */
  { at: S(L.S18 + 58), src: "road_bed_dry.wav",  v: LEVELS.SFX_BED * db(3), dur: 2.7, rate: 1.16 },
  ...[30, 52, 74].map((a, i) => ({
...
/* ---- THE BED -------------------------------------------------------------
   ...
   ⛔⛤⛔ AND USING THE RIGHT SONG IS STILL NOT ENOUGH — ROUND 54. Alex, twice on
   this reel: *"the BG music is completely wrong, it's not the one we typically
   use."* Both times the reel WAS on `ados`. **The house bed is a PASSAGE, not a
   track.** `ados_bed_loud.wav` is one 50s window of a 228.5s song, and it starts
   at **165.9s**. This reel had re-derived its own window by scoring mean level /
   onset / internal range and landed on **84.0s** — a different section of the
   same song, which is why it kept sounding unfamiliar.

   ⭐ THE FIX IS TO STOP RE-DERIVING A WINDOW AND USE THE HOUSE FILE. All three
   cuts are now built FROM `ados_bed_loud.wav` / `ebm_bed_hot.wav` themselves,
   loop-extended past 61.07s with a 0.6s crossfade at the wrap:

     house  ados_bed_loud            83.7% under 250 Hz   onset 0.005s
     amber  ebm_bed_hot              59.9% under 250 Hz   onset 0.010s
     steel  ados_bed_loud @ +24s     81.8% under 250 Hz   onset 0.000s

   ⭐⭐ AND THE HOUSE BED IS BASS-FORWARD — that is WHY it can sit audibly under a
   VO. Measured share of energy under 250 Hz: ados_bed_loud 80.3%, ebm_bed_hot
   56.9%, and every bed the show has drifted onto 15-29%. A bed that keeps its
   midrange fights the voice however far you turn it down.
   ⛔ NO LOW PASS IS APPLIED HERE any more: the house files are already dark by
   construction, and filtering them further is the same drift in the other
   direction.
   ...
   ⛔⛔⛔ THE HIGH SHELF IS WHERE "THE PUFF OF AIR" LIVES — reel 115 spent three
   rounds hunting a named cue that did not exist; the air was the bed's own
   cymbal wash above 5k. `ebm_bed_hot` still gets -9 dB @ 3.4k; `ados` does not
   need it.
   ⛔ NO `afade in`: a fade kills the first downbeat and `MUSIC_ONSET_0` wants
   the bed audible inside 150ms.
   ⛔ ALL THREE ARE LEVELLED TO -20.4 LUFS, which is what the previous files
   measured — so `BED_GAIN` below is unchanged and one variable moved, not two. */
...
/* =========================================================================
   ...
   ====================================================================== */
/* ⛔⛔⛔ ROUND 60 — THE HEADER WAS GIVING THE ANSWER AWAY FOR EIGHTEEN SECONDS.
   The first band read `RUNNING AI LOCALLY / COSTS $112,000` and it ran from 1.8s
   to 19.8s — the entire stretch in which the VO is BUILDING that number: 2.8
   trillion parameters, then seven cards, then $16,000 each, then "that's over
   $110,000". Every one of those beats is a reveal, and the header had already
   printed the total above all four of them. A viewer who reads the band at 2s
   has no reason to watch the arithmetic.
   ⭐ A BAND STATES THE CLAIM OF THE SECTION IT IS OVER, AND NEVER THE NEXT ONE.
   The number now lands on S6 — the frame the VO says it — and the header
   CHANGING is the payoff beat rather than a caption that was always true.
   ⛔ EVERY FIGURE COMES OUT OF `R`, so a band cannot drift off the honesty
   ledger the way a hand-typed one can. */
const BANDS = [
  { from: L.S0,  big: "WHAT IT COSTS",       hot: "TO RUN AI AT HOME" },
  { from: L.S2,  big: "THE BIGGEST MODEL",   hot: "YOU CAN ACTUALLY RUN" },
  { from: L.S4,  big: "IT NEEDS",            hot: `${R.cards.n} GRAPHICS CARDS` },
  { from: L.S6,  big: "IN CARDS ALONE",      hot: R.cards.total },
  { from: L.S7,  big: "PLUS THE POWER BILL", hot: R.power.kw.toUpperCase() },
  { from: L.S8,  big: "EVERY MONTH",         hot: `${R.bill.month} JUST FOR POWER` },
  { from: L.S9,  big: "AND IT STILL",        hot: "DOES NOT RUN" },
  { from: L.S13, big: "THE SAME WORK",       hot: `IS ${R.api.hour} AN HOUR` },
...
/** ⛔ the band is IDENTICAL across the three cuts and it owns the top two rows of
    ...
    only cells the hook's own per-cut PILE cannot reach. */
const BAND_DY: Record<Variant, number> = { house: 0, amber: 26, steel: -22 };

const SectionBand: React.FC<{ f: number; v: Variant }> = ({ f, v }) => {
  /* ⛔⛔ ROUND 20 TOOK THE BAND OFF THE HOOK ("no text on the hook") and round 60
     put it back, asked for as directly: *"where is the header in the hook
     scene."* The reel opened with no header at all and then popped one in at
     1.77s, which is what "wtf is going on with the headers" was pointing at.
     ...
     four that did not had none. `HOOK_PLATE` is satisfied again.
     ⛔ The band RAISES frame-0 luma (cream on a lit room), so HOOK_LUMA moves in
     the safe direction — but it is re-measured on the encoded file regardless. */
  let b = BANDS[0];
  for (const x of BANDS) if (f >= x.from) b = x;
  return (
    <div style={{ position: "absolute", inset: 0, transform: `translateY(${BAND_DY[v]}px)` }}>
      {/* ⛔ +12 ON THE FIRST BAND TOO. A CLAIM PLATE HAS TO BE THERE AT FRAME 0 —
             feeding it f=0 starts `SectionHeader`'s own entry animation, so the
             one frame the plate exists for is the one frame it is not drawn. */}
      <HookHeader big={b.big} hot={b.hot} f={f - b.from + 12} />
    </div>
  );
};
