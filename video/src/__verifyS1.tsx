...
const S1: React.FC<{ lf: number }> = ({ lf }) => {
  ...
  const pulse = (at: number, len: number) => (lf >= at ? Math.max(0, 1 - (lf - at) / len) : 0);
  const kf = (p: number[], v: number[]) => interpolate(lf, p, v, cl);
  const popIn = (at: number) => interpolate(lf, [at, at + 1, at + 5], [0.25, 1.3, 1], cl);

  // ===================== LAYER COUNTER: seven shots, 03 -> 09, one flip per shot =====================
  // DONE is a die on a prop, never copy. The serial drops to micro-hatching. This counter is the scene's ONE number.
  const SHOTS = [
    { n: 3, at: 0 },    // the wreck's rear fender  (the hero's only shot: the gun leaves him before he fires again)
    { n: 4, at: 70 },   // the bare hub             (a wheel that is not there)
    { n: 5, at: 78 },   // the burning hood         (the money shot, and the S4 carry)
    { n: 6, at: 86 },   // the taped door           (on top of the sticker already there)
    { n: 7, at: 104 },  // car1's rear glass        (re-certifying what he is about to destroy)
    { n: 8, at: 140 },  // the hero's chest         (he certifies the man he just shoved)
    { n: 9, at: 150 },  // the panel glass          (in your face)
  ];
  const TGT: { [k: number]: { x: number; y: number } } = {
    3: { x: 320, y: 552 }, 4: { x: 68, y: 584 }, 5: { x: 258, y: 396 }, 6: { x: 306, y: 478 },
    7: { x: 668, y: 434 }, 8: { x: 548, y: 662 }, 9: { x: 470, y: 350 },
  };
  const fired = SHOTS.filter((s) => lf >= s.at).length;
  const count = 2 + fired;                                          // 03 at f0 ... 09 at f150
  const lastAt = fired ? SHOTS[fired - 1].at : 0;
  const flip = interpolate(lf, [lastAt, lastAt + 9], [1, 0], cl);   // still mid-flip on the last frame
  const thwack = SHOTS.reduce((a, s) => a + pulse(s.at, 7), 0);

  // ===================== CAMERA: 28mm, knee height on the apron, never nailed down =====================
  const hold = 1 - (ramp(lf, 20, 24) - ramp(lf, 36, 40));           // the drift HOLDS f24-f36 so the peel owns the frame
  const push = lf <= 137 ? kf([0, 137], [1, 1.04]) : kf([137, 154], [1.04, 1.09]);
  const shakeAmt = thwack * 3 + pulse(133, 9) * 5 + pulse(106, 10) * 2.4 + pulse(116, 9) * 2;
  const camX = Math.sin(lf * 0.031) * 6 * hold + Math.sin(lf * 4.1) * shakeAmt - pulse(133, 9) * 13;
  const camY = Math.cos(lf * 0.024) * 4 * hold + Math.cos(lf * 4.7) * shakeAmt * 0.6;
  const camRot = pulse(133, 11) * 2;

  // ===================== LAYER FIRE: it DOES the lighting, and it never decays =====================
  const fireT = lf <= 24 ? lf : lf <= 36 ? 24 : lf - 12;            // growth held flat f24-f36
  const grow = Math.min(1.9, Math.pow(1.4, fireT / 58));
  const flick = 0.78 + Math.sin(lf * 0.9) * 0.11 + Math.sin(lf * 2.27) * 0.07 + Math.sin(lf * 0.41) * 0.1;
  const belch = pulse(116, 15);
  const FIRE_X = 150, FIRE_BASE = 430;
  const fireH = (150 + belch * 60) * grow * (0.94 + flick * 0.08);
  const tyreFire = ramp(lf, 138, 154);                              // it jumps to a rear tyre: the last half second escalates
  const key = flick * (0.86 + grow * 0.2);

  // ===================== LAYER ROW: re-lit by the fire's climb, so you can see them fall =====================
  const rowLift = ramp(lf, 100, 154);
  const rowBlur = lerp(1.5, 0.5, rowLift);
  const rowExp = lerp(0.82, 1, rowLift);

  // ===================== THE DOMINO: every fold is caused by the collision one car up the line =====================
  const c1Roll = ramp(lf, 106, 114), c1Buck = ramp(lf, 108, 126);
  const c2Fold = ramp(lf, 118, 136), c3Fold = ramp(lf, 130, 148), c4Fold = ramp(lf, 144, 178);   // car4 is STILL folding at the cut
  const hazard = lf >= 130 && Math.floor((lf - 130) / 7) % 2 === 0 ? 1 : 0;

  // ===================== THE HERO: never parked, never smug. he loses his SAY, not his skill. =====================
  const heroShock = Math.max(pulse(116, 11) * 0.46, lf >= 133 ? 0.62 : 0);
  const HP = [0, 72, 80, 106, 114, 116, 122, 126, 133, 137, 154];
  const HX = kf(HP, [392, 392, 478, 478, 392, 400, 440, 470, 508, 540, 300]);
  const HY = kf(HP, [466, 466, 458, 458, 466, 470, 478, 485, 486, 636, 682]);
  const heroNod = lf < 72 ? 1.4 : lf < 137 ? 3.4 : 0;
  const hHopP = Math.max(0, Math.sin(lf / (9 * 0.6)));
  const hHop = hHopP * heroNod * 2.2 * (1 - heroShock);
  const hJump = heroShock > 0.05 ? Math.max(0, 1 - Math.abs(heroShock - 0.35) * 4) * 42 : 0;
  const hRide = -hHop - hJump;
  const fall = ramp(lf, 133, 141);
  const heroRot = fall * -96;                                       // knocked onto his back, boots up, sliding
  const heroGaze = kf([0, 30, 56, 70, 74, 100, 118, 122, 133, 154], [-2, -2, 0, 1, 3, 3, -2, 4, 4, 3]);
  const heroStern = kf([0, 56, 74, 88, 94, 110, 122, 133], [0, 0, 0.7, 0.4, 0.8, 0.95, 0.2, 0]);
  const grab = lf >= 56 && lf < 70 ? Math.abs(Math.sin((lf - 56) * 0.5)) : 0;   // his nub opens and closes on nothing

  // ===================== THE VILLAIN: enters as NOTHING, exits owning everything =====================
  const VP = [0, 24, 30, 36, 44, 56, 62, 68, 78, 86, 90, 96, 100, 104, 142, 154];
  const vx = kf(VP, [392, 392, 350, 328, 322, 356, 300, 150, 150, 270, 360, 470, 566, 612, 612, 372]);
  const vy = kf(VP, [466, 466, 486, 498, 500, 496, 516, 524, 520, 486, 478, 484, 476, 468, 468, 392]);
  const VS = 200 * kf([142, 154], [1, 1.5]);
  const vTilt = kf([24, 30, 38, 43], [0, 12, 10, 0]);                // peels at a 12 degree tilt, then snaps flat
  const vKey = kf([24, 36, 40, 46], [0.16, 0.3, 0.42, 1]);           // a rim-lit silhouette, then fully keyed by the fire
  const glint = pulse(40, 7);
  const vWalk = (lf > 56 && lf < 104) || lf > 142 ? 3.2 : lf < 44 ? 0 : 1.1;
  const vHopP = Math.max(0, Math.sin(lf / (10 * 0.6)));
  const vRide = -vHopP * vWalk * 2.2;
  const sep = Math.max(0, HX - vx);                                  // the rim-lit gap of firelight opening between two bodies
  const rainO = kf([30, 56, 70, 104], [0.34, 0.6, 0.76, 1]);
  const settle = ramp(lf, 142, 154);                                 // fixed-x ONLY for the turn to lens

  // ===================== THE TAGGER: the only prop in the reel that changes owner =====================
  let gr = 0, gn = 0;
  for (const s of SHOTS) {
    const r = lf < s.at ? interpolate(lf, [s.at - 10, s.at], [0, 1], cl) : Math.max(0, 1 - (lf - s.at) / 8);
    if (r > gr) { gr = r; gn = s.n; }
  }
  const held = lf < 50 ? { x: HX, y: HY + hRide, s: 200 } : { x: vx, y: vy + vRide, s: VS };
  const tg = gn ? TGT[gn] : null;
  const nubL = { x: held.x + held.s * 0.105, y: held.y + held.s * 0.5 };
  const nubR = { x: held.x + held.s * 0.895, y: held.y + held.s * 0.5 };
  const nub = tg && tg.x > held.x + held.s * 0.5 ? nubR : nubL;
  const gunX = tg ? lerp(nub.x, tg.x, gr) : nubL.x;
  const gunY = tg ? lerp(nub.y, tg.y, gr) : nubL.y;
  const gunRot = (Math.atan2(gunY - nub.y, gunX - nub.x) * 180) / Math.PI + (gn ? 0 : 118) - thwack * 16;
  const spool = 1 - ((lf * 0.9) % 34) / 34;                           // the spool visibly unwinds all scene
  const tailLen = 34 + fired * 22 + (lf % 30) * 0.5;                  // free proof of volume

  // ===================== LAYER WHEEL: the spinning-coin exit, still wobbling at the cut =====================
  const wRoll = ramp(lf, 22, 96);
  const wX = lerp(330, 238, Math.pow(wRoll, 0.7)), wY = lerp(620, 690, Math.pow(wRoll, 1.7));
  const wDie = ramp(lf, 88, 150);
  const wSquash = 1 - wDie * 0.86;
  const wWob = Math.sin(lf * (0.5 + wDie * 1.5)) * (1 - wDie * 0.5);

  // ===================== LAYER LINER: sticker, liner, and the thing it came off, all in frame at once =====================
  const linerOn = lf >= 30;
  const lnX = kf([30, 34, 38], [404, 468, 500]);
  const lnY = kf([30, 33, 36, 38], [478, 548, 622, 660]);
  const lnRot = kf([30, 33, 36, 38, 46], [-16, 26, -9, 5, 3]) + (lf > 38 ? Math.sin(lf * 0.13) * 1.2 : 0);
  const lnSq = kf([30, 36, 38], [1, 0.7, 0.4]);

  // ===================== THE FOUR PASS TAGS: they unhook in the background, they land in the readable plane =====================
  const TAGS = [
    { hx: 700, hy: 300, off: 112, land: 116, lx: 640, ly: 602, ls: 1 },
    { hx: 790, hy: 296, off: 122, land: 126, lx: 836, ly: 554, ls: 0.86 },
    { hx: 860, hy: 292, off: 134, land: 138, lx: 924, ly: 520, ls: 0.74 },
    { hx: 912, hy: 288, off: 999, land: 999, lx: 0, ly: 0, ls: 0.64 },   // car4's is still hanging, still swinging
  ];

  return (
    <Panel label="self-issued">
      <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
      <div style={{ position: "absolute", inset: 0, transformOrigin: "506px 560px", transform: `translate(${camX}px, ${camY}px) scale(${push}) rotate(${camRot}deg)` }}>

        {/* ==================== BG: night sky, sodium haze, the strip-mall block ==================== */}
        <div style={{ position: "absolute", left: 0, top: 46, width: 1012, height: 372, background: "linear-gradient(180deg,#141119 0%,#1B1620 46%,#241A18 100%)" }} />
        <div style={{ position: "absolute", left: 0, top: 150, width: 1012, height: 268, background: `radial-gradient(ellipse at 15% 100%, rgba(240,147,46,${0.2 * key}), transparent 62%)` }} />
        {Array.from({ length: 26 }).map((_, i) => {
          const r = seed(i * 3.1 + 4);
          return <div key={"st" + i} style={{ position: "absolute", left: 300 + r * 700, top: 56 + seed(i * 1.9) * 120, width: 2, height: 2, borderRadius: 1, background: "rgba(226,232,244,0.6)", opacity: 0.18 + Math.abs(Math.sin(lf * 0.04 + i)) * 0.3 }} />;
        })}
        <div style={{ position: "absolute", left: 566, top: 74, width: 446, height: 300, background: grad("#2A2229", "#171319"), borderTop: "3px solid rgba(210,180,150,0.16)", borderLeft: "2px solid rgba(210,180,150,0.1)" }} />
        <div style={{ position: "absolute", left: 0, top: 208, width: 578, height: 166, background: grad("#241D24", "#141116"), borderTop: "3px solid rgba(210,180,150,0.12)" }} />
        {/* the soot bloom the fire has been painting on this facade for a week */}
        <div style={{ position: "absolute", left: 40, top: 96, width: 300, height: 190, background: `radial-gradient(ellipse at 40% 100%, rgba(20,12,8,${0.5 + key * 0.14}), transparent 68%)`, filter: "blur(9px)" }} />
        {/* the rust weep under the gantry */}
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={"rw" + i} style={{ position: "absolute", left: 596 + i * 84, top: 214, width: 4, height: 40 + seed(i * 7) * 44, background: "linear-gradient(180deg, rgba(150,96,50,0.5), transparent)", opacity: 0.5 }} />
        ))}
        {/* the shuttered bays: tape residue ghosting the glass */}
        {[[16, 250], [180, 250]].map((b, i) => (
          <div key={"bay" + i} style={{ position: "absolute", left: b[0], top: b[1], width: 148, height: 122, background: "repeating-linear-gradient(180deg,#2E2731 0 9px,#211B24 9px 14px)", border: "2px solid rgba(180,160,140,0.14)", borderRadius: 3, boxShadow: "inset 0 0 26px rgba(0,0,0,0.7)" }}>
            <div style={{ position: "absolute", left: 22, top: 18, width: 62, height: 34, border: "2px solid rgba(230,220,200,0.09)", transform: `rotate(${i ? -4 : 3}deg)` }} />
            <div style={{ position: "absolute", left: 8, top: 74, width: 90, height: 3, background: "rgba(230,220,200,0.07)" }} />
          </div>
        ))}

        {/* ==================== PROP 9: THE JIFFY LOOB OVAL. shape only, no words, one dead tube. ==================== */}
        <div style={{ position: "absolute", left: 610, top: 96, width: 290, height: 80 }}>
          <div style={{ position: "absolute", left: -6, top: -6, width: 302, height: 92, borderRadius: 46, background: `radial-gradient(ellipse, rgba(231,178,76,${0.16 + key * 0.06}), transparent 68%)`, filter: "blur(7px)" }} />
          <div style={{ position: "absolute", left: 0, top: 0, width: 290, height: 80, borderRadius: 40, background: grad("#E7B24C", "#B87C22"), border: "4px solid #8E2E22", boxShadow: "0 12px 26px rgba(0,0,0,0.6), inset 0 3px 0 rgba(255,240,200,0.4)" }} />
          <div style={{ position: "absolute", left: 12, top: 10, width: 258, height: 52, borderRadius: 30, border: "3px solid rgba(142,46,34,0.75)" }} />
          {[0, 1, 2, 3, 4].map((i) => {
            const dead = i === 2;
            const on = dead ? (Math.floor(lf / 7) % 2 === 0 ? 0.14 : 0.9) : 0.9;
            return <div key={"jl" + i} style={{ position: "absolute", left: 40 + i * 46, top: 26, width: 26, height: 28, borderRadius: 6, background: "#8E2E22", opacity: on, boxShadow: on > 0.5 ? "0 0 12px rgba(255,220,180,0.5)" : "none" }} />;
          })}
          <div style={{ position: "absolute", left: 140, top: 80, width: 10, height: 22, background: "#3A2F2A" }} />
        </div>

        {/* ==================== PROP 8: POST 2, the sodium pole with the bad ballast ==================== */}
        <div style={{ position: "absolute", left: 110, top: 110, width: 40, height: 512 }}>
          <div style={{ position: "absolute", left: 12, top: 26, width: 12, height: 496, background: grad("#4A4048", "#231D24"), boxShadow: "2px 0 4px rgba(0,0,0,0.5)" }} />
          <div style={{ position: "absolute", left: -2, top: 0, width: 62, height: 26, borderRadius: "12px 12px 3px 3px", background: grad("#5A5058", "#282228"), border: "1.5px solid rgba(220,200,170,0.22)" }} />
        </div>
        {(() => {
          const stut = Math.floor(lf / 5) % 9 === 0 ? 0.34 : 1;
          const lampO = (0.2 + Math.abs(Math.sin(lf * 0.7)) * 0.05) * stut;
          return <>
            <div style={{ position: "absolute", left: 116, top: 118, width: 30, height: 12, borderRadius: 6, background: "#FFE9A8", opacity: 0.6 * stut, boxShadow: `0 0 ${16 + flick * 8}px #FFDF92` }} />
            <div style={{ position: "absolute", left: -160, top: 128, width: 600, height: 620, transformOrigin: "50% 0%", background: `linear-gradient(180deg, rgba(255,228,160,${lampO}), rgba(255,228,160,0.03) 62%, transparent)`, clipPath: "polygon(45.5% 0%, 54.5% 0%, 92% 100%, 8% 100%)", mixBlendMode: "screen", pointerEvents: "none", zIndex: 33 }} />
          </>;
        })()}
        {/* dust and ash ripping upward through the beam, FROZEN f24-f36 for the peel */}
        {Array.from({ length: 34 }).map((_, i) => {
          const r = seed(i * 2.7 + 11);
          const cy = lf <= 24 ? lf : lf <= 36 ? 24 : lf - 12;
          const y = 748 - ((cy * (1.5 + r * 3) + r * 620) % 640);
          const x = 130 + (y - 128) * (r - 0.5) * 0.62 + Math.sin(cy * 0.05 + i) * 8;
          return <div key={"ash" + i} style={{ position: "absolute", left: x, top: y, width: 2 + r * 3, height: 2 + r * 3, borderRadius: "50%", background: r > 0.62 ? "rgba(255,196,110,0.85)" : "rgba(236,230,216,0.5)", opacity: 0.2 + r * 0.5, zIndex: 34 }} />;
        })}
        {/* the moth that cooks itself on the lens and drops at f66 */}
        {(() => {
          const dead = lf > 66;
          const dy = dead ? Math.min(500, Math.pow(lf - 66, 2) * 0.36) : 0;
          const mx = 130 + Math.sin(lf * 0.14) * (dead ? 3 : 22);
          return <div style={{ position: "absolute", left: mx, top: 126 + Math.cos(lf * 0.11) * 9 + dy, width: 8, height: 5, borderRadius: 3, background: "rgba(255,246,220,0.9)", opacity: dead ? Math.max(0, 1 - dy / 480) : 0.85, transform: `rotate(${Math.sin(lf * (dead ? 1.4 : 0.8)) * 60}deg)`, zIndex: 34 }} />;
        })()}

        {/* ==================== PROP 10 + 11: THE LAST WEEK GANTRY. the scene's ONE header. ==================== */}
        <div style={{ position: "absolute", left: 640, top: 206, width: 330, height: 10, borderRadius: 3, background: grad("#5C5054", "#282024"), boxShadow: "0 6px 12px rgba(0,0,0,0.6)", zIndex: 6 }} />
        {[690, 920].map((px, i) => <div key={"gp" + i} style={{ position: "absolute", left: px, top: 210, width: 7, height: 200, background: grad("#4A4046", "#221C20"), zIndex: 5 }} />)}
        {(() => {
          const sw = Math.sin(lf * 0.055) * 1.9 + thwack * 1.4 + c1Roll * 0.6;
          return (
            <div style={{ position: "absolute", left: 680, top: 214, width: 250, transformOrigin: "50% 0%", transform: `rotate(${sw}deg)`, zIndex: 7 }}>
              {[16, 226].map((cx, i) => <div key={"ch" + i} style={{ position: "absolute", left: cx, top: 0, width: 3, height: 22, background: "#9A8E86" }} />)}
              <div style={{ position: "absolute", left: 0, top: 22, width: 250, height: 56, borderRadius: 7, background: grad("#33292C", "#1B1518"), border: "2.5px solid rgba(231,178,76,0.44)", boxShadow: "0 12px 24px rgba(0,0,0,0.7), inset 0 2px 0 rgba(255,240,200,0.1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ fontFamily: fraunces.fontFamily, fontWeight: 900, fontSize: 34, letterSpacing: "0.01em", color: "rgba(243,231,205,0.94)" }}>LAST WEEK</div>
              </div>
              {TAGS.map((t2, i) => {
                if (lf >= t2.off) return null;
                const tsw = Math.sin(lf * 0.11 + i * 1.7) * 8 + thwack * 5;
                return (
                  <div key={"tg" + i} style={{ position: "absolute", left: t2.hx - 680, top: t2.hy - 214, transformOrigin: "50% 0%", transform: `rotate(${tsw}deg) scale(${t2.ls})` }}>
                    <div style={{ position: "absolute", left: 12, top: -8, width: 3, height: 12, background: "#B4A69A" }} />
                    <div style={{ position: "relative", width: 27, height: 36, borderRadius: 4, background: grad(FAKE, "#7E8E32"), border: "2px solid rgba(52,60,16,0.6)", boxShadow: "0 5px 10px rgba(0,0,0,0.6)" }}>
                      <div style={{ position: "absolute", left: 8, top: 3, width: 7, height: 7, borderRadius: "50%", background: "rgba(20,24,8,0.55)" }} />
                      <div style={{ position: "absolute", left: 3, top: 15, width: 17, height: 2, background: "rgba(30,36,10,0.4)" }} />
                      <div style={{ position: "absolute", left: 3, top: 21, width: 11, height: 2, background: "rgba(30,36,10,0.4)" }} />
                    </div>
                  </div>
                );
              })}
        ...
        {/* ==================== PROP 12: THE LAST WEEK ROW ==================== */}
        {/* car4: cropped by the panel. sag begins f144 and it is STILL FOLDING at the cut, tag still on its hook. */}
        <div style={{ position: "absolute", left: 952, top: 368 + c4Fold * 6, width: 88, height: 74, filter: `blur(${rowBlur}px) brightness(${rowExp})`, transformOrigin: "50% 100%", transform: `translateX(${c4Fold * 5}px) skewX(${c4Fold * 5}deg) scaleY(${1 - c4Fold * 0.3})`, zIndex: 4 }}>
          <div style={{ position: "absolute", left: 8, top: 0, width: 68, height: 26, borderRadius: "8px 8px 0 0", background: grad("#4C5064", "#282C3C") }} />
          <div style={{ position: "absolute", left: 0, top: 24, width: 88, height: 34, borderRadius: 5, background: grad("#565A70", "#2A2E3E"), border: "1px solid rgba(200,210,240,0.14)" }} />
          <div style={{ position: "absolute", left: 22, top: 6, width: 42, height: 16, borderRadius: 3, background: "rgba(150,180,220,0.3)" }} />
          <div style={{ position: "absolute", left: 30, top: 9, width: 15, height: 11, borderRadius: 2, background: "rgba(168,184,74,0.4)", transform: "rotate(-6deg)" }} />
          {[6, 62].map((wx, i) => <div key={"w4" + i} style={{ position: "absolute", left: wx, top: 52 - c4Fold * 4, width: 20, height: 20, borderRadius: "50%", background: "#161A22" }} />)}
        </div>
        {/* car3: hit at f126, sinks onto its rims at f130. the lonely hazard lamp starts and never stops. */}
        <div style={{ position: "absolute", left: 891, top: 372 + c3Fold * 12, width: 118, height: 96, filter: `blur(${rowBlur}px) brightness(${rowExp})`, transformOrigin: "50% 100%", transform: `translateX(${c3Fold * 10}px) skewX(${c3Fold * 6}deg) scaleY(${1 - c3Fold * 0.24})`, zIndex: 5 }}>
          <div style={{ position: "absolute", left: 12, top: 0, width: 92, height: 34, borderRadius: "10px 10px 0 0", background: grad("#4C5064", "#282C3C"), transform: `skewX(${-c3Fold * 9}deg)` }} />
          <div style={{ position: "absolute", left: 0, top: 32, width: 118, height: 44, borderRadius: 6, background: grad("#565A70", "#2A2E3E"), border: "1px solid rgba(200,210,240,0.16)" }} />
          <div style={{ position: "absolute", left: 28, top: 8, width: 60, height: 22, borderRadius: 3, background: "rgba(150,180,220,0.32)" }} />
          <div style={{ position: "absolute", left: 40, top: 12, width: 20, height: 14, borderRadius: 2, background: "rgba(168,184,74,0.42)", transform: "rotate(5deg)" }} />
          {[8, 84].map((wx, i) => <div key={"w3" + i} style={{ position: "absolute", left: wx, top: 68 - c3Fold * 8, width: 26, height: 26, borderRadius: "50%", background: "#161A22", transform: `scaleY(${1 - c3Fold * 0.5})` }} />)}
          {/* PROP 13: one small amber pulse in the dark, still ticking on the last frame */}
          <div style={{ position: "absolute", left: 96, top: 40, width: 11, height: 9, borderRadius: 2, background: hazard ? AMBER : "rgba(120,90,40,0.4)", boxShadow: hazard ? `0 0 16px ${AMBER}` : "none" }} />
        </div>
        {/* car2: nose hit at f114, roof folds inward f118, dust drifting up for a full second after */}
        <div style={{ position: "absolute", left: 784, top: 376 + c2Fold * 16, width: 176, height: 130, filter: `blur(${rowBlur}px) brightness(${rowExp})`, transformOrigin: "50% 100%", transform: `translateX(${c2Fold * 14}px) skewX(${c2Fold * 7}deg)`, zIndex: 6 }}>
          <div style={{ position: "absolute", left: 20, top: 2 + c2Fold * 22, width: 134, height: 48, borderRadius: "12px 12px 0 0", background: grad("#4C5064", "#282C3C"), transform: `skewX(${-c2Fold * 12}deg) scaleY(${1 - c2Fold * 0.46})`, transformOrigin: "50% 100%" }} />
          <div style={{ position: "absolute", left: 0, top: 46, width: 176, height: 60, borderRadius: 8, background: grad("#565A70", "#2A2E3E"), border: "1.5px solid rgba(200,210,240,0.18)" }} />
          <div style={{ position: "absolute", left: 40, top: 12 + c2Fold * 24, width: 92, height: 32, borderRadius: 4, background: "rgba(150,180,220,0.34)", transform: `scaleY(${1 - c2Fold * 0.46})`, transformOrigin: "50% 100%" }} />
          <div style={{ position: "absolute", left: 58, top: 18 + c2Fold * 22, width: 28, height: 20, borderRadius: 3, background: "rgba(168,184,74,0.44)", transform: "rotate(-7deg)" }} />
          {[12, 126].map((wx, i) => <div key={"w2" + i} style={{ position: "absolute", left: wx, top: 94, width: 38, height: 38, borderRadius: "50%", background: "#161A22", border: "3px solid #2C313E" }} />)}
          {lf >= 118 && Array.from({ length: 5 }).map((_, i) => {
            const p = ((lf - 118) * 0.03 + seed(i * 4)) % 1;
            return <div key={"d2" + i} style={{ position: "absolute", left: 30 + seed(i * 3) * 120, top: 40 - p * 46, width: 20 + p * 22, height: 20 + p * 22, borderRadius: "50%", background: "radial-gradient(circle, rgba(214,196,168,0.3), transparent 66%)", opacity: (1 - p) * 0.7 }} />;
          })}
        </div>
        {/* CAR1: MIDGROUND. the villain's own plane, zero blur, full exposure, never dimmed.
            this is why the backhand is an event and not a rumour. */}
        <div style={{ position: "absolute", left: 590, top: 386, width: 300, height: 190, transformOrigin: "18% 100%", transform: `translate(${c1Roll * 30}px, ${c1Roll * 4}px) rotate(${c1Roll * 3.4}deg)`, zIndex: 9 }}>
          <div style={{ position: "absolute", left: 18, top: 168, width: 268, height: 20, borderRadius: "50%", background: "rgba(0,0,0,0.6)", filter: "blur(9px)" }} />
          <div style={{ position: "absolute", left: 34, top: 0, width: 232, height: 78, borderRadius: "16px 16px 0 0", background: grad("#5E6278", "#2E3242"), border: "2px solid rgba(210,220,245,0.2)" }} />
          {/* the rear glass, wearing the faded pass the same gun gave it last week */}
          <div style={{ position: "absolute", left: 56, top: 12, width: 188, height: 52, borderRadius: 6, background: "linear-gradient(160deg, rgba(158,188,228,0.4), rgba(60,80,120,0.5))", border: "1.5px solid rgba(220,232,255,0.2)", overflow: "hidden" }}>
            <div style={{ position: "absolute", left: -40, top: -20, width: 90, height: 110, background: `rgba(255,206,140,${0.06 + key * 0.1})`, transform: "rotate(24deg)" }} />
          </div>
          <div style={{ position: "absolute", left: 82, top: 22, width: 40, height: 30, borderRadius: 4, background: "rgba(168,184,74,0.46)", border: "1.5px solid rgba(60,70,20,0.4)", transform: "rotate(-5deg)" }} />
          {/* STICKER 07: straight over the faded one. he re-certifies what he is about to destroy. */}
          {lf >= 104 && (
            <div style={{ position: "absolute", left: 78, top: 16, transform: `scale(${popIn(104) * 0.56}) rotate(-9deg)`, transformOrigin: "50% 50%" }}>
              <DoneSticker x={0} y={0} s={1} rot={0} o={1} />
            </div>
          )}
          <div style={{ position: "absolute", left: 0, top: 72, width: 300, height: 84, borderRadius: "10px 10px 8px 8px", background: grad("#6A6E86", "#313546"), border: "2px solid rgba(210,220,245,0.22)", boxShadow: "inset 0 3px 0 rgba(255,255,255,0.1)" }} />
          {/* the near rear quarter: inside his reach, and it buckles ON CONTACT */}
          <div style={{ position: "absolute", left: 0, top: 66, width: 96, height: 96, borderRadius: "12px 4px 8px 10px", background: grad("#727690", "#363B4E"), border: "2px solid rgba(210,220,245,0.22)", transform: `skewY(${c1Buck * 9}deg) scaleX(${1 - c1Buck * 0.16})`, transformOrigin: "0% 50%" }}>
            {c1Buck > 0 && Array.from({ length: 4 }).map((_, i) => (
              <div key={"cr" + i} style={{ position: "absolute", left: 10 + i * 18, top: 22 + seed(i * 5) * 40, width: 3, height: 26 * c1Buck, background: "rgba(16,18,26,0.75)", transform: `rotate(${(seed(i) - 0.5) * 60}deg)` }} />
            ))}
          </div>
          <div style={{ position: "absolute", left: 6, top: 104, width: 54, height: 12, borderRadius: 4, background: grad("#9AA0B6", "#4A5062") }} />
          {[36, 226].map((wx, i) => (
            <div key={"w1" + i} style={{ position: "absolute", left: wx, top: 130, width: 56, height: 56, borderRadius: "50%", background: grad("#252A36", "#0E1117"), border: "5px solid #3E4454", boxShadow: "0 8px 16px rgba(0,0,0,0.6)" }}>
              <div style={{ position: "absolute", left: 12, top: 12, width: 22, height: 22, borderRadius: "50%", background: grad("#8E94A8", "#454B5A") }} />
            </div>
          ))}
        </div>

        {/* ==================== THE APRON: worn chalk, arrows, hatch marks. zero digits, no resolvable glyphs. ==================== */}
        <div style={{ position: "absolute", left: 0, top: 404, width: 1012, height: 388, background: "linear-gradient(180deg,#241E22 0%,#2C2529 34%,#332A2A 100%)", zIndex: 2 }} />
        <div style={{ position: "absolute", left: 0, top: 404, width: 1012, height: 388, background: `radial-gradient(ellipse at 14% 24%, rgba(240,140,44,${0.24 * key}), transparent 58%)`, zIndex: 2 }} />
        <svg width={1012} height={792} style={{ position: "absolute", left: 0, top: 0, zIndex: 3 }}>
          {[[60, 792, 430, 452], [340, 792, 560, 448], [700, 792, 700, 446], [1000, 780, 852, 448]].map((l, i) => (
            <line key={"bl" + i} x1={l[0]} y1={l[1]} x2={l[2]} y2={l[3]} stroke="rgba(226,214,190,0.09)" strokeWidth={4} strokeDasharray="34 22" />
          ))}
          <path d="M 300 742 L 372 742 L 372 726 L 410 754 L 372 782 L 372 766 L 300 766 Z" fill="rgba(226,214,190,0.07)" />
          {Array.from({ length: 7 }).map((_, i) => (
            <line key={"ht" + i} x1={620 + i * 17} y1={716} x2={600 + i * 17} y2={758} stroke="rgba(226,214,190,0.06)" strokeWidth={5} />
          ))}
          {Array.from({ length: 9 }).map((_, i) => (
            <path key={"ck" + i} d={`M ${40 + i * 112} ${470 + seed(i * 3) * 300} l ${18 + seed(i) * 40} ${(seed(i * 2) - 0.5) * 26} l ${12 + seed(i * 5) * 26} ${(seed(i * 7) - 0.5) * 22}`} stroke="rgba(10,8,10,0.5)" strokeWidth={1.6} fill="none" />
          ))}
        </svg>
        {Array.from({ length: 30 }).map((_, i) => {
          const r = seed(i * 5.7 + 2);
          return <div key={"gr" + i} style={{ position: "absolute", left: 20 + r * 970, top: 440 + seed(i * 2.1) * 340, width: 2 + r * 3, height: 1.5 + r * 2, borderRadius: 1, background: "rgba(14,10,12,0.6)", opacity: 0.5, zIndex: 3 }} />;
        })}
        {/* PROP 23: POST 2's debris circle. cooked moths, dead tube glass, liner scraps. WEAR THAT MOVES. */}
        {Array.from({ length: 22 }).map((_, i) => {
          const r = seed(i * 4.3 + 9);
          const a = r * Math.PI * 2;
          const lift = thwack * 8 + belch * 10;
          const dd = 20 + seed(i * 2.9) * 92;
          return <div key={"db" + i} style={{ position: "absolute", left: 130 + Math.cos(a) * dd + lift * Math.cos(a), top: 664 + Math.sin(a) * dd * 0.34 - lift * 0.7 * Math.abs(Math.sin(lf * 0.3 + i)), width: 4 + r * 6, height: 2 + r * 3, borderRadius: 1, background: r > 0.66 ? "rgba(168,184,74,0.55)" : r > 0.33 ? "rgba(236,230,216,0.32)" : "rgba(90,70,50,0.7)", transform: `rotate(${r * 180 + lift * 3}deg)`, zIndex: 4 }} />;
        })}

        {/* ==================== PROP 7: THE OIL SLICK. a black mirror holding the fire, rippling on every THWACK. ==================== */}
        <div style={{ position: "absolute", left: 200, top: 640, width: 360, height: 60, borderRadius: "50%", background: "radial-gradient(ellipse at 40% 40%, rgba(28,22,26,0.96), rgba(10,8,10,0.9))", boxShadow: "0 0 24px rgba(0,0,0,0.8)", zIndex: 10, transform: `scaleY(${1 + thwack * 0.05})` }}>
          {Array.from({ length: 5 }).map((_, i) => {
            const p = (lf * 0.02 + i * 0.2 + thwack * 0.3) % 1;
            return <div key={"rp" + i} style={{ position: "absolute", left: 180 - p * 170, top: 30 - p * 26, width: p * 340, height: p * 52, borderRadius: "50%", border: `1.5px solid rgba(240,150,60,${(1 - p) * 0.24 * key})` }} />;
          })}
          <div style={{ position: "absolute", left: 20, top: 8, width: 130, height: 40, borderRadius: "50%", background: `radial-gradient(ellipse, rgba(240,140,44,${0.42 * key}), transparent 70%)`, filter: "blur(4px)" }} />
          <div style={{ position: "absolute", left: 214, top: 20, width: 42, height: 16, borderRadius: "50%", background: "rgba(226,206,120,0.16)", filter: "blur(2px)" }} />
        </div>
        {/* the planted banana skin */}
        <div style={{ position: "absolute", left: 246, top: 654, width: 40, height: 16, zIndex: 11, transform: `rotate(${-14 + thwack * 4}deg)` }}>
          <div style={{ position: "absolute", left: 0, top: 6, width: 34, height: 8, borderRadius: "6px 2px 2px 6px", background: grad("#D8C44E", "#8E7C1E") }} />
          <div style={{ position: "absolute", left: 12, top: 0, width: 28, height: 7, borderRadius: 4, background: grad("#C8B444", "#7E6E18"), transform: "rotate(22deg)" }} />
        </div>

        {/* ==================== PROP 2: UNIT 04, THE WRECK. the thing being certified. ====================
            silhouette signature, locked here for S4: (a) the hood at 80 degrees on ONE hinge,
            (b) the bare hub with the disc still lazily spinning, (c) sticker 05 smeared across the hood, corner curled. */}
        <div style={{ position: "absolute", left: 30, top: 546, width: 76, height: 76, zIndex: 12 }}>
          <div style={{ position: "absolute", left: 8, top: 62, width: 60, height: 12, borderRadius: "50%", background: "rgba(0,0,0,0.6)", filter: "blur(4px)" }} />
          <div style={{ position: "absolute", left: 4, top: 4, width: 68, height: 68, borderRadius: "50%", background: grad("#8A8E9C", "#3A3E4A"), border: "3px solid #5A5E6C", boxShadow: "0 6px 14px rgba(0,0,0,0.6)", transform: `rotate(${lf * 2.1}deg)` }}>
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={"vn" + i} style={{ position: "absolute", left: 31, top: 6, width: 3, height: 56, background: "rgba(20,18,22,0.5)", transformOrigin: "50% 50%", transform: `rotate(${i * 22.5}deg)` }} />
            ))}
            <div style={{ position: "absolute", left: 22, top: 22, width: 24, height: 24, borderRadius: "50%", background: grad("#D4A056", "#7A5A20"), border: "2px solid rgba(255,230,180,0.3)" }} />
          </div>
          {[0, 1, 2, 3, 4].map((i) => (
            <div key={"lg" + i} style={{ position: "absolute", left: 34 + Math.cos((i / 5) * 6.283) * 17, top: 34 + Math.sin((i / 5) * 6.283) * 17, width: 6, height: 6, borderRadius: "50%", background: "#C9CFDC" }} />
          ))}
        </div>
        {/* STICKER 04: DONE, on a wheel that is not there */}
        {lf >= 70 && (
          <div style={{ position: "absolute", left: 22, top: 552, zIndex: 13, transform: `scale(${popIn(70) * 0.5}) rotate(14deg)`, transformOrigin: "50% 50%" }}>
            <DoneSticker x={0} y={0} s={1} rot={0} o={1} />
          </div>
        )}
        {/* the body: three-quarter front, the nose at screen-left, receding to the rear at x 330 */}
        <div style={{ position: "absolute", left: 30, top: 360, width: 320, height: 260, zIndex: 12 }}>
          <div style={{ position: "absolute", left: 6, top: 232, width: 316, height: 24, borderRadius: "50%", background: "rgba(0,0,0,0.62)", filter: "blur(10px)" }} />
          <div style={{ position: "absolute", left: 8, top: 76, width: 116, height: 74, borderRadius: "10px 4px 4px 8px", background: grad("#8E5A3E", "#4A2E20"), border: "2px solid rgba(255,190,140,0.18)" }} />
          <div style={{ position: "absolute", left: 0, top: 118, width: 40, height: 26, borderRadius: 4, background: grad("#A8A296", "#4E4A42") }} />
          <div style={{ position: "absolute", left: 10, top: 86, width: 62, height: 34, borderRadius: 3, background: "repeating-linear-gradient(90deg,#3A3238 0 5px,#221C20 5px 9px)", border: "2px solid rgba(200,180,150,0.16)" }} />
          {/* the open engine bay: the fire lives in here */}
          <div style={{ position: "absolute", left: 78, top: 62, width: 172, height: 62, borderRadius: 5, background: "linear-gradient(180deg,#0A0608,#1E1210)", border: "2px solid rgba(120,84,60,0.4)", boxShadow: `inset 0 0 34px rgba(255,120,30,${0.5 * key})` }} />
          {/* the cowl, the windscreen and the cabin */}
          <div style={{ position: "absolute", left: 214, top: 24, width: 92, height: 88, borderRadius: "16px 8px 3px 3px", background: grad("#9A6244", "#4E3020"), border: "2px solid rgba(255,190,140,0.18)", transform: "skewX(-9deg)" }} />
          <div style={{ position: "absolute", left: 226, top: 34, width: 66, height: 50, borderRadius: 4, background: "linear-gradient(160deg, rgba(150,180,220,0.3), rgba(40,54,80,0.6))", border: "1.5px solid rgba(220,232,255,0.18)", transform: "skewX(-9deg)" }} />
          <div style={{ position: "absolute", left: 60, top: 108, width: 262, height: 90, borderRadius: "6px 8px 6px 6px", background: grad("#966042", "#4A2C1E"), border: "2px solid rgba(255,190,140,0.16)", boxShadow: "inset 0 4px 0 rgba(255,200,150,0.08)" }} />
          {/* the rear quarter + fender: the hero's one shot lands here */}
          <div style={{ position: "absolute", left: 264, top: 128, width: 58, height: 76, borderRadius: "4px 10px 8px 4px", background: grad("#A06A48", "#523222") }} />
          <div style={{ position: "absolute", left: 268, top: 196, width: 52, height: 10, borderRadius: 4, background: grad("#B0A69A", "#4E4640") }} />
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={"rst" + i} style={{ position: "absolute", left: 80 + i * 40, top: 186, width: 5, height: 12 + seed(i * 3) * 26, background: "linear-gradient(180deg, rgba(140,74,32,0.7), transparent)" }} />
          ))}
          <div style={{ position: "absolute", left: 60, top: 108, width: 130, height: 90, background: "radial-gradient(ellipse at 20% 0%, rgba(10,6,8,0.6), transparent 70%)" }} />
          <div style={{ position: "absolute", left: 244, top: 188, width: 62, height: 62, borderRadius: "50%", background: grad("#232833", "#0C0F14"), border: "5px solid #3C424E", transform: `rotate(${Math.sin(lf * 0.08) * 3}deg)` }}>
            <div style={{ position: "absolute", left: 15, top: 15, width: 22, height: 22, borderRadius: "50%", background: grad("#88909E", "#3E4450") }} />
          </div>
          {/* STICKER 03: the hero's ONLY shot. it bit at f0 and the gun left him before he fired again. */}
          <div style={{ position: "absolute", left: 268, top: 154, transform: `scale(${popIn(0) * 0.48}) rotate(-7deg)`, transformOrigin: "50% 50%" }}>
            <DoneSticker x={0} y={0} s={1} rot={0} o={1} />
          </div>
        </div>

        {/* PROP 5: THE TAPED DOOR. held on by two crossed strips and a pass. it takes a SECOND pass at f86. */}
        {(() => {
          const sw = 4 + Math.sin(lf * 0.13) * 2 + pulse(86, 26) * 16 + thwack * 3 + belch * 5;
          return (
            <div style={{ position: "absolute", left: 274, top: 446, width: 74, height: 116, transformOrigin: "6% 12%", transform: `rotate(${sw}deg)`, zIndex: 14 }}>
              <div style={{ position: "absolute", left: 0, top: 0, width: 70, height: 112, borderRadius: "5px 7px 7px 5px", background: grad("#9A6444", "#4C2E1E"), border: "2px solid rgba(255,190,140,0.2)", boxShadow: "0 10px 20px rgba(0,0,0,0.6)" }} />
              <div style={{ position: "absolute", left: 10, top: 8, width: 48, height: 30, borderRadius: 3, background: "linear-gradient(160deg, rgba(150,180,220,0.26), rgba(40,54,80,0.55))" }} />
              <div style={{ position: "absolute", left: 44, top: 52, width: 22, height: 6, borderRadius: 3, background: grad("#C4BAAA", "#6A6258") }} />
              {/* the two crossed strips of gaffer tape doing the actual engineering */}
              <div style={{ position: "absolute", left: -14, top: 30, width: 96, height: 15, background: "repeating-linear-gradient(90deg,#3A3A3E 0 6px,#2A2A2E 6px 11px)", border: "1px solid rgba(0,0,0,0.5)", transform: "rotate(26deg)", boxShadow: "0 3px 6px rgba(0,0,0,0.5)" }} />
              <div style={{ position: "absolute", left: -14, top: 30, width: 96, height: 15, background: "repeating-linear-gradient(90deg,#3A3A3E 0 6px,#2A2A2E 6px 11px)", border: "1px solid rgba(0,0,0,0.5)", transform: "rotate(-26deg)", boxShadow: "0 3px 6px rgba(0,0,0,0.5)" }} />
              {/* the sticker that was already there, slapped over the tape */}
              <div style={{ position: "absolute", left: 8, top: 46, transform: "scale(0.42) rotate(-11deg)", transformOrigin: "50% 50%", filter: "saturate(0.6) brightness(0.8)" }}>
                <DoneSticker x={0} y={0} s={1} rot={0} o={0.85} />
              </div>
              {/* STICKER 06: a second pass, on top of the first, on a door held on with tape */}
              {lf >= 86 && (
                <div style={{ position: "absolute", left: 14, top: 40, transform: `scale(${popIn(86) * 0.46}) rotate(9deg)`, transformOrigin: "50% 50%" }}>
                  <DoneSticker x={0} y={0} s={1} rot={0} o={1} />
                </div>
              )}
            </div>
        ...
        {/* the hood: 80 degrees, ONE hinge, bouncing on its spring the entire scene */}
        {(() => {
          const bounce = Math.sin(lf * 0.31) * 1.6 + belch * 9 + thwack * 1.2;
          const curl = ramp(lf, 78, 154);
          return (
            <div style={{ position: "absolute", left: 268, top: 430, width: 170, height: 30, transformOrigin: "98% 60%", transform: `rotate(${-80 - bounce}deg)`, zIndex: 15 }}>
              <div style={{ position: "absolute", left: 0, top: 0, width: 166, height: 27, borderRadius: "10px 3px 3px 10px", background: grad("#A46A48", "#4E3020"), border: "2px solid rgba(255,190,140,0.2)", boxShadow: "0 8px 18px rgba(0,0,0,0.55)" }} />
              <div style={{ position: "absolute", left: 6, top: 4, width: 150, height: 5, borderRadius: 3, background: "rgba(255,214,170,0.14)" }} />
              <div style={{ position: "absolute", left: 150, top: -6, width: 18, height: 12, borderRadius: 3, background: grad("#9AA0AE", "#4A5060") }} />
              <div style={{ position: "absolute", left: 156, top: 22, width: 4, height: 16, background: "#6A7080" }} />
              <div style={{ position: "absolute", left: 12, top: 22, width: 46, height: 5, background: "rgba(20,14,10,0.6)" }} />
              {/* STICKER 05: THE MONEY SHOT. fat, crooked, ink smeared, corner already cooking. */}
              {lf >= 78 && (
                <div style={{ position: "absolute", left: 42, top: -20, transform: "rotate(74deg)", transformOrigin: "50% 50%" }}>
                  <div style={{ position: "relative", transform: `scale(${popIn(78) * 0.62}) rotate(-13deg)`, transformOrigin: "50% 50%" }}>
                    <DoneSticker x={0} y={0} s={1} rot={0} o={1} />
                    <div style={{ position: "absolute", left: 74, top: 14, width: 44, height: 46, borderRadius: 6, background: "linear-gradient(90deg, rgba(168,184,74,0.5), transparent)", filter: "blur(2px)", zIndex: 27 }} />
                    <div style={{ position: "absolute", left: 62, top: 0, width: 36 * curl + 6, height: 34 * curl + 6, zIndex: 28, transformOrigin: "100% 0%", transform: `rotate(${-curl * 8}deg)` }}>
                      <div style={{ position: "absolute", left: 0, top: 0, right: 0, bottom: 0, background: "linear-gradient(225deg, #6E7A28, #F0EEE0 70%)", clipPath: "polygon(100% 0, 100% 100%, 0 0)", boxShadow: "-3px 3px 7px rgba(0,0,0,0.6)" }} />
                    </div>
                  </div>
                </div>
              )}
        ...
        {/* ==================== PROP 3: THE FLAME COLUMN. six flat planes. it re-keys the frame on every lick. ==================== */}
        <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, zIndex: 17, pointerEvents: "none" }}>
          {Array.from({ length: 6 }).map((_, i) => {
            const ph = lf * (0.17 + i * 0.04) + i * 1.9;
            const h = fireH * (1 - i * 0.11) * (0.9 + Math.sin(ph) * 0.13);
            const w = (152 - i * 20) * (0.88 + Math.cos(ph * 1.4) * 0.16);
            const col = i < 2 ? "linear-gradient(180deg, rgba(196,74,58,0), #C44A3A 58%, #E9803C)" : i < 4 ? "linear-gradient(180deg, rgba(240,147,46,0), #F0932E 52%, #FFC66A)" : "linear-gradient(180deg, rgba(255,246,214,0), #FFE9A8 50%, #FFFBEE)";
            return <div key={"fl" + i} style={{ position: "absolute", left: FIRE_X - w / 2 + Math.sin(ph * 0.7) * 8, top: FIRE_BASE - h, width: w, height: h, background: col, clipPath: "polygon(50% 0%, 76% 30%, 64% 54%, 88% 76%, 100% 100%, 0% 100%, 12% 76%, 36% 54%, 24% 30%)", transform: `skewX(${Math.sin(ph * 0.8) * 9}deg)`, opacity: 0.6 + i * 0.06, filter: "blur(0.5px)" }} />;
          })}
          {/* f138: it jumps to a rear tyre. the last half second is escalating. */}
          {tyreFire > 0 && Array.from({ length: 4 }).map((_, i) => {
            const ph = lf * 0.22 + i * 2.1;
            const h = (18 + tyreFire * 74) * (0.8 + Math.sin(ph) * 0.2);
            return <div key={"tf" + i} style={{ position: "absolute", left: 258 + i * 12, top: 250 - h, width: 26 - i * 3, height: h, background: i < 2 ? "linear-gradient(180deg, rgba(240,147,46,0), #F0932E 60%, #FFC66A)" : "linear-gradient(180deg, rgba(255,246,214,0), #FFE9A8)", clipPath: "polygon(50% 0%, 100% 100%, 0% 100%)", transform: `skewX(${Math.sin(ph) * 12}deg)`, opacity: 0.72 * tyreFire }} />;
          })}
          <div style={{ position: "absolute", left: FIRE_X - 340, top: FIRE_BASE - 400, width: 680, height: 640, background: `radial-gradient(circle at 50% 62%, rgba(255,150,50,${0.2 * key}), transparent 62%)`, filter: "blur(12px)", mixBlendMode: "screen" }} />
        </div>
        {/* smoke rolling up-left through Post 2's beam for the entire scene */}
        {Array.from({ length: 16 }).map((_, i) => {
          const r = seed(i * 3.9 + 5);
          const p = (lf * (0.006 + r * 0.008) + r) % 1;
          const s = (120 + r * 200) * (0.4 + p * 1.7) * (0.8 + grow * 0.35);
          return <div key={"sk" + i} style={{ position: "absolute", left: FIRE_X + r * 90 - p * 190 - s / 2, top: FIRE_BASE - 90 - p * 330 - s / 2, width: s, height: s, borderRadius: "50%", background: "radial-gradient(circle at 42% 42%, rgba(60,48,44,0.5), rgba(50,40,38,0.14) 58%, transparent 74%)", opacity: Math.min(0.62, (1 - p) * 0.62), filter: "blur(6px)", zIndex: 18 }} />;
        })}
        {Array.from({ length: 38 }).map((_, i) => {
          const r = seed(i * 2.3 + 17);
          const p = (lf * (0.008 + r * 0.014) + r) % 1;
          return <div key={"eb" + i} style={{ position: "absolute", left: FIRE_X - 40 + r * 180 - p * 160 + Math.sin(lf * 0.09 + i) * 14, top: FIRE_BASE - p * (300 + r * 300), width: 2 + r * 4, height: 2 + r * 4, borderRadius: "50%", background: r > 0.5 ? "#FFC66A" : "#F0932E", boxShadow: `0 0 ${5 + r * 7}px rgba(255,170,70,0.85)`, opacity: (1 - p) * (0.35 + grow * 0.3), zIndex: 19 }} />;
        })}

        {/* ==================== PROP 15: THE TOOL TROLLEY, THE MUG, AND THE FOLDED TARP (the S4 plant) ==================== */}
        <div style={{ position: "absolute", left: 440, top: 566, width: 108, height: 118, zIndex: 13 }}>
          <div style={{ position: "absolute", left: 4, top: 106, width: 102, height: 12, borderRadius: "50%", background: "rgba(0,0,0,0.6)", filter: "blur(5px)" }} />
          <div style={{ position: "absolute", left: 0, top: 22, width: 104, height: 12, borderRadius: 3, background: grad("#B44A38", "#6A2418"), border: "1.5px solid rgba(255,180,150,0.22)", boxShadow: "0 5px 10px rgba(0,0,0,0.5)" }} />
          <div style={{ position: "absolute", left: 4, top: 66, width: 96, height: 11, borderRadius: 3, background: grad("#9A3E30", "#5A1E14") }} />
          {[6, 90].map((rx, i) => <div key={"tr" + i} style={{ position: "absolute", left: rx, top: 30, width: 7, height: 76, background: grad("#6E6660", "#302A28") }} />)}
          {[12, 84].map((cx, i) => <div key={"cw" + i} style={{ position: "absolute", left: cx, top: 100, width: 13, height: 13, borderRadius: "50%", background: "#1E1A1C", border: "2px solid #4A4448" }} />)}
          {/* THE TARP, folded and soot-edged on the lower shelf. nobody touches it in S1. */}
          <div style={{ position: "absolute", left: 18, top: 44, width: 70, height: 22, borderRadius: 3, background: "repeating-linear-gradient(180deg,#3E6250 0 5px,#2E4A3C 5px 10px)", border: "1.5px solid rgba(140,180,160,0.2)", boxShadow: "0 4px 8px rgba(0,0,0,0.5)" }}>
            <div style={{ position: "absolute", left: 0, top: 0, width: 70, height: 6, background: "rgba(16,12,10,0.55)" }} />
            <div style={{ position: "absolute", left: -3, top: 4, width: 5, height: 5, borderRadius: "50%", background: "#8E9098" }} />
            <div style={{ position: "absolute", left: 66, top: 12, width: 5, height: 5, borderRadius: "50%", background: "#8E9098" }} />
          </div>
          {/* the loose 17mm socket that rolls off the top tray at f118 */}
          {(() => {
            const g = ramp(lf, 118, 136);
            return <div style={{ position: "absolute", left: 62 + g * 46, top: 14 + Math.pow(g, 2) * 104, width: 13, height: 13, borderRadius: 3, background: grad("#C4C8D2", "#5E626C"), border: "1.5px solid rgba(20,20,24,0.5)", transform: `rotate(${g * 420}deg)` }} />;
          })()}
          {[26, 42, 56].map((sx, i) => <div key={"so" + i} style={{ position: "absolute", left: sx, top: 14, width: 9, height: 9, borderRadius: 2, background: grad("#B4B8C2", "#54585E") }} />)}
        </div>
        {/* the half-drunk mug: its surface rings on every THWACK and lurches on the backhand */}
        <div style={{ position: "absolute", left: 476, top: 552, width: 34, height: 34, zIndex: 14, transform: `rotate(${pulse(106, 12) * 9}deg)`, transformOrigin: "50% 100%" }}>
          <div style={{ position: "absolute", left: 0, top: 4, width: 27, height: 30, borderRadius: "3px 3px 6px 6px", background: grad("#E4DED0", "#A29A88"), border: "1.5px solid rgba(30,26,24,0.4)" }} />
          <div style={{ position: "absolute", left: 25, top: 11, width: 10, height: 12, borderRadius: "0 6px 6px 0", border: "2.5px solid #C8C0AE", borderLeft: "none" }} />
          <div style={{ position: "absolute", left: 3, top: 14, width: 21, height: 13, borderRadius: 2, background: "#3A2A1E", overflow: "hidden" }}>
            <div style={{ position: "absolute", left: 0, top: 4 + Math.sin(lf * 0.9) * 1.2 + thwack * 3, width: 21, height: 10, background: "#5A4028" }} />
          </div>
        </div>

        {/* ==================== PROP 18: THE ROLL-BIN OF SPENT LINER. a week of counterfeit passes as a physical mass. ==================== */}
        <div style={{ position: "absolute", left: 406, top: 662, width: 92, height: 84, zIndex: 13 }}>
          <div style={{ position: "absolute", left: 0, top: 22, width: 88, height: 62, borderRadius: "5px 5px 9px 9px", background: grad("#4E4A52", "#221E26"), border: "2px solid rgba(190,200,220,0.2)", boxShadow: "0 10px 20px rgba(0,0,0,0.6), inset 0 0 20px rgba(0,0,0,0.7)" }} />
          {Array.from({ length: 20 }).map((_, i) => {
            const r = seed(i * 3.3 + 1);
            const heap = fired * 3.4;
            return <div key={"lo" + i} style={{ position: "absolute", left: 2 + r * 78, top: 20 - r * 20 - heap * (0.4 + r * 0.6), width: 16 + r * 24, height: 5 + r * 5, borderRadius: 4, background: r > 0.5 ? "rgba(244,242,232,0.9)" : "rgba(214,212,200,0.8)", transform: `rotate(${(r - 0.5) * 90}deg)`, boxShadow: "0 2px 4px rgba(0,0,0,0.5)" }} />;
          })}
          <div style={{ position: "absolute", left: 0, top: 34, width: 88, height: 6, background: "rgba(168,184,74,0.3)" }} />
        </div>
        {/* PROP 17: THE BACKING-PAPER TAIL, out of the gun's throat, longer on every shot, still feeding at the cut */}
        <svg width={1012} height={792} style={{ position: "absolute", left: 0, top: 0, zIndex: 14, pointerEvents: "none" }}>
          <path d={`M ${gunX} ${gunY} C ${gunX - 30} ${gunY + tailLen * 0.5}, ${lerp(gunX, 500, 0.6)} ${640 - tailLen * 0.18}, 500 660`} stroke="rgba(120,116,108,0.4)" strokeWidth={9} fill="none" strokeLinecap="round" />
          <path d={`M ${gunX} ${gunY} C ${gunX - 30} ${gunY + tailLen * 0.5}, ${lerp(gunX, 500, 0.6)} ${640 - tailLen * 0.18}, 500 660 C ${470 + Math.sin(lf * 0.1) * 10} 676, 460 682, 448 674`} stroke="rgba(244,242,232,0.82)" strokeWidth={7} fill="none" strokeLinecap="round" strokeDasharray="13 5" />
        </svg>

        {/* ==================== PROP 19: THE LINER SILHOUETTE. what makes the peel legible instead of poetic. ==================== */}
        {linerOn && (
          <div style={{ position: "absolute", left: lnX - 46, top: lnY - 42, width: 92, height: 84, zIndex: 15, transform: `rotate(${lnRot}deg) scaleY(${lnSq})`, transformOrigin: "50% 100%", filter: `drop-shadow(0 6px 10px rgba(0,0,0,0.55)) brightness(${0.8 + key * 0.4})` }}>
            <svg viewBox="0 0 200 200" width={92} height={84} shapeRendering="crispEdges" style={{ overflow: "visible" }}>
              <rect x={8} y={86} width={26} height={26} fill="#F6F4EC" />
              <rect x={166} y={86} width={26} height={26} fill="#F6F4EC" />
              <rect x={34} y={44} width={132} height={102} fill="#F6F4EC" />
              <rect x={52} y={146} width={17} height={38} fill="#F6F4EC" />
              <rect x={77} y={146} width={17} height={38} fill="#F6F4EC" />
              <rect x={124} y={146} width={17} height={38} fill="#F6F4EC" />
              <rect x={149} y={146} width={17} height={38} fill="#F6F4EC" />
              <rect x={34} y={44} width={132} height={16} fill="#FFFFFF" />
              <rect x={44} y={64} width={54} height={5} fill="rgba(190,186,172,0.6)" />
              <rect x={44} y={96} width={112} height={4} fill="rgba(190,186,172,0.45)" />
              <rect x={44} y={116} width={78} height={4} fill="rgba(190,186,172,0.45)" />
            </svg>
          </div>
        )}

        {/* ==================== PROP 14: THE DEAD EXTINGUISHER. it wears a pass. it does not work. ==================== */}
        {(() => {
          const onStand = lf < 80;
          const dropped = lf >= 106;
          const ex = onStand ? 566 : dropped ? 520 : kf([80, 106], [566, 528]);
          const ey = onStand ? 520 : dropped ? kf([106, 114, 154], [546, 652, 656]) : kf([80, 106], [520, 508]);
          const shake = lf >= 94 && lf < 100 ? Math.sin(lf * 5.5) * 9 : 0;
          const rot = dropped ? kf([106, 116, 154], [10, 92, 92]) + Math.sin((lf - 116) * 0.34) * 4 : shake + (lf >= 88 ? -14 : 0);
          const hiss = 0.3 + Math.abs(Math.sin(lf * 0.5)) * 0.35;
          return <>
            <div style={{ position: "absolute", left: 560, top: 566, width: 46, height: 56, zIndex: 12 }}>
              <div style={{ position: "absolute", left: 16, top: 0, width: 12, height: 54, background: grad("#5E5660", "#2A242A") }} />
              <div style={{ position: "absolute", left: 0, top: 50, width: 44, height: 8, borderRadius: 3, background: grad("#5E5660", "#28222A") }} />
              <div style={{ position: "absolute", left: 4, top: -8, width: 38, height: 10, borderRadius: 3, border: `2px solid ${lf < 80 ? "#8A3A2C" : "#5A4A44"}`, borderTop: "none" }} />
            </div>
            <div style={{ position: "absolute", left: ex, top: ey, width: 42, height: 88, zIndex: dropped ? 11 : 24, transformOrigin: "50% 80%", transform: `rotate(${rot}deg)` }}>
              <div style={{ position: "absolute", left: 2, top: 18, width: 38, height: 66, borderRadius: "6px 6px 8px 8px", background: grad("#C4402E", "#6E1C12"), border: "2px solid rgba(255,170,140,0.24)", boxShadow: "0 8px 16px rgba(0,0,0,0.6), inset -6px 0 12px rgba(0,0,0,0.35)" }} />
              <div style={{ position: "absolute", left: 12, top: 4, width: 18, height: 18, borderRadius: 3, background: grad("#A8ACB8", "#4E525C") }} />
              <div style={{ position: "absolute", left: 4, top: 0, width: 34, height: 7, borderRadius: 3, background: grad("#C8CCD6", "#5E626C") }} />
              {/* the pressure needle, buried in the red */}
              <div style={{ position: "absolute", left: 28, top: 6, width: 16, height: 16, borderRadius: "50%", background: "#EDEAE0", border: "2px solid #4E525C" }}>
                <div style={{ position: "absolute", left: 0, top: 0, width: 12, height: 12, borderRadius: "50%", background: "conic-gradient(#C44A3A 0 34%, #EDEAE0 34% 100%)" }} />
                <div style={{ position: "absolute", left: 6, top: 3, width: 2, height: 5, background: "#1A1813", transformOrigin: "50% 100%", transform: `rotate(${-64 + shake * 0.4}deg)` }} />
              </div>
              {/* the broken seal, dangling */}
              <div style={{ position: "absolute", left: 6, top: 10, width: 10, height: 3, background: FAKE, transform: `rotate(${40 + Math.sin(lf * 0.7) * 22}deg)`, transformOrigin: "0% 50%" }} />
              {/* the pass it is wearing. it was certified full. */}
              <div style={{ position: "absolute", left: 2, top: 32, transform: "scale(0.34) rotate(-8deg)", transformOrigin: "50% 50%", filter: "saturate(0.7) brightness(0.85)" }}>
                <DoneSticker x={0} y={0} s={1} rot={0} o={0.9} />
              </div>
              {/* the thin useless plume out of the cracked valve, all scene */}
              {Array.from({ length: 6 }).map((_, i) => {
                const p = (lf * 0.03 + i * 0.17) % 1;
                return <div key={"hs" + i} style={{ position: "absolute", left: 34 + p * 26, top: 4 - p * 16, width: 4 + p * 12, height: 4 + p * 12, borderRadius: "50%", background: "rgba(232,236,240,0.4)", opacity: (1 - p) * hiss, filter: "blur(2px)" }} />;
              })}
            </div>
            {/* f98: one weak grey cough of powder that the fire eats */}
            {lf >= 98 && lf < 118 && Array.from({ length: 10 }).map((_, i) => {
              const p = ramp(lf, 98, 114) + seed(i * 3) * 0.2;
              const r = seed(i * 5.1);
              return <div key={"pw" + i} style={{ position: "absolute", left: 500 - p * (60 + r * 60), top: 500 + p * (30 + r * 40), width: 12 + p * 30, height: 12 + p * 30, borderRadius: "50%", background: "radial-gradient(circle, rgba(196,198,196,0.5), transparent 68%)", opacity: Math.max(0, 1 - p * 1.6), filter: "blur(3px)", zIndex: 21 }} />;
            })}
          </>;
        })()}

        {/* ==================== PROP 6: THE ESCAPING WHEEL. the spinning-coin exit, still wobbling at the cut. ==================== */}
        <div style={{ position: "absolute", left: wX - 36, top: wY - 36, width: 72, height: 72, zIndex: 20 }}>
          <div style={{ position: "absolute", left: 2, top: 58, width: 68, height: 12, borderRadius: "50%", background: "rgba(0,0,0,0.55)", filter: "blur(4px)" }} />
          <div style={{ position: "absolute", left: 0, top: 0, width: 72, height: 72, transformOrigin: "50% 100%", transform: `rotate(${wWob * 24}deg) scaleY(${wSquash}) rotate(${wRoll * -640 + wDie * Math.sin(lf * (0.6 + wDie * 2)) * 40}deg)` }}>
            <div style={{ position: "absolute", left: 0, top: 0, width: 72, height: 72, borderRadius: "50%", background: grad("#2A303C", "#0E1117"), border: "7px solid #3E4450", boxShadow: "0 8px 16px rgba(0,0,0,0.6)" }}>
              <div style={{ position: "absolute", left: 15, top: 15, width: 28, height: 28, borderRadius: "50%", background: grad("#9098A6", "#42485A"), border: "2px solid rgba(220,232,255,0.25)" }} />
              {[0, 1, 2, 3, 4].map((i) => <div key={"wb" + i} style={{ position: "absolute", left: 27 + Math.cos((i / 5) * 6.283) * 10, top: 27 + Math.sin((i / 5) * 6.283) * 10, width: 4, height: 4, borderRadius: "50%", background: "rgba(230,240,255,0.6)" }} />)}
            </div>
          </div>
        </div>

        {/* ==================== THE PEEL: a hard rim-lit gap of firelight opening between two overlapping bodies ==================== */}
        {lf >= 22 && lf < 52 && (
          <div style={{ position: "absolute", left: vx + 118, top: HY + 40, width: Math.max(1, sep + 24), height: 156, zIndex: 20, background: `linear-gradient(90deg, rgba(255,190,90,${0.5 * key}), rgba(255,240,200,${0.85 * key}) 46%, rgba(255,170,60,${0.4 * key}))`, filter: "blur(3px)", opacity: ramp(lf, 22, 28) * (1 - ramp(lf, 44, 52)) }} />
        )}
        {/* f18: the die-cut notch line opening down the hero's flame side */}
        {lf >= 18 && lf < 34 && (
          <div style={{ position: "absolute", left: HX + 32, top: HY + 40 + hRide, width: 5, height: 148 * ramp(lf, 18, 24), zIndex: 24, background: "repeating-linear-gradient(180deg, rgba(255,248,224,0.95) 0 5px, rgba(255,180,80,0.2) 5px 10px)", boxShadow: "0 0 12px rgba(255,214,140,0.9)", opacity: 1 - ramp(lf, 28, 34) }} />
        )}

        {/* ==================== THE VILLAIN. code-rain PARENTED to his body, travelling with him. ==================== */}
        {lf >= 22 && (
          <div style={{ position: "absolute", left: vx, top: vy, width: VS, height: VS, zIndex: lf < 40 ? 19 : 32, transformOrigin: "50% 96%", transform: `rotate(${vTilt}deg)` }}>
            {/* PROP 20: offsets from his body origin. 2 columns at f30, the 3rd on the rip, the 4th on his first DONE. */}
            {[{ o: -24, at: 30, tp: 430 }, { o: 32, at: 30, tp: 478 }, { o: 88, at: 56, tp: 542 }, { o: 136, at: 70, tp: 598 }].map((c, i) => {
              const co = ramp(lf, c.at, c.at + 11) * rainO;
              if (co <= 0.01) return null;
              return <CodeRain key={"cr" + i} lf={lf + i * 29} x={lerp(c.o * (VS / 200), c.tp - vx, settle)} y={-VS * 0.16} h={VS * 0.72} cols={1} o={co} gap={0} />;
            })}
            <div style={{ position: "absolute", left: 0, top: 0, filter: `brightness(${vKey}) contrast(${lerp(1.5, 1, ramp(lf, 30, 44))})` }}>
              <Villain lf={lf} size={VS} rain={0} nodAmp={vWalk} nodSpeed={10} gaze={lf > 142 ? 0 : lf > 104 ? -1 : 1} stern={lf > 142 ? 1 : 0} />
            </div>
            {/* the fire's rim light on his flame side, all scene */}
            <div style={{ position: "absolute", left: VS * 0.15, top: VS * 0.2 + vRide, width: 4, height: VS * 0.52, background: `linear-gradient(180deg, rgba(255,214,140,${0.9 * key}), rgba(255,150,60,${0.4 * key}))`, filter: "blur(1.4px)", zIndex: 3 }} />
            {/* f40: the shades resolve LAST. one hard white glint bar, and it never softens. */}
            {lf >= 40 && (
              <div style={{ position: "absolute", left: VS * 0.27, top: VS * 0.335 + vRide, width: VS * 0.1, height: VS * 0.035, background: "#FFFFFF", transform: "rotate(-16deg)", boxShadow: `0 0 ${6 + glint * 26}px rgba(255,255,255,${0.5 + glint * 0.5})`, zIndex: 4, opacity: 0.85 + glint * 0.15 }} />
            )}
            {glint > 0 && (
              <div style={{ position: "absolute", left: VS * -0.3 + glint * VS * 0.9, top: VS * 0.28 + vRide, width: VS * 0.24, height: VS * 0.2, background: "linear-gradient(96deg, transparent, rgba(255,255,255,0.85), transparent)", transform: "skewX(-20deg)", zIndex: 5 }} />
            )}
            {/* his cast shadow, swinging on every lick */}
            <div style={{ position: "absolute", left: VS * 0.1, top: VS * 0.9, width: VS * 0.8, height: VS * 0.1, borderRadius: "50%", background: "rgba(0,0,0,0.55)", filter: "blur(6px)", transform: `skewX(${34 + flick * 16}deg) scaleX(${1 + flick * 0.3})`, zIndex: 0 }} />
          </div>
        )}

        {/* ==================== THE HERO. on frame 0, ACTING. ==================== */}
        <div style={{ position: "absolute", left: HX, top: HY, width: 200, height: 200, zIndex: 22, transformOrigin: "50% 88%", transform: `rotate(${heroRot}deg)` }}>
          <div style={{ position: "absolute", left: 20, top: 178, width: 160, height: 16, borderRadius: "50%", background: "rgba(0,0,0,0.5)", filter: "blur(6px)", transform: `skewX(${30 + flick * 14}deg)` }} />
          <Mascot lf={lf} size={200} gaze={heroGaze} nodAmp={heroNod} nodSpeed={9} stern={heroStern} shock={heroShock} cheer={ramp(lf, 126, 132) * (1 - ramp(lf, 133, 136)) * 0.75} capBack={lf < 137 ? 1 : 0} hiVis={1} />
          {/* the workshop coverall, worn OPEN over the hi-vis chevron vest, carried from the night shift */}
          <div style={{ position: "absolute", left: 30, top: 96 + hRide, width: 20, height: 52, background: grad("#2E4A63", "#1A2C3C"), borderRight: "2px solid rgba(180,210,240,0.2)", transform: "skewY(6deg)" }} />
          <div style={{ position: "absolute", left: 150, top: 96 + hRide, width: 20, height: 52, background: grad("#2E4A63", "#1A2C3C"), borderLeft: "2px solid rgba(180,210,240,0.2)", transform: "skewY(-6deg)" }} />
          {/* the blank oval name patch */}
          <div style={{ position: "absolute", left: 116, top: 116 + hRide, width: 24, height: 13, borderRadius: 7, background: "rgba(238,232,218,0.75)", border: "1.5px solid rgba(60,50,40,0.35)" }} />
          {/* THE RED L BADGE, velcroed to the chest: the safest identity token in the reel */}
          <div style={{ position: "absolute", left: 62, top: 112 + hRide, width: 22, height: 22, borderRadius: 4, background: PAPER, border: `2.5px solid ${RED}`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: fraunces.fontFamily, fontWeight: 900, fontSize: 14, color: RED, lineHeight: 1, boxShadow: "0 3px 6px rgba(0,0,0,0.5)", transform: "rotate(-4deg)" }}>L</div>
          {/* the soot smear on his cheek. it grows through the scene. */}
          <div style={{ position: "absolute", left: 132, top: 84 + hRide, width: 14 + ramp(lf, 0, 154) * 14, height: 9 + ramp(lf, 0, 154) * 5, borderRadius: 5, background: "rgba(28,20,18,0.5)", filter: "blur(1.6px)", transform: "rotate(-12deg)" }} />
          {/* the welding visor, flipped up on the cap brim */}
          {lf < 137 && <div style={{ position: "absolute", left: 116, top: 8 + hRide, width: 44, height: 11, borderRadius: 5, background: grad("#3E5E74", "#1E2E3C"), border: "1.5px solid rgba(180,220,245,0.3)", transformOrigin: "0% 100%", transform: "rotate(-38deg)" }} />}
          {/* f106: the rag comes out and he beats at the hood fire. f116: the belch drives him back, rag smoking. */}
          {lf >= 106 && lf < 130 && (
            <div style={{ position: "absolute", left: 4 + Math.sin(lf * 0.8) * 12, top: 66 + hRide + Math.cos(lf * 0.8) * 16, width: 34, height: 22, borderRadius: 4, background: grad("#B4483A", "#6A2418"), transform: `rotate(${Math.sin(lf * 0.8) * 40}deg)`, boxShadow: "0 4px 8px rgba(0,0,0,0.5)" }}>
              {lf >= 116 && Array.from({ length: 4 }).map((_, i) => {
                const p = (lf * 0.05 + i * 0.25) % 1;
                return <div key={"rs" + i} style={{ position: "absolute", left: 6 + i * 6, top: -p * 26, width: 5 + p * 9, height: 5 + p * 9, borderRadius: "50%", background: "rgba(70,60,56,0.55)", opacity: (1 - p) * 0.8, filter: "blur(2px)" }} />;
              })}
            </div>
          )}
          {lf < 106 && <div style={{ position: "absolute", left: 40, top: 128 + hRide, width: 16, height: 12, borderRadius: 3, background: grad("#B4483A", "#6A2418"), transform: "rotate(-16deg)" }} />}
          {/* f58: the first honest confusion of the whole reel. his nub closes on nothing. */}
          {grab > 0 && <div style={{ position: "absolute", left: 8, top: 86 + hRide, width: 26, height: 26, background: "#D97757", transform: `scaleY(${1 - grab * 0.4}) translateY(${grab * 5}px)` }} />}
          {/* STICKER 08: he certifies the man he just shoved. still crooked on his chest at the cut. */}
          {lf >= 140 && (
            <div style={{ position: "absolute", left: 76, top: 96, transform: `scale(${popIn(140) * 0.52}) rotate(17deg)`, transformOrigin: "50% 50%", zIndex: 6 }}>
              <DoneSticker x={0} y={0} s={1} rot={0} o={1} />
            </div>
          )}
        </div>
        {/* the cap, coming off */}
        {lf >= 137 && (
          <div style={{ position: "absolute", left: 470 - (lf - 137) * 9, top: 640 - Math.sin(ramp(lf, 137, 154) * 3.14) * 54, width: 62, height: 20, zIndex: 23, transform: `rotate(${(lf - 137) * 22}deg)` }}>
            <div style={{ position: "absolute", left: 0, top: 0, width: 48, height: 14, borderRadius: 3, background: grad("#4C7FA3", "#2F5673") }} />
            <div style={{ position: "absolute", left: 44, top: 5, width: 18, height: 6, borderRadius: 2, background: "#2F5673" }} />
          </div>
        )}
        {/* the oil spray off the landing, still travelling */}
        {lf >= 137 && Array.from({ length: 12 }).map((_, i) => {
          const p = ramp(lf, 137, 150) + seed(i * 2) * 0.25;
          const r = seed(i * 4.4);
          return <div key={"os" + i} style={{ position: "absolute", left: 540 - p * (80 + r * 120), top: 664 - Math.sin(p * 3.14) * (30 + r * 40), width: 5 + r * 7, height: 4 + r * 5, borderRadius: "50%", background: "rgba(18,14,16,0.85)", opacity: Math.max(0, 1 - p), zIndex: 21 }} />;
        })}

        {/* ==================== PROP 1: THE TAGGER. it changes owner on camera. ==================== */}
        <div style={{ position: "absolute", left: gunX, top: gunY, width: 1, height: 1, zIndex: 33 }}>
          <div style={{ position: "absolute", left: -78, top: -22, width: 92, height: 46, transformOrigin: "88% 50%", transform: `rotate(${gunRot}deg) scale(${lf > 142 ? 1.3 : 1})` }}>
            {/* the exposed spool of die-cut stickers on top: it visibly unwinds all scene */}
            <div style={{ position: "absolute", left: 30, top: -22, width: 30, height: 30, borderRadius: "50%", background: grad(FAKE, "#77862C"), border: "2px solid rgba(40,48,12,0.6)", transform: `rotate(${-lf * 8}deg)`, boxShadow: "0 3px 7px rgba(0,0,0,0.5)" }}>
              <div style={{ position: "absolute", left: 11, top: 11, width: 8, height: 8, borderRadius: "50%", background: "#2A2E14" }} />
              <div style={{ position: "absolute", left: 13, top: 1, width: 3, height: 12, background: "rgba(40,48,12,0.5)" }} />
            </div>
            <div style={{ position: "absolute", left: 32 - spool * 4, top: -8, width: 26, height: 8, borderRadius: 2, background: FAKE, border: "1px solid rgba(40,48,12,0.5)" }} />
            {/* chunky moulded body */}
            <div style={{ position: "absolute", left: 24, top: -4, width: 62, height: 30, borderRadius: "6px 8px 8px 6px", background: grad("#E88A38", "#8E4A12"), border: "2px solid rgba(60,30,6,0.5)", boxShadow: "0 6px 12px rgba(0,0,0,0.6), inset 0 2px 0 rgba(255,220,170,0.4)" }} />
            {/* the ratchet trigger */}
            <div style={{ position: "absolute", left: 62, top: 24, width: 11, height: 20, borderRadius: "0 0 4px 4px", background: grad("#3E3A38", "#1A1614"), transform: `rotate(${thwack * 22}deg)`, transformOrigin: "50% 0%" }} />
            {/* the hard rubber head + the spring-loaded feed foot: it CLACKS on every shot */}
            <div style={{ position: "absolute", left: -2, top: -2, width: 30, height: 26, borderRadius: "5px 2px 2px 5px", background: grad("#3A3634", "#141210"), border: "2px solid rgba(180,170,160,0.2)", transform: `translateX(${-thwack * 5}px)` }} />
            <div style={{ position: "absolute", left: -6, top: 20, width: 22, height: 7, borderRadius: 2, background: grad("#9AA0A8", "#464C54"), transform: `translateX(${-thwack * 7}px) rotate(${thwack * -10}deg)` }} />
            {/* the small brass flip-counter on the flank: the scene's ONE number */}
            <div style={{ position: "absolute", left: 36, top: 2, width: 34, height: 22, borderRadius: 3, background: grad("#4A3A1E", "#241A0C"), border: "1.5px solid rgba(231,190,110,0.6)", boxShadow: "inset 0 2px 4px rgba(0,0,0,0.7)", overflow: "hidden", perspective: "60px" }}>
              <div style={{ position: "absolute", left: 0, top: 0, width: 34, height: 22, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: mono, fontWeight: 700, fontSize: 15, color: GOLD, background: grad("#3A2C14", "#1A1208"), transformOrigin: "50% 0%", transform: `rotateX(${flip * 84}deg)`, textShadow: "0 0 6px rgba(231,178,76,0.7)" }}>{"0" + count}</div>
              <div style={{ position: "absolute", left: 0, top: 10, width: 34, height: 1, background: "rgba(0,0,0,0.8)" }} />
            </div>
          </div>
        </div>

        {/* ==================== THE TAGS THAT CROSS PLANES: they unhook in the background, they land in the readable one ==================== */}
        {TAGS.map((t3, i) => {
          if (lf < t3.off) return null;
          const p = Math.min(1, (lf - t3.off) / (t3.land - t3.off));
          const px = lerp(t3.hx, t3.lx, p);
          const py = lerp(t3.hy, t3.ly, Math.pow(p, 1.9));
          const down = lf >= t3.land;
          const settled = down ? Math.max(0, 1 - (lf - t3.land) / 12) : 0;
          const sc = down ? 1.5 : lerp(t3.ls, 1.4, p);
          return (
            <div key={"ft" + i} style={{ position: "absolute", left: px, top: py, zIndex: down ? 16 : 8, transform: `rotate(${down ? 84 + Math.sin(lf * 0.9) * settled * 12 : p * 340}deg) scale(${sc}) scaleY(${down ? 0.44 : 1})`, transformOrigin: "50% 50%", filter: down ? "drop-shadow(0 5px 8px rgba(0,0,0,0.7))" : "none" }}>
              <div style={{ position: "relative", width: 27, height: 36, borderRadius: 4, background: grad(FAKE, "#7E8E32"), border: "2px solid rgba(52,60,16,0.6)" }}>
                <div style={{ position: "absolute", left: 8, top: 3, width: 7, height: 7, borderRadius: "50%", background: "rgba(20,24,8,0.55)" }} />
                <div style={{ position: "absolute", left: 3, top: 15, width: 17, height: 2, background: "rgba(30,36,10,0.4)" }} />
                <div style={{ position: "absolute", left: 3, top: 21, width: 11, height: 2, background: "rgba(30,36,10,0.4)" }} />
              </div>
        ...
        {/* ==================== PROP 22: THE CUSTOMER. the audience surrogate leaving the building. ==================== */}
        {(() => {
          const cx = 952 + Math.floor(lf / 20) * 9 + interpolate(lf % 20, [0, 6], [0, 9], cl);
          if (cx > 1040) return null;
          return (
            <div style={{ position: "absolute", left: cx, top: 496, zIndex: 8, filter: `blur(${rowBlur * 0.6}px) brightness(${rowExp})` }}>
              <Mascot lf={lf + 17} size={72} gaze={-3} nodAmp={2.6} nodSpeed={7} cheer={0.85} tint="#C08A6A" />
              <div style={{ position: "absolute", left: 46, top: 44, width: 20, height: 16, borderRadius: 3, background: grad("#6E5A44", "#3A2E22"), border: "1px solid rgba(220,200,170,0.2)" }} />
            </div>
          );
        })()}

        {/* ==================== FOREGROUND: PROP 21, THE ATTENDANT. this is fine. ==================== */}
        <div style={{ position: "absolute", left: 852, top: 566, width: 180, height: 226, zIndex: 40 }}>
          <div style={{ position: "absolute", left: 18, top: 6, width: 96, height: 118, borderRadius: "10px 10px 3px 3px", background: grad("#3E6E86", "#1E3A4A"), border: "2px solid rgba(160,200,225,0.2)", boxShadow: "0 14px 26px rgba(0,0,0,0.7)" }}>
            <div style={{ position: "absolute", left: 34, top: 12, width: 3, height: 74, background: "rgba(230,244,250,0.3)", transform: "rotate(9deg)" }} />
            <div style={{ position: "absolute", left: 36, top: 40, width: 22, height: 3, background: "rgba(230,244,250,0.22)", transform: "rotate(-38deg)" }} />
          </div>
          <div style={{ position: "absolute", left: 4, top: 116, width: 124, height: 18, borderRadius: 4, background: grad("#4A7E96", "#22404E"), border: "2px solid rgba(160,200,225,0.18)" }} />
          {[10, 112].map((lx, i) => <div key={"cl" + i} style={{ position: "absolute", left: lx, top: 132, width: 9, height: 62, background: grad("#3A6478", "#183038"), transform: `rotate(${i ? -8 : 8}deg)` }} />)}
          <div style={{ position: "absolute", left: 6, top: 26 }}>
            <Mascot lf={lf + 61} size={118} gaze={-2} nodAmp={0.7} nodSpeed={16} shock={pulse(106, 9) * 0.66} tint="#C87A52" />
            <div style={{ position: "absolute", left: 20, top: 14, width: 78, height: 9, borderRadius: "6px 6px 0 0", border: "3px solid #2A2E38", borderBottom: "none" }} />
            <div style={{ position: "absolute", left: 12, top: 30, width: 13, height: 20, borderRadius: 4, background: "#2A2E38" }} />
            <div style={{ position: "absolute", left: 92, top: 30, width: 13, height: 20, borderRadius: 4, background: "#2A2E38" }} />
            <div style={{ position: "absolute", left: 24, top: 48, width: 18, height: 3, borderRadius: 2, background: "#2A2E38", transform: "rotate(24deg)" }} />
          </div>
          {/* the mug, raised HIGHER after the flinch */}
          <div style={{ position: "absolute", left: 108, top: 66 - ramp(lf, 108, 128) * 26, width: 30, height: 30, transform: `rotate(${-6 - ramp(lf, 108, 128) * 8}deg)` }}>
            <div style={{ position: "absolute", left: 0, top: 2, width: 24, height: 27, borderRadius: "3px 3px 6px 6px", background: grad("#E4DED0", "#A29A88"), border: "1.5px solid rgba(30,26,24,0.4)" }} />
            <div style={{ position: "absolute", left: 22, top: 8, width: 9, height: 11, borderRadius: "0 6px 6px 0", border: "2.5px solid #C8C0AE", borderLeft: "none" }} />
            {Array.from({ length: 3 }).map((_, i) => {
              const p = (lf * 0.026 + i * 0.33) % 1;
              return <div key={"stm" + i} style={{ position: "absolute", left: 6 + i * 6 + Math.sin(lf * 0.13 + i) * 4, top: -p * 26, width: 4, height: 4, borderRadius: "50%", background: "rgba(240,238,230,0.5)", opacity: (1 - p) * 0.7, filter: "blur(1.4px)" }} />;
            })}
          </div>
          {/* the thumbs-up. it never comes down. */}
          <div style={{ position: "absolute", left: 8, top: 52 - ramp(lf, 108, 128) * 8, width: 22, height: 30, transform: `rotate(${-8 + pulse(106, 9) * 20}deg)` }}>
            <div style={{ position: "absolute", left: 0, top: 10, width: 22, height: 20, borderRadius: 5, background: "#C87A52", border: "1.5px solid rgba(60,30,20,0.3)" }} />
            <div style={{ position: "absolute", left: 3, top: 0, width: 9, height: 16, borderRadius: 4, background: "#C87A52", border: "1.5px solid rgba(60,30,20,0.3)" }} />
          </div>
        </div>

        {/* ==================== PROP 16: THE TYRE STACK. foreground occluder, cropping bottom-left. ==================== */}
        <div style={{ position: "absolute", left: -20, top: 646, width: 176, height: 150, zIndex: 41, filter: "blur(2px)", transform: `translateY(${pulse(137, 12) * 4}px)` }}>
          {[0, 1, 2, 3].map((i) => {
            const rock = i === 3 ? ramp(lf, 146, 160) : 0;
            return (
              <div key={"ty" + i} style={{ position: "absolute", left: 2 + rock * 96, top: 116 - i * 33 + rock * 90, width: 158, height: 44, borderRadius: "50%", background: grad("#2E2A30", "#0E0C10"), border: "3px solid #43404A", boxShadow: "0 8px 16px rgba(0,0,0,0.7)", transform: `rotate(${(seed(i) - 0.5) * 5 + rock * 60}deg)` }}>
                <div style={{ position: "absolute", left: 40, top: 10, width: 78, height: 22, borderRadius: "50%", background: "#08070A", boxShadow: "inset 0 3px 8px rgba(0,0,0,0.9)" }} />
                {Array.from({ length: 7 }).map((_, k) => <div key={"tt" + k} style={{ position: "absolute", left: 12 + k * 20, top: 2, width: 5, height: 40, background: "rgba(120,116,128,0.14)", transform: "skewX(-14deg)" }} />)}
              </div>
            );
          })}
        </div>

        {/* ==================== FOREGROUND: drifting smoke, the horizontal sodium flare, heat shimmer ==================== */}
        {Array.from({ length: 8 }).map((_, i) => {
          const r = seed(i * 6.1 + 3);
          const p = (lf * 0.011 + r) % 1;
          const s = (200 + r * 240) * (0.6 + p * 1.4);
          return <div key={"fs" + i} style={{ position: "absolute", left: 60 + r * 800 - p * 170 - s / 2, top: 792 - p * 260 - s / 2, width: s, height: s, borderRadius: "50%", background: "radial-gradient(circle at 44% 44%, rgba(60,52,50,0.22), transparent 66%)", opacity: (1 - p) * 0.5, filter: "blur(8px)", zIndex: 43, pointerEvents: "none" }} />;
        })}
        <div style={{ position: "absolute", left: 0, top: 108, width: 1012, height: 36, background: `linear-gradient(90deg, transparent, rgba(255,226,164,${0.06 + flick * 0.09}) 14%, rgba(255,226,164,${0.03 + flick * 0.05}) 40%, transparent 78%)`, filter: "blur(6px)", zIndex: 44, pointerEvents: "none" }} />
        {Array.from({ length: 7 }).map((_, i) => (
          <div key={"hz" + i} style={{ position: "absolute", left: 600 + i * 58, top: 380 + Math.sin(lf * 0.2 + i) * 8, width: 44, height: 96, borderRadius: 20, background: "rgba(255,220,170,0.05)", filter: "blur(5px)", transform: `skewX(${Math.sin(lf * 0.26 + i * 1.5) * 12}deg)`, zIndex: 30, pointerEvents: "none" }} />
        ))}

        {/* ==================== f150: STICKER 09. flat on the glass, in your face, and it has NOT adhered. ==================== */}
        {lf >= 150 && (
          <div style={{ position: "absolute", left: 0, top: 0, width: 1012, height: 792, zIndex: 60, pointerEvents: "none" }}>
            <div style={{ position: "absolute", left: 320, top: 240, width: 300, height: 60 + (lf - 150) * 5, background: "linear-gradient(180deg, rgba(168,184,74,0.5), transparent)", filter: "blur(5px)", transform: "rotate(-6deg)" }} />
            <div style={{ position: "absolute", left: 356, top: 292 + (lf - 150) * 3.4, transform: `rotate(-6deg) scale(${popIn(150) * 2.4})`, transformOrigin: "50% 50%" }}>
              <DoneSticker x={0} y={0} s={1} rot={0} o={1} />
            </div>
            <div style={{ position: "absolute", left: 330, top: 236, width: 260, height: 26, background: "radial-gradient(ellipse, rgba(255,255,255,0.28), transparent 70%)", filter: "blur(4px)" }} />
          </div>
        )}

        {/* ==================== VIGNETTE ==================== */}
        <div style={{ position: "absolute", left: 0, top: 46, width: 1012, height: 746, pointerEvents: "none", boxShadow: `inset 0 0 210px rgba(0,0,0,${0.62 - key * 0.06})`, zIndex: 70 }} />
      </div>
      </div>
    </Panel>
  );
...
export const VerifyS1Reel: React.FC = () => {
