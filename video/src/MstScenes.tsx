...
export const CAM: Record<Variant, { dx: number; dy: number; s: number; rot: number }> = {
  kerb: { dx: 0,   dy: 10, s: 1.030, rot: 0 },
  rank: { dx: 44,  dy: 26, s: 1.052, rot: 0 },
  gate: { dx: -38, dy: -2, s: 1.014, rot: 0 },
};
...
export const S10: React.FC<SP> = ({ v, dur }) => {
  ...
  const CREW0 = 6, GUIDE = 44, COUNT = [58, 66, 74, 82], KW = 96;
  ...
  return (
    <Scene p={p} slug="" push={[0, dur, 1.182]} vig={0.54} glow={hexa(p.key, 0.22)}>
      ...
      {f >= KW && (
        <div style={{ position: "absolute", left: 0, top: 92, width: W, zIndex: 88,
      ...
      <Ring x={506} y={140} f={f} at={KW} c={hexa(CLAY, 0.8)} z={90} s={0.9} />
