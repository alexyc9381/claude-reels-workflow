...
import words from "./data/words_powers.json";
...
// scene onsets (sec, sped VO): hook / superdesign / superpowers / security / karpathy / playwright / cta
const L = [0, 7.78, 16.89, 27.73, 38.2, 46.71, 52.87];
const Lf = L.map(fr);
const CUT = 55.27;
...
const S0: React.FC<{ lf: number }> = ({ lf }) => {
  ...
  const gemAt = [fr(2.7), fr(3.0), fr(3.3), fr(3.6), fr(3.9)];
  ...
  return (
    <>
      <div style={{ position: "absolute", inset: 0, transform: `translate(${shake}px, ${shake * 0.4}px)` }}>
        ...
        {/* the 5 gems streak in */}
        {gemAt.map((at, i) => { const ap = over(lf, at, fr(0.28), Easing.in(Easing.cubic)); if (ap <= 0.01 || ap >= 1) return null; const sx = i % 2 ? 1040 : -40; return (
          <div key={`g${i}`} style={{ position: "absolute", left: interpolate(ap, [0, 1], [sx, cx + 96]), top: interpolate(ap, [0, 1], [90 + i * 44, 486]), zIndex: 12 }}>
            <Gem s={52} c={GEMS5[i]} />
            <div style={{ position: "absolute", left: -34, top: 20, width: 40, height: 4, background: `linear-gradient(90deg, transparent, ${GEMS5[i]})`, opacity: 0.8 }} />
          </div>); })}
...
const S4: React.FC<{ lf: number }> = ({ lf }) => {
  ...
  const slashE = lf >= slashAt && lf < slashAt + 9 ? 1 - (lf - slashAt) / 9 : 0;
  const cutDone = lf >= slashAt + 4;
  const wreckOn = lf >= fr(5.2);
  const wSwing = Math.sin(lf / 4.5);
  return (
    <>
      {/* night dojo: big moon, petals, paper wall */}
      <div style={{ position: "absolute", left: 96, top: 76, width: 190, height: 190, borderRadius: "50%", background: "radial-gradient(circle at 42% 38%, #F0FAF4, #AFD8CC 70%)", boxShadow: "0 0 80px rgba(200,240,225,0.45)" }} />
      <div style={{ position: "absolute", left: 620, top: 60, width: 330, height: 250, background: "linear-gradient(180deg, #2A3E3A, #1A2A26)", borderRadius: 10, border: "3px solid rgba(150,210,190,0.35)" }}>
        {[0, 1, 2].map((i) => <div key={i} style={{ position: "absolute", left: 20 + i * 104, top: 14, width: 88, height: 220, background: "rgba(255,224,170,0.16)", border: "2.5px solid rgba(150,210,190,0.3)", boxShadow: "inset 0 0 26px rgba(255,214,150,0.14)" }} />)}
        {/* hanging calligraphy scroll in the middle pane */}
        <div style={{ position: "absolute", left: 138, top: 26, width: 56, height: 190, background: "#EDE4CE", border: "3px solid #6E5236", borderRadius: 4 }}>
          <div style={{ position: "absolute", left: "50%", top: 16, width: 8, height: 44, marginLeft: -4, borderRadius: 4, background: "#2A2622", transform: "rotate(6deg)" }} />
          <div style={{ position: "absolute", left: "50%", top: 64, width: 26, height: 9, marginLeft: -13, borderRadius: 4, background: "#2A2622", transform: "rotate(-8deg)" }} />
          <div style={{ position: "absolute", left: "50%", top: 84, width: 8, height: 52, marginLeft: -6, borderRadius: 4, background: "#2A2622", transform: "rotate(-4deg)" }} />
          <div style={{ position: "absolute", left: "50%", top: 146, width: 18, height: 18, marginLeft: -9, borderRadius: "50%", border: "4px solid #C44A3A" }} />
        </div>
      </div>
      {/* hanging paper lantern, gently swinging */}
      <div style={{ position: "absolute", left: 560, top: 0, width: 4, height: 90, background: "rgba(150,210,190,0.3)", transform: `rotate(${Math.sin(lf / 18) * 4}deg)`, transformOrigin: "50% 0%", zIndex: 8 }}>
        <div style={{ position: "absolute", left: -26, top: 86, width: 56, height: 68, borderRadius: "40% 40% 44% 44%", background: "radial-gradient(circle at 50% 40%, #F6D890, #C99442)", border: "2.5px solid #8A6420", boxShadow: "0 0 34px rgba(246,216,144,0.55)" }}>
          {[0, 1, 2].map((i) => <div key={i} style={{ position: "absolute", left: 6 + i * 16, top: 6, width: 2.5, height: 54, background: "rgba(120,80,20,0.4)" }} />)}
        </div>
      </div>
      <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, height: 170, background: "linear-gradient(180deg,#0C1A18,#071010)", clipPath: "polygon(8% 0, 92% 0, 100% 100%, 0 100%)" }} />
      {Array.from({ length: 10 }).map((_, i) => { const t = ((lf / fr(5.5) + seed(i)) % 1); return <div key={`p${i}`} style={{ position: "absolute", left: 120 + seed(i) * 800 - t * 150, top: 100 + t * 560, width: 12, height: 9, borderRadius: "0 60% 0 60%", background: "rgba(233,150,180,0.7)", transform: `rotate(${t * 260}deg)`, zIndex: 5 }} />; })}
      <SpotCone x={420} top={30} topW={60} botW={380} h={520} color="rgba(190,240,220,0.10)" />
      {/* the target stack: 5 code bricks, ONE gets severed */}
      <div style={{ position: "absolute", left: 300, top: 400, width: 430, height: 96, borderRadius: 12, background: "rgba(10,26,24,0.6)", border: "2.5px solid rgba(120,180,160,0.3)", zIndex: 12, display: "flex", alignItems: "center", padding: "0 12px", gap: 9 }}>
        {[0, 1, 2, 3, 4].map((i) => { const target = i === 2; return (
          <div key={i} style={{ flex: 1, height: 58, borderRadius: 7, background: target ? (cutDone ? "rgba(63,158,116,0.95)" : "#C8A24C") : "#3A5C52", border: `2.5px solid ${target ? (cutDone ? "#7FE0B4" : "#E7CB7A") : "rgba(150,210,190,0.5)"}`, opacity: target ? 1 : 0.55, transform: target && slashE > 0 ? `translateY(${-slashE * 26}px)` : "none", boxShadow: target ? (cutDone ? "0 0 22px rgba(63,158,116,0.9)" : `0 0 0 4px rgba(231,178,76,${0.5 + Math.sin(lf / 5) * 0.3}), 0 0 20px rgba(200,162,76,0.7)`) : "none", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: mono, fontSize: 14, color: target ? "#101C18" : "rgba(200,240,220,0.8)" }}>{target ? "the fix" : "works"}</div>); })}
      </div>
      {/* the slash flash */}
      {slashE > 0 && <div style={{ position: "absolute", left: 280, top: 438, width: 470, height: 7, background: "linear-gradient(90deg, transparent, #FFFFFF, transparent)", opacity: slashE, transform: `rotate(-9deg) scaleX(${1 + slashE * 0.6})`, boxShadow: "0 0 24px #fff", zIndex: 24 }} />}
      {cutDone && <Stamp x={516} y={352} s={Math.min(1.1, spr(lf, slashAt + 5, 10, 220))} text="1 CUT" c={GREEN} />}
      <Sparkles lf={lf} at={3.65} x={516} y={440} n={12} spread={170} colors={["#7FE0B4", "#fff", GOLD]} dur={0.8} />
      {/* the samurai */}
      <div style={{ position: "absolute", left: 110, bottom: 122, width: 250, zIndex: 20 }}>
        <Mascot lf={lf} size={250} nodAmp={1.6} nodSpeed={10} gaze={7} samurai={1} stern={lf < slashAt ? 0.55 : 0} cheer={cutDone ? 0.4 : 0} />
        <div style={{ position: "absolute", right: -78, top: lf >= slashAt ? 80 : -56, transform: `rotate(${lf >= slashAt ? 48 : -58}deg)`, transformOrigin: "18% 84%", zIndex: 22 }}><Katana s={160} /></div>
      </div>
      ...
      {/* THE FOIL: rewrite-berserker at samurai scale, raging on the same baseline */}
      {wreckOn && (<>
        <div style={{ position: "absolute", left: 700, bottom: 122, width: 210, zIndex: 15, transform: `rotate(${Math.sin(lf * 1.3) * 6}deg) scaleX(-1)`, transformOrigin: "50% 100%", filter: "drop-shadow(0 0 18px rgba(220,70,40,0.55))" }}>
          <Mascot lf={lf * 1.6} size={210} nodAmp={6} nodSpeed={3} tint="#9A5450" xeyes={1} />
        </div>
        {/* rage aura */}
        <div style={{ position: "absolute", left: 690, bottom: 130, width: 230, height: 230, borderRadius: "50%", background: "radial-gradient(circle, rgba(220,80,40,0.22), transparent 62%)", zIndex: 14, transform: `scale(${1 + Math.sin(lf / 5) * 0.08})` }} />
        {/* the wrecking ball swings big */}
        <div style={{ position: "absolute", left: 892, bottom: 300, width: 7, height: 160, background: "#3A3430", transform: `rotate(${wSwing * 38}deg)`, transformOrigin: "50% 0%", zIndex: 16 }}>
          <div style={{ position: "absolute", left: -37, bottom: -66, width: 80, height: 80, borderRadius: "50%", background: "radial-gradient(circle at 36% 30%, #6A6A76, #26262E)", border: "4px solid #17171C" }} />
        </div>
        {/* shredded red rewrite-cards at its feet + dust */}
        <div style={{ position: "absolute", left: 690, bottom: 112, width: 280, height: 70, zIndex: 14 }}>
          {Array.from({ length: 10 }).map((_, i) => <div key={i} style={{ position: "absolute", left: seed(i * 2) * 230, bottom: seed(i) * 34, width: 40, height: 26, borderRadius: 4, background: "#33272A", border: "2.5px solid #C44A3A", transform: `rotate(${seed(i * 3) * 80 - 40}deg)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, color: "#E06050", fontFamily: mono }}>✗</div>)}
        </div>
        {Array.from({ length: 6 }).map((_, i) => { const t = ((lf / 26 + seed(i * 2)) % 1); return <div key={`d${i}`} style={{ position: "absolute", left: 720 + seed(i) * 200, bottom: 150 + t * 80, width: 28 * (1 - t) + 8, height: 28 * (1 - t) + 8, borderRadius: "50%", background: "rgba(170,150,150,0.28)", opacity: 1 - t, zIndex: 13 }} />; })}
        <Stamp x={806} y={318} s={Math.min(1.05, spr(lf, fr(6.4), 10, 210))} text="REWRITES EVERYTHING" c={"#A24A3E"} rot={4} />
      </>)}
    </>
  );
};
...
const HeroHeader: React.FC<{ f: number }> = ({ f }) => {
  ...
  return (
    <div style={{ position: "absolute", left: 0, right: 0, top: 296, display: "flex", justifyContent: "center", zIndex: 200, opacity: out, transform: `translateY(${(1 - settle) * -14}px)` }}>
      <div style={{ display: "inline-flex", alignItems: "center", gap: 14, padding: "16px 34px", borderRadius: 999, background: "#FFFFFF", border: "3px solid #E7E2D6", boxShadow: "0 18px 44px -12px rgba(20,26,45,0.4)" }}>
        <span style={{ fontFamily: inter.fontFamily, fontWeight: 900, fontSize: 40, color: INK, letterSpacing: "0.01em" }}>THE <span style={{ color: CLAY }}>5</span> CLAUDE CODE SKILLS THAT MATTER</span>
      </div>
    </div>
  );
};
...
const ProgressBar: React.FC = () => {
  ...
  const marks = [L[1], L[2], L[3], L[4], L[5]]; const STARS = [4.6, 32.2, 42.0]; const PELLETS = [1.0, 3.0, 6.2, 11.6, 14.4, 20.0, 24.8, 30.0, 35.6, 41.7, 48.4, 51.1];
...
// ============================== MAIN ==============================
export const ClaudePowersReel: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: CREAM, fontFamily: inter.fontFamily }}>
      <Audio src={staticFile("vo_powers.wav")} />
      <Audio loop src={staticFile("powers_bed.wav")} volume={(ff) => interpolate(ff, [0, fr(0.4), fr(CUT) - 20, fr(CUT)], [0.42, 0.42, 0.42, 0.14], { extrapolateRight: "clamp" })} />
      {/* ===== SFX: hook ===== */}
      <Sfx at={0} src="lib_deep_whoosh.wav" v={0.55} dur={1.0} />
      <Sfx at={0.85} src="thock.wav" v={0.6} dur={0.6} />
      <Sfx at={0.9} src="lib_boom.wav" v={0.5} dur={1.2} />
      {[2.7, 3.0, 3.3, 3.6, 3.9].map((t, i) => <Sfx key={`gb${i}`} at={t} src={`blip${i + 1}.wav`} v={0.3} dur={0.4} />)}
      <Sfx at={3.6} src="riser.wav" v={0.4} dur={1.1} />
      <Sfx at={4.55} src="snap.wav" v={0.65} dur={0.5} />
      <Sfx at={4.6} src="lib_boom.wav" v={0.55} dur={1.4} />
      <Sfx at={4.7} src="angelic.wav" v={0.34} dur={1.8} />
      <Sfx at={4.75} src="crowd_cheer.wav" v={0.2} dur={1.6} />
      ...
      {/* ===== S1 painter ===== */}
      {[1.0, 1.45, 1.9].map((d, i) => <Sfx key={`br${i}`} at={L[1] + d} src="swish.wav" v={0.3} dur={0.4} />)}
      ...
      {/* ===== S2 assemble ===== */}
      {[0.5, 0.9, 1.3, 1.7, 2.1].map((d, i) => <Sfx key={`ld${i}`} at={L[2] + d} src="impact.wav" v={0.3 + i * 0.04} dur={0.5} />)}
      ...
      <Sfx at={L[2] + 8.3} src="crowd_cheer.wav" v={0.16} dur={1.4} />
      {/* ===== S3 wizard vs balrog ===== */}
      ...
      <Sfx at={L[3] + 4.6} src="shimmer.wav" v={0.34} dur={1.2} />
      ...
      {/* ===== S4 samurai ===== */}
      <Sfx at={L[4] + 0.6} src="shimmer.wav" v={0.22} dur={1.6} />
      <Sfx at={L[4] + 3.5} src="slash.wav" v={0.55} dur={0.6} />
      <Sfx at={L[4] + 3.7} src="ding.wav" v={0.34} dur={0.5} />
      <Sfx at={L[4] + 5.3} src="crash.wav" v={0.3} dur={0.8} />
      <Sfx at={L[4] + 6.4} src="crash.wav" v={0.24} dur={0.8} />
      {/* ===== S5 speedrun ===== */}
      {[0.9, 1.9].map((d, i) => <Sfx key={`co${i}`} at={L[5] + d} src="blip3.wav" v={0.32} dur={0.4} />)}
      <Sfx at={L[5] + 1.7} src="crash.wav" v={0.22} dur={0.5} />
      <Sfx at={L[5] + 2.9} src="resolve.wav" v={0.36} dur={0.8} />
      <Sfx at={L[5] + 4.4} src="swooshup.wav" v={0.4} dur={0.6} />
      <Sfx at={L[5] + 4.5} src="crowd_cheers2.wav" v={0.2} dur={1.4} />
      {/* ===== CTA ===== */}
