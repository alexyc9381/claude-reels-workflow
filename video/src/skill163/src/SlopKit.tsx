...
export const Mascot: React.FC<{ lf: number; size?: number; gaze?: number; nodAmp?: number; nodSpeed?: number; shock?: number; cheer?: number; stern?: number; glasses?: number; wizard?: number; constr?: number; chef?: number; suit?: number; beard?: number; xeyes?: number; samurai?: number; fro?: number; girl?: number; cop?: number; prof?: number; capeC?: string; tint?: string; powerSuit?: number }> = ({ lf, size = 250, gaze = 0, nodAmp = 3.5, nodSpeed = 10, shock = 0, cheer = 0, stern = 0, glasses = 0, wizard = 0, constr = 0, chef = 0, suit = 0, beard = 0, xeyes = 0, samurai = 0, fro = 0, girl = 0, cop = 0, prof = 0, capeC, tint, powerSuit=0 }) => {
  const C = tint || "#D97757";
  ...
  return (
    <div style={{ width: size, height: size, position: "relative", transform: `translateY(${-hop - jump}px) scaleY(${squash})`, transformOrigin: "50% 100%" }}>
      <svg viewBox="0 0 200 200" width={size} height={size} shapeRendering="crispEdges" style={{ overflow: "visible" }}>
        ...
        {capeC && <>
          {powerSuit>0?<path d={`M33 53L17 ${142+Math.sin(lf/4)*8}L46 164L76 156L104 174L133 156L163 164L190 ${146-Math.sin(lf/4)*9}L165 53Z`} fill="#493D85" stroke="#F5CD73" strokeWidth="4"/>:<rect x={24} y={50} width={152} height={104 + Math.sin(lf / 6) * 6} fill={capeC} transform={`rotate(${Math.sin(lf / 7) * 2.5} 100 56)`} />}
        ...
        {/* Superpowers armor snaps onto the canonical body; face remains unobstructed. */}
        {powerSuit>0&&<g opacity={Math.min(1,powerSuit*3)} shapeRendering="geometricPrecision">
          <path d={`M34 ${106+68*(1-powerSuit)}H73L100 ${119+68*(1-powerSuit)}L127 ${106+68*(1-powerSuit)}H166V146H34Z`} fill="#153F57" stroke="#F1C56B" strokeWidth="3"/>
          <path d={`M34 ${106+68*(1-powerSuit)}L70 136H88M166 ${106+68*(1-powerSuit)}L130 136H112`} stroke="#91EDD0" strokeWidth="4" fill="none"/>
          <g transform={`translate(${-74*(1-powerSuit)} 0)`}><path d="M33 91H60L68 111H33Z" fill="#235D70" stroke="#F2C96F" strokeWidth="4"/></g>
          <g transform={`translate(${74*(1-powerSuit)} 0)`}><path d="M140 91H167V111H132Z" fill="#235D70" stroke="#F2C96F" strokeWidth="4"/></g>
          <path d="M43 69L57 64H67M133 64H148L158 69" stroke="#C9FCE4" strokeWidth="5" fill="none"/>
          <path d="M97 114L87 132H98L94 144L113 124H102L111 114Z" fill="#FFF4AE" stroke="#81EAC5" strokeWidth="2"/>
          <path d="M35 143H165" stroke="#EFC879" strokeWidth="5"/>
        </g>}
        {/* business suit + tie */}
        {suit > 0 && <>
        ...
        {powerSuit>0&&<g opacity={powerSuit} fill="#163C54" stroke="#EBC477" strokeWidth="3">{[52,77,124,149].map((lx,i)=><path key={i} d={`M${lx} ${165-legLift(i%2)}H${lx+17}V${184-legLift(i%2)}H${lx-5}V${176-legLift(i%2)}H${lx}Z`}/>)}</g>}
...
const ProgressBarInner: React.FC = () => {
  ...
  return (
    <div style={{ position: "absolute", left: 46, right: 46, top: 262, height: 60, zIndex: 120 }}>
      ...
      {PELLETS.map((np, i) => (np > p ? <div key={`pl${i}`} style={{ position: "absolute", left: `${np * 100}%`, top: 31, width: 13, height: 13, borderRadius: "50%", background: GOLD, border: "2px solid #F6E4A0", boxShadow: `0 0 9px ${GOLD}`, opacity: 0.9, transform: `translate(-50%,-50%) scale(${1 + Math.sin(f / 7 + i * 2) * 0.16})` }} /> : null))}
