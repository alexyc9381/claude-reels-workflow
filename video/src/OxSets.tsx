...
import type { Place } from "./OxWorld";
...
export const Hall: React.FC<{ p: Place; f: number; dx?: number;
  ...
  /** per-cut RAKE PITCH — the band count, which moves every band EDGE */
  rakeN?: number }> =
  ({ p, f, dx = 0, lamp: kx, overhead = "truss", rake = 0.28, rakeX = 0, rakeRate = 2.1,
     bands = 3, kind = "bay", grit = 1, z = 0, rakeN = 7 }) => (
  <>
    ...
    {rake > 0 && <Rake f={f} y={0} h={H} c={p.key} o={rake} x0={rakeX} rate={rakeRate} z={z + 21} n={rakeN} />}
  </>
);
