...
import { ShopWall, BayLamp, TyreStack, Toolbox, Drum, Lift, Chain, Hook, Tag, HangPart, Hoist, CrewBand } from "./RpsSets";
...
export const LIFTHOOK: React.FC<SP> = ({ v, dur }) => {
  ...
  /* ⭐ TWO RISES, so the last third is a travel and not a wait: up to the INTAKE (f14-52), the
     lock, then on up toward the DOME (f60-100), which is descending to meet him and is still
     30px above his head at the cut */
  const rise = 6 + 300 * E(f, 14, 52, 0, 1, IO) + 80 * E(f, 60, 100, 0, 1, IO);
  const platTop = GY - 18 - rise - 26;
  const meet = E(f, 14, 52, 0, 1, IO);                      /* the parts come down to meet him */
  const lock = E(f, 52, 58, 0, 1, BACK);
  const locked = f >= 52;
  const impact = f >= 52 ? Math.exp(-(f - 52) / 4) : 0;
  ...
  return (
    <Scene p={p} slug="" push={[0, dur, 1.0]} vig={0.40}>
      ...
      {/* the four hoists — the parts the lift is rising toward; the whole gantry shudders on the jolts */}
      {REPOS.map((r, i) => {
        const len = HOIST_LEN[i] + meet * 60 + (i === 0 && locked ? -40 * E(f, 52, 64, 0, 1, OUT) : 0) + (i === 2 ? coreDown : 0);
        const sw = i === 1 ? swing2 : Math.sin(f / 13 + i * 1.7) * 1.6 + jolt * 0.25 * (i % 2 ? 1 : -1);
        return (
          <Hoist key={r.key} repo={r} x={HOIST_X[i] + L.b * 0.2 + jolt * 0.4} len={len} f={f} z={60 + i} swing={sw} partSize={250}
            tag={false} hidePart={i === 0 && locked} />
        );
      })}
      ...
      <Rig f={f} x={HX} y={platTop + 20 - jolt} size={HS} z={56} act={3} ph={0.3} kit={kit}
        strain={f < 14 ? 0.32 : impact * 0.7} gaze={f < 52 ? -0.6 : 0.4} shock={impact > 0.4 ? 0.6 : 0}
        cheer={f < 52 ? 0.25 * E(f, 14, 30, 0, 1, OUT) : E(f, 64, 78, 0.25, 0.75, OUT)} />
      {/* the lock costs: a ring, sparks off the hip, dust off the platform */}
      {locked && f < 78 && (<>
        <Ring x={a.hipL.x} y={a.hipL.y} f={f} at={52} c={mxh(REPOS[0].c, 0.4)} z={72} s={1.4} dur={18} />
        <Fall x={a.hipL.x - 80} y={a.hipL.y} w={160} f={f} at={52} n={12} z={74} c={mxh(GOLD, 0.3)} rate={1.8} s={1.0} />
        <Puff x={HX} y={platTop + 26} f={f} at={52} c="#E4DCC8" z={58} n={10} s={1.2} up={0.2} />
      </>)}
      <ShopCrew f={f} v={v} />
...
export const DROPHOOK: React.FC<SP> = ({ v, dur }) => {
  ...
  const HOX = [HX - 70, HX + 90, HX, HX + 40];
      ...
      {REPOS.map((r, i) => {
        ...
        return (
          <React.Fragment key={r.key}>
            ...
            {!gone && <HangPart repo={r} x={px} y={py} size={250} z={70} f={f} swing={k * 8 * (i % 2 ? 1 : -1)} />}
          </React.Fragment>
        );
      })}
...
export const PITHOOK: React.FC<SP> = ({ v, dur }) => {
  ...
  const camS = 1.14 + 0.12 * E(f, 16, dur, 0, 1, LIN);
  ...
  return (
    <Scene p={p} slug="" push={[0, dur, 1.03]} vig={0.40}>
      ...
      <Room p={p} f={f} bands={2} kind="rack" overhead="gantry" rake={0.07} rakeX={RAKE_X[v]} rakeRate={2.8 * RAKE_K[v]}
        rakeN={RAKE_N[v]} floorKind="slab" grit={0.5} window={null} />
      <ShopWall p={p} f={f} seed={0} bay={null} door pegX={560} pegW={400} />
      {/* the empty hoists, hooks swinging: the parts have been taken down */}
      {REPOS.map((r, i) => (
        <React.Fragment key={r.key}>
      ...
      {REPOS.map((r, i) => {
        ...
        return (
          <React.Fragment key={r.key}>
            ...
            {f < t1 + 4 && (
              <HangPart repo={r} x={rx - side * 20} y={GY - 4 - 176 * 0.9 - 30 - raise * 60} size={200} z={68} f={f}
                swing={Math.sin(f / 3) * 6 * (1 - raise)} />
            )}
          </React.Fragment>
        );
      })}
