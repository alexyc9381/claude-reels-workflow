...
const Preview=()=>{
 const f=useCurrentFrame(); const pull=p(f,4,21), reveal=p(f,7,23); const collapse=p(f,34,60), chat=p(f,62,73), waste=p(f,157,172), next=p(f,223,239);
 ...
 return <AbsoluteFill style={{background:paper,color:ink,fontFamily:'Arial,sans-serif',overflow:'hidden'}}>
  ...
  <div style={{position:'absolute',left:mix(0,80,pull),top:mix(0,255,pull),width:mix(1920,560,pull),height:mix(1080,710,pull),borderRadius:mix(0,24,pull),overflow:'hidden',boxShadow:'0 22px 50px #32231420'}}>
   <AbsoluteFill style={{transform:`scale(${mix(mix(1,1.16,p(f,0,4)),1,pull)})`}}>
  ...
  <Audio src={staticFile('opening-v2-mix.wav')}/>
 </AbsoluteFill>;
};
registerRoot(()=> <Composition id="ClaudeOpeningV2" component={Preview} width={1920} height={1080} fps={30000/1001} durationInFrames={300}/>);
