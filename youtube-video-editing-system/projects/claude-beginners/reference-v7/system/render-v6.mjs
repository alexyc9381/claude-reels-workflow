...
const work=path.resolve(root,'../../work/v6-full');fs.mkdirSync(path.join(work,'empty'),{recursive:true});
const serveUrl=await bundle({entryPoint:path.join(root,'src/v6-full.tsx'),outDir:path.join(work,'bundle'),publicDir:path.join(work,'empty')});
...
try {
const composition=await selectComposition({serveUrl,id:'ClaudeFullV6',puppeteerInstance:browser});
if(!process.argv.includes('--render-only'))for(const frame of [0,12,45,59,61,63,65,68,72,78]){await renderStill({serveUrl,composition,puppeteerInstance:browser,frame,output:path.join(work,`frame-${frame}.png`),imageFormat:'png'});}
