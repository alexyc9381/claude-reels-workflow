...
const work=path.resolve(root,'../../work/v3-full');fs.mkdirSync(path.join(work,'empty'),{recursive:true});
const serveUrl=await bundle({entryPoint:path.join(root,'src/v3-full.tsx'),outDir:path.join(work,'bundle'),publicDir:path.join(work,'empty')});
...
try {
const composition=await selectComposition({serveUrl,id:'ClaudeFullV3',puppeteerInstance:browser});
if(!process.argv.includes('--render-only'))for(const frame of [35,990,1039,1138,1189,1995,2051,2108,2178,2318,2500,2640,3715,5610,5800,6490,6589,6928,7100,7330,8960,9399,9455,9525,10498,11030].map(map)){await renderStill({serveUrl,composition,puppeteerInstance:browser,frame,output:path.join(work,`frame-${frame}.png`),imageFormat:'png'});}
