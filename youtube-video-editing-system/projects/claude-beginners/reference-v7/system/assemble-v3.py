...
root=Path(__file__).resolve().parents[1]; work=root.parents[1]/'work'; chunks=work/'v3-full'; fps=30000/1001
files=[]
...
out=root.parent/'Claude-Full-Animated-V3-Preview.mp4'
subprocess.run([str(work/'bin/ffmpeg'),'-hide_banner','-y','-f','concat','-safe','0','-i',str(manifest),'-i',str(root/'public/v3-mix.wav'),'-map','0:v:0','-map','1:a:0','-c:v','copy','-c:a','aac','-b:a','192k','-t',str(json.loads((root/'src/revision-timing.json').read_text())['frames']/fps),'-video_track_timescale','30000','-movflags','+faststart',str(out)],check=True)
print(out)
