...
root=Path(__file__).resolve().parents[1];sr=44100;dur=900/(30000/1001);n=round(sr*dur);rng=np.random.default_rng(913)
...
def bell(hz):
 ...
 return (np.sin(2*np.pi*hz*t)*np.exp(-t*5.4)+.32*np.sin(2*np.pi*hz*2.76*t)*np.exp(-t*8.5)+.12*np.sin(2*np.pi*hz*4.08*t)*np.exp(-t*12))*(1-np.exp(-t*650))
def pop(hz=470):
...
cue(bell(880)*.6,418,.043,'chart completion sparkle',.2)
cue(snap(),448,.054,'research intake switch',-.2)
cue(bell(740),475,.058,'research success chime',.2)
...
cue(bell(1318.51)*.65,755,.035,'table settles',.2)
cue(pop(290),811,.07,'resource card arrival')
cue(snap(),839,.065,'copy click',.2)
cue(bell(1046.5),847,.062,'copied confirmation ding',.2)
