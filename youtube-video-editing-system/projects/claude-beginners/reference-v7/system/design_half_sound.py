...
root=Path(__file__).resolve().parents[1];sr=44100;dur=5592/(30000/1001);n=round(sr*dur);rng=np.random.default_rng(913)
...
for frame in [2490,2595,3056,3976,4010,4050,4246,4366,4594,4678,4958]:cue(snap(),frame,.032,'interface control click',.1)
...
save(root/'public/half-mix.wav',mix);save(root/'public/half-effects.wav',fx)
...
(root/'half-sound-design.json').write_text(json.dumps(receipt,indent=2));print(json.dumps({k:v for k,v in receipt.items() if k!='cues'},indent=2))
