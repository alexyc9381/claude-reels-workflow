...
const work=path.resolve(root,'../../work/v5-full');fs.mkdirSync(path.join(work,'empty'),{recursive:true});
const serveUrl=await bundle({entryPoint:path.join(root,'src/v5-full.tsx'),outDir:path.join(work,'bundle'),publicDir:path.join(work,'empty')});
const pub=path.join(serveUrl,'public');fs.rmSync(pub,{recursive:true,force:true});fs.symlinkSync(path.join(root,'public'),pub,'dir');
const timing=JSON.parse(fs.readFileSync(path.join(root,'src/v5-timing.json'),'utf8'));
...
try {
const composition=await selectComposition({serveUrl,id:'ClaudeFullV5',puppeteerInstance:browser});
if(!process.argv.includes('--render-only'))for(const frame of [0,12,45,59,68,78,map(856),map(10825),map(11036),10922,10962,11026]){await renderStill({serveUrl,composition,puppeteerInstance:browser,frame,output:path.join(work,`frame-${frame}.png`),imageFormat:'png'});}
