...
root=Path(__file__).resolve().parents[1];sr=44100;dur=1800/(30000/1001);n=round(sr*dur);rng=np.random.default_rng(913)
...
for frame in [1000,1021,1042,1063]:cue(tone(.12,310,31),frame,.032,'usage segment drains')
...
save(root/'public/opening-60s-mix.wav',mix);save(root/'public/opening-60s-effects.wav',fx)
...
(root/'opening-60s-sound-design.json').write_text(json.dumps(receipt,indent=2));print(json.dumps({k:v for k,v in receipt.items() if k!='cues'},indent=2))
