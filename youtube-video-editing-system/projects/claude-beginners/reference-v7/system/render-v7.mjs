...
const work=path.resolve(root,'../../work/v7-full');fs.mkdirSync(path.join(work,'empty'),{recursive:true});
const serveUrl=await bundle({entryPoint:path.join(root,'src/v7-full.tsx'),outDir:path.join(work,'bundle'),publicDir:path.join(work,'empty')});
...
try {
const composition=await selectComposition({serveUrl,id:'ClaudeFullV7',puppeteerInstance:browser});
