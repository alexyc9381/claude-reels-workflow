...
const old=JSON.parse(readFileSync(path.join(work,'revision-v4/roughcut-v3.props.json'))).manifest;
assert.equal(m.editVersion,'v4');assert.deepEqual(m.cameras,old.cameras);assert.deepEqual(m.obs,old.obs);
...
const assembly=read('RoughCut.tsx'),polish=read('YouTubeV4.tsx'),scenes=read('YouTubeV4Scenes.tsx');
assert.match(assembly,/data-audio-role="obs-narration" src=\{staticFile\(m.obs.source\)\}/);
for(const code of [assembly,polish,read('YouTubeV4Primitives.tsx')])for(const tag of code.match(/<OffthreadVideo[\s\S]*?\/>/g)||[])assert.match(tag,/\bmuted\b/);
...
const {soundEvents}=evaluate(evCode,{roughTimeline:timing.roughTimeline,roughChapters:timing.roughChapters,v4Cues});
