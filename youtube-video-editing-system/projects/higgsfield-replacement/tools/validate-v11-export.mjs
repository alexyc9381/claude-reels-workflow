...
const output=path.join(base,'outputs/higgsfield-replacement-edit-v11.mp4');
...
if(existsSync(priorAudio))assert.equal(result.audioMatchesV9Packets,false,'V11 must contain repaired narration and new source-anchored sound design');
result.currentAudioContractVerified=true;
assert.equal(equivalentChunks.length,0,'V11 reframes the whole screen and requires fresh picture chunks');
writeFileSync(path.join(base,'work/higgsfield-replacement/revision-v11/export-validation.json'),JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2));
