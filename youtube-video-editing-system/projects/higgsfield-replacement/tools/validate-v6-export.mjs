...
const output=path.join(base,'outputs/higgsfield-replacement-edit-v6.mp4');
...
assert.equal(m.editVersion,'v6');assert.equal(v.width,1920);assert.equal(v.height,1080);assert.equal(v.r_frame_rate,'30/1');assert.equal(Number(v.nb_frames),frames);
...
writeFileSync(path.join(base,'work/higgsfield-replacement/revision-v6/export-validation.json'),JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2));
