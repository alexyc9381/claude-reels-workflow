...
const rows=JSON.parse(readFileSync(path.join(project,'v12-timeline.json'))),get=id=>rows.find(r=>r.id===id),fps=30;
const sourceMode=process.argv.includes('--source'),out=path.join(work,'revision-v12',sourceMode?'edl-joins':'encoded-joins');mkdirSync(out,{recursive:true});
...
for(const [id,start,end] of checks){
 ...
 }else execFileSync(ff,['-v','error','-y','-ss',String(start),'-i',path.join(base,'outputs/higgsfield-replacement-edit-v12.mp4'),'-t',String(end-start),'-ac','1','-ar','16000',wav]);
