...
const revision=process.env.REVIEW_REVISION||'v12';if(!/^v\d+$/.test(revision))throw Error('Invalid revision');
const rows=JSON.parse(readFileSync(path.join(project,revision+'-timeline.json'))),get=id=>rows.find(r=>r.id===id),fps=30;
const sourceMode=process.argv.includes('--source'),out=path.join(work,'revision-'+revision,sourceMode?'edl-joins':'encoded-joins');mkdirSync(out,{recursive:true});
...
const checks=[['bridge',get('s033').from/fps-2,get('s033').from/fps+12],['playback-reaction',get('s035').from/fps+3,get('s036').from/fps+3],['retake-removal',get('s037').from/fps-3,get('s037').from/fps+8],['ending',get('s044').from/fps,(rows.at(-1).from+rows.at(-1).duration)/fps]];
const results=[];
for(const [id,start,end] of checks){
 ...
 }else execFileSync(ff,['-v','error','-y','-ss',String(start),'-i',path.join(base,`outputs/higgsfield-replacement-edit-${revision}.mp4`),'-t',String(end-start),'-ac','1','-ar','16000',wav]);
