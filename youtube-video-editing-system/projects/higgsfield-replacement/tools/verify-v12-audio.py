...
# Decode the delivery from its true start once. Input-side random seeking near
# an MP4's final GOP can move the decoded audio origin (~99 ms in this file),
# creating a false sync failure which does not occur during continuous playback.
delivery=np.frombuffer(subprocess.check_output([ff,'-v','error','-i',str(candidate),'-vn','-ac','1','-ar','16000','-f','f32le','pipe:1']),dtype=np.float32)

def samples(file,start,duration=1.2):
    if file==candidate:
        return delivery[round(start*16000):round((start+duration)*16000)].astype(np.float64)
...
for output_time in [50,86,145,194,229,273,282,326,386,396,418.8,423,440.7,443,455]:
