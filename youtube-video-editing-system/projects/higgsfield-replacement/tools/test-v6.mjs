...
const old=JSON.parse(readFileSync(path.join(work,'revision-v6/roughcut-v5.props.json'))).manifest;
assert.equal(m.editVersion,'v6');assert.deepEqual(m.cameras,old.cameras);assert.deepEqual(m.obs,old.obs);
...
assert.ok(get('r-teaser').duration>=117);assert.equal(get('r-teaser').end,1996.37);assert.equal(get('s007').start,242);assert.equal(get('s016').start,800.46);assert.equal(get('s019').start,985.76);assert.equal(get('s045').end,2459.03);
...
const assembly=read('RoughCut.tsx'),polish=read('YouTubeV6.tsx'),scenes=read('EditorialScenes.tsx');
assert.match(assembly,/data-audio-role="obs-narration" src=\{staticFile\(m.obs.source\)\}/);
for(const code of [assembly,polish,read('YouTubeV6Primitives.tsx')])for(const tag of code.match(/<OffthreadVideo[\s\S]*?\/>/g)||[])assert.match(tag,/\bmuted\b/);
...
assert.match(read('YouTubeV6Primitives.tsx'),/=>null;/);
...
assert.match(read('YouTubeV6Primitives.tsx'),/blur\(22px\)/);
assert.match(scenes,/w=\{860\}/);
assert.doesNotMatch(polish+scenes,/ProductionWorkshop|FilingIntake|DispatchStamp|ModelReel|Conveyor/);
assert.match(read('CameraPlate.tsx'),/blur\(5px\)/);
