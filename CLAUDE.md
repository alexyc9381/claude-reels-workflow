...
<!-- INDEX:AUTO -->
| reels (code in-repo) | **103** | factory logs | **62** |
|---|---|---|---|
| storyboards | 64 | caption files | 113 |
| memory `.md` | 214 | creator packs | 3 |
...
| **got "it goes dead still right before it changes"** | [`docs/ANIMATION-QUALITY.md`](docs/ANIMATION-QUALITY.md) §23 + [`tools/precut_audit.py`](tools/precut_audit.py) — ⭐ an `IO`/`OUT` ease DECELERATES into its end whether or not that end is on screen, so extending it past the cut fixes nothing. Anything crossing a cut must be `LIN` or `IN`. ⛔ `scene_tail_audit`'s quarters are too coarse to see it: it rated two dead scenes "ok" |
| **got "not enough motion in the top / only the bottom is moving"** | [`docs/ANIMATION-QUALITY.md`](docs/ANIMATION-QUALITY.md) §24 — ⭐ split the panel and measure the HALVES. A scene can pass motion, tail AND pre-cut audits with half its frame dead, because all three average over the panel. Suspect a **z-index** before a rate |
| **got "still too static" after already adding motion** | [`docs/ANIMATION-QUALITY.md`](docs/ANIMATION-QUALITY.md) §25-26 — ⭐ a grid filling is ONE IDEA REPEATED N TIMES. If three rounds of animating the OBJECTS did not move the note, the LAYOUT is the static thing. And check the RATE: one hook ran seven rounds of notes at 36°/sec |
