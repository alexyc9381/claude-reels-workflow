...
segs, info = m.transcribe("FREE_raw_16k.wav", word_timestamps=True, vad_filter=False)
words=[]; lines=[]
for s in segs:
...
# longest words = buried restarts
lw = sorted(words, key=lambda x:-(x["end"]-x["start"]))[:12]
print("\nLONGEST WORDS (a >1.0s word is a buried restart):")
