...
v=load(R/'vo/lm146/final-vo.wav');vo=np.zeros((N,2));vo[:min(len(v),N)]=v[:N]
...
cues=[(0,'impact','hero','Claude is already cranking and lowering the AI chip as the camera starts its push')]
def c(scene,frame,name,action,role='support'):cues.append((S[scene]+frame/30,name,role,action))
for f in [0,16]:c('hook',f,'lever','Claude rapidly cranks the hoist as the chip descends','texture')
c('hook',32,'thock','the chip lands; the laptop and Claude recoil','hero')
c('hook',40,'unlock','the installed processor powers the keyboard and circuit paths')
c('hook',38,'paper','Claude moves from the released crank to the mouse','texture')
c('hook',57,'key','Claude presses the mouse to open the display')
c('hook',64,'lever','Claude launches into the computer','texture')
c('hook',82,'thock','Claude lands inside the display','texture')
c('hook',92,'lever','the landing drives the processor shutters apart','texture')
c('hook',112,'pickup','the open processor sends signals across the computer','texture')
c('hook',150,'paper','the activated display expands into the real website')
...
for f in [4,23]:c('cta',f/1.5,'key','illustrated comment field and arrow press')
c('cta',31/1.5,'unlock','Claude reacts to the completed setup','hero')
...
c('cta',10/1.5,'lever','Claude launches toward the comment arrow','texture')
c('cta',36/1.5,'thock','Claude lands after pressing the illustrated arrow','texture')
...
for i,(sec,name,role,action) in enumerate(cues):
 x=load(bank/(name+'.wav'));target=-24 if sec==0 and role=='hero' else {'hero':-27,'support':-32,'texture':-36}[role];gain=target-active(aw(x));gain=min(gain,-7-20*np.log10(max(abs(x).max(),1e-9)));x*=10**(gain/20);start=round(sec*SR);nn=min(len(x),N-start);sfx[start:start+nn]+=x[:nn]
...
subprocess.run([str(F),'-y','-v','error','-i',str(R/'vo/lm146/prelimit.wav'),'-af',f'alimiter=limit=0.86:attack=2:release=50:level=false:latency=true,afade=t=out:st={DUR-.005:.3f}:d=0.005','-c:a','pcm_s16le',str(P/'master.wav')],check=True)
