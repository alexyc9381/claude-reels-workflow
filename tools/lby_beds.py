#!/usr/bin/env python3
"""REEL 130 — build the three reel-local music beds, measured.
...
BEDS = [
    ("130library_bed_house.wav", MASTER, SONG_START, 31.0),
    ("130library_bed_amber.wav", MASTER, SONG_START, 31.0),
    ("130library_bed_steel.wav", MASTER, SONG_START, 31.0),
]
...
def _assert_starts_at_the_top(src, start, master=MASTER, limit=8.0):
    ...
    eb = _env(f"{PUB}/{master}", start, 31.0); eb = (eb - eb.mean()) / (eb.std() + 1e-9)
