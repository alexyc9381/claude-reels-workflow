# No Code Alex YouTube editing system

Latest: [V15](../youtube-video-editing-system/projects/higgsfield-replacement/REVISION-V15.md) supersedes the first-minute price/production/roadmap/guide/wrapper graphics with visual-first actions, both real logos, original Claude acting and early actual OBS workflow proof. V14 opening geometry and later footage/audio remain. [Visual-first and title/thumbnail rules](../youtube-video-editing-system/VISUAL-FIRST-EXPLAINERS.md); [V15 resume prompt](../youtube-video-editing-system/projects/higgsfield-replacement/OTHER-MAC-HANDOFF-V15.md). Exact title/thumbnail not yet supplied; do not claim a verified silent opening match.

Preserved baseline: [V14](../youtube-video-editing-system/projects/higgsfield-replacement/REVISION-V14.md). Opening clips nearly fill the frame with slim margins; facecam overlays the lower center from frame zero; circular 1/2 badges sit inside the lower clip centers. This supersedes adding decorative elements below them. The next scene clearly contrasts the recorded monthly subscription example against a per-generation estimate without a false savings multiplier. Preserve all V13 EDL/audio/privacy/tutorial/reveal repairs. Claude Code/Codex/Cursor remain workflow destinations, not verified universal installers.

Canonical entry: [current direction](../youtube-video-editing-system/CURRENT-DIRECTION.md). Full indexed conversation learnings: [learning index](../youtube-video-editing-system/LEARNINGS-INDEX.md) and [decision ledger](../youtube-video-editing-system/REVISION-DECISIONS.md).

## Standing rules

- Remotion, never Supereditor. Separate 16:9 YouTube from the short-form reel chassis and retired Matchtern VSL style.
- OBS sole recorded narration; Sony muted. Preserve clap-measured offsets, complete words, useful teaching and context. Do not silently shorten a film by discarding useful content.
- One cumulative frame EDL for picture/audio/graphics/chapters. New source coverage requires corresponding camera-plate coverage.
- Screen large over cream/orange gradient; measured menu/Dock crop; typed field fully visible. Right rounded inset with independent crop; opening comparison may use bottom-center facecam.
- Per-frame person matte blurs room only, never face or a fixed oval. Fast-to-slow opening face push; smooth full-to-inset handoff.
- Mature Manrope sans-serif, white optical glass, orange-led color, strong contrast and centered safe hierarchy. No black glass, serif titles, competing abstract slab/ribbon, toy factories or empty generic rectangle/checkmark explainers.
- Distinct beginning → action → payoff for each explanation. Large original 2D sprites do real jobs; concurrent related supporting motion, natural contact/recovery, equal eye apertures. Existing role outfits are permitted by the later request.
- Abstract is allowed; arbitrary is not. Every scene follows a specific spoken clause and reaches a changed state. Do not substitute a spinning film prop or decorative inventory for an explanation. In A/B scenes, enlarge real playing footage and reduce graphics; protect the subject while cropping.
- Do not repeat whole scenes. Shared rigs/materials are fine. Prefer actual UI/logos; never substitute a different video inside a result-review card.
- Meaningful technical definitions only, legible copy and orange perimeter timer; vary tips, brands, text lenses, chapters and full scenes. Never cover active typing.
- Original glowing CGI is preserved with provenance, but **removed from the current Higgsfield film** per later explicit feedback.
- Tactile quiet action SFX and finite music beneath real narration; no constant dings. Source-boundary repair before masking a problem with sound. Loudness tests do not prove listening quality.
- Keep A/B proof and cost/claim verification separate from polish. New narration needs Alex's recording; do not synthesize his voice or advertise an unspecified bonus.
- Deliver the full rendered video, not only the hook. Inspect current-source output, audio joins, end word, chapters and screen clearance.
- Code/docs/manifests in Git; raw private media and exports outside Git. Push does not mean publish video.

## Current project

[Higgsfield Replacement V11](../youtube-video-editing-system/projects/higgsfield-replacement/REVISION-V11.md): new access/credential/rooftop/installation scenes; original role outfits and short-form WorldKit depth; persistent fal.ai checklist; larger OBS viewport; larger A/B reveal logos; earlier gift-style bonus with truthful countdown. Restore the full Veo landing and first reaction, remove repeated bridge/reaction and retake-adjacent lead-in. **V11 changes audio/EDL**, although the renderer selector intentionally remains editVersion v9. Current source/timestamp audit and V11 tests are authoritative, not the historical V9 timeline. Real six-recipe bonus remains present. No CGI, synthetic narration or footage substitution. Review status is recorded in the revision document; a successful render is not creative approval.

[Course transfers](../youtube-video-editing-system/TRANSFER-LOG.md) · [Content preservation](../youtube-video-editing-system/CONTENT-PRESERVATION.md) · [Sound principles](../youtube-video-editing-system/SOUND-DESIGN-PRINCIPLES.md) · [Voiceover explainers](../youtube-video-editing-system/VOICEOVER-EXPLAINER-MODE.md) · [Reproduction](../youtube-video-editing-system/projects/higgsfield-replacement/REPRODUCE.md).

The complete former note is preserved in [historical memory](../youtube-video-editing-system/history/MEMORY-PRE-V9.md). Its serif/no-costume/CGI and prior pending-status paragraphs are historical, not active instructions.
